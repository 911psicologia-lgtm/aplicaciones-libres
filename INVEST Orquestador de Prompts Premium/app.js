...
const STORE = "rizoma_promptlab_premium_v0_3_2";
const OLD_STORE = "rizoma_promptlab_premium_v0_3_1";
...
function generatePrompt(){
  ...
  const prompt = {
    ...
    instruccion_principal: `Actúa como un equipo senior-premium de agentes expertos. Responde exclusivamente en JSON válido, sin markdown exterior, sin texto antes ni después del JSON. ${instruccionNaturaleza}`,
...
function buildReport(){
  ...
  const finalText = r.informe_final || r.analisis || JSON.stringify(r,null,2);
...
${typeof finalText === "string" ? finalText : JSON.stringify(finalText,null,2)}
...
- ${recomendaciones.join("\n- ")}${referencias.length ? "\n\nREFERENCIAS BIBLIOGRÁFICAS\n"+referencias.map(x=>"• "+x).join("\n") : ""}`;
  ...
  state.currentReportHTML = buildReportHTML({title,p,d,resumen,analysis,finalText,limites,recomendaciones,referencias});
  $("reportTitle").textContent = title;
  $("reportOutput").innerHTML = state.currentReportHTML;
  go("report"); saveState();
}

function buildReportHTML({title,p,d,resumen,analysis,finalText,limites,recomendaciones,referencias}){
  ...
  return `<article class="report-doc">
    ...
    ${section("Consulta central", `<p><strong>${escapeHTML(d.title||"Sin título")}</strong></p><p>${escapeHTML(d.question||"No registrada")}</p><div class="chips">${natureBadge}<span class="chip">${escapeHTML(d.productType||"Producto no definido")}</span>${d.customProduct?`<span class="chip">${escapeHTML(d.customProduct)}</span>`:""}</div>`)}
    ...
    ${section("Respuesta estructurada", `<p>${escapeHTML(typeof finalText === "string" ? finalText : JSON.stringify(finalText,null,2)).replace(/\n/g,"<br>")}</p>`)}
