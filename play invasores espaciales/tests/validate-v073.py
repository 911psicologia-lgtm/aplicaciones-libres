...
sw=(ROOT/'sw.js').read_text(encoding='utf-8'); main=(ROOT/'js/main.js').read_text(encoding='utf-8')
...
cfg=(ROOT/'js/config.js').read_text(encoding='utf-8'); game=(ROOT/'js/game.js').read_text(encoding='utf-8')
check("VERSION: '0.7.6'" in cfg,'wrong app version')
check("mode: 'assist'" in cfg,'Reactive Matrix assist mode lost')
for token in ('battleRhythm:','subbossRhythm:','desperation:','hunterDoctrine:','formationCoordination:','escortDoctrine:','adaptiveMemory:','systemBreak:','supportNetwork:'):
    check(token in cfg,f'missing config: {token}')
for token in ('updateBossCounterWindow','mutateBossSignature','shootSubbossBasic','updateFormationDesperation','triggerKamikazeChain','combatPatternSnapshot','coordinateFormationStrike','updateBossHunterDoctrine','fireBossHuntPattern','updateBossAdaptiveEcho','noteBossSystemBreak','triggerSupportNetworkCollapse'):
    check(token in game,f'missing runtime: {token}')
check('!e.huntPending && !e.signaturePending && !e.echoPending' in game,'threat arbitration around adaptive telegraphs missing')
check('CONTRAPATRÓN' in game and 'LECTURA TÁCTICA' in game,'hunter doctrine cues missing')
check("escortRole=kamikaze?'rammer'" in game,'escort role differentiation missing')
...
check('ECO ADAPTATIVO' in game and 'SYSTEM BREAK' in game and 'RED ROTA' in game,'v0.7.6 tactical ecosystem cues missing')
