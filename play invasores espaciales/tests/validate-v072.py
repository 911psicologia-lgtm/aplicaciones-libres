...
check('starfall-shell-v0.7.2' in sw,'wrong service worker cache version')
check('build=0720' in main,'wrong service worker registration build')
...
check("VERSION: '0.7.2'" in cfg,'wrong app version')
check("mode: 'assist'" in cfg,'Reactive Matrix assist mode lost')
for token in ('battleRhythm:','subbossRhythm:','desperation:','hunterDoctrine:','formationCoordination:','escortDoctrine:'):
    check(token in cfg,f'missing config: {token}')
for token in ('updateBossCounterWindow','mutateBossSignature','shootSubbossBasic','updateFormationDesperation','triggerKamikazeChain','combatPatternSnapshot','coordinateFormationStrike','updateBossHunterDoctrine','fireBossHuntPattern'):
    check(token in game,f'missing runtime: {token}')
check('!e.huntPending && !e.signaturePending' in game,'threat arbitration around hunt telegraph missing')
