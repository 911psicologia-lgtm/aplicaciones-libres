  ...
  async function registerPWA(){
    ...
    try{
      ...
      for(const reg of regs){
        if(reg.scope===wantedScope && reg.active && !reg.active.scriptURL.includes('build=0760')){
          await reg.unregister();
        }
      }
      const reg=await navigator.serviceWorker.register('./sw.js?build=0760',{scope:'./',updateViaCache:'none'});
