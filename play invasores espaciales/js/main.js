...
(function(NS){
  ...
  const main = {
    init(){
      ...
      main.refreshPanels();
      main.refreshEconomy();
      UI.setStatus('v0.6.7 · cargando recursos esenciales…');
      NS.assets.loadAll().then(async()=>{ let h=NS.assets.health?.(); if(h?.failed?.length){ UI.setStatus(`v0.6.7 · reintentando ${h.failed.length} recurso(s)…`); h=await NS.assets.retryFailed?.(); } UI.setStatus(h?.failed?.length ? `v0.6.7 · ASSETS ${h.loaded}/${h.expected} · fallos ${h.failed.length} · juego con fallback` : 'v0.6.7 · ASSETS OK · W01–05 realistas · carga adaptativa'); });
      registerPWA();
  ...
  async function registerPWA(){
    ...
    try{
      ...
      for(const reg of regs){
        if(reg.scope===wantedScope && reg.active && !reg.active.scriptURL.includes('build=0670')){
          await reg.unregister();
        }
      }
      const reg=await navigator.serviceWorker.register('./sw.js?build=0670',{scope:'./',updateViaCache:'none'});
