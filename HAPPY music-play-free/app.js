...
const els = {
  ...
  homeRecent: $('#homeRecent'), homePlaylists: $('#homePlaylists'), homeAlbums: $('#homeAlbums'), homeAlbumsSection: $('#homeAlbumsSection'), homeRhythms: $('#homeRhythms'), homeRhythmsSection: $('#homeRhythmsSection'), homeArtists: $('#homeArtists'), homeArtistsSection: $('#homeArtistsSection'), homePodcasts: $('#homePodcasts'), homePodcastsSection: $('#homePodcastsSection'), homeAllPodcastsBtn: $('#homeAllPodcastsBtn'), homeMusicVideo: $('#homeMusicVideo'), homeMusicVideoSection: $('#homeMusicVideoSection'), homeSearchBtn: $('#homeSearchBtn'), homeAllPlaylistsBtn: $('#homeAllPlaylistsBtn'), homeAllAlbumsBtn: $('#homeAllAlbumsBtn'),
...
const state = {
  ...
  songCategories: [], songSource: 'all', soundMode: SOUND_MODES.auto, soundProfile: 'original', searchOriginPlaylistId: '', podcastRate: 1
};
...
function sampleListenSession(current,duration){
  ...
  if(!s.completed&&s.duration>0&&current/s.duration>=.85){
    ...
    track.completedPlays=(track.completedPlays||0)+1;track.lastCompleted=now();if(isPodcastTrack(track)){track.resumePosition=0;track.resumeDuration=s.duration||track.duration||0;track.resumeUpdatedAt=now();}saveRemoteTrack(track);recordHistory('complete',track,{ratio:current/s.duration});
  }
...
function restoreRemoteDockPosition(){
  if(!els.remoteDock||els.remoteDock.classList.contains('is-hidden'))return;let pos=null;try{pos=JSON.parse(localStorage.getItem('mpf-remote-dock-pos')||'null');}catch{}if(!pos||!Number.isFinite(pos.left)||!Number.isFinite(pos.top)){defaultRemoteDockPosition();return;}const c=clampRemoteDockPosition(pos.left,pos.top);els.remoteDock.style.right='auto';els.remoteDock.style.bottom='auto';els.remoteDock.style.left=`${c.left}px`;els.remoteDock.style.top=`${c.top}px`;
}
...
async function prepareNativePodcastStart(player,track){
  const isPod=isPodcastTrack(track),resumeAt=podcastResumePosition(track);
...
async function playYouTube(track){
  ...
  return new Promise(resolve=>{
    ...
    ytPlayer=new YT.Player('ytMainPlayer',{width:'100%',height:'100%',videoId:track.sourceKind==='youtube'?track.remoteId:undefined,playerVars:{autoplay:0,playsinline:1,controls:1,rel:0,...(location.origin&&location.origin!=='null'?{origin:location.origin}:{})},events:{
      onReady:async e=>{try{const frame=e.target.getIframe?.();if(frame){frame.setAttribute('allow','autoplay; encrypted-media; picture-in-picture');frame.referrerPolicy='strict-origin-when-cross-origin';}e.target.setVolume(Math.round(state.volume*100));const resumeAt=podcastResumePosition(track);if(isPodcastTrack(track))try{e.target.setPlaybackRate(state.podcastRate);}catch{}if(track.sourceKind==='youtube-playlist')e.target.loadPlaylist({listType:'playlist',list:track.remoteId,index:0,startSeconds:resumeAt||0});else{if(resumeAt)try{e.target.seekTo(resumeAt,true);}catch{}e.target.playVideo();}if(track.sourceKind==='youtube'&&/^YouTube ·/.test(track.title)){const data=e.target.getVideoData?.()||{};if(data.title){track.title=data.title;track.artist=data.author||'YouTube';await saveRemoteTrack(track);render();}}}catch(err){console.debug('youtube play',err);finish(false);}},
...
function recoveryPrefs(){return {currentId:state.currentId,queueIds:[...state.queueIds],baseQueueIds:[...state.baseQueueIds],manualQueueIds:[...state.manualQueueIds],repeatOneId:state.repeatOneId,queueIndex:state.queueIndex,volume:state.volume,activePlaylistId:state.activePlaylistId,activeSmartId:state.activeSmartId,theme:state.theme,libraryTab:state.libraryTab,librarySort:state.librarySort,firstRunComplete:state.firstRunComplete,playbackMode:state.playbackMode,shuffle:state.playbackMode===PLAY_MODES.shuffle,songCategories:[...state.songCategories],songSource:state.songSource,soundMode:state.soundMode,podcastRate:state.podcastRate};}
...
async function restoreBackupData(data){
  ...
  for(const raw of data.tracks){const incoming=normalizeTrack(raw),current=existing.get(incoming.id);let merged;if(current){const newerResume=(incoming.resumeUpdatedAt||0)>(current.resumeUpdatedAt||0)?incoming:current;merged=normalizeTrack({...incoming,...current,favorite:!!(incoming.favorite||current.favorite),validPlays:Math.max(incoming.validPlays||0,current.validPlays||0),completedPlays:Math.max(incoming.completedPlays||0,current.completedPlays||0),listenedMs:Math.max(incoming.listenedMs||0,current.listenedMs||0),skipCount:Math.max(incoming.skipCount||0,current.skipCount||0),replayCount:Math.max(incoming.replayCount||0,current.replayCount||0),resumePosition:newerResume.resumePosition||0,resumeDuration:newerResume.resumeDuration||0,resumeUpdatedAt:newerResume.resumeUpdatedAt||0});}else merged=normalizeTrack({...incoming,sourceMissing:incoming.sourceKind==='local'?true:incoming.sourceMissing});existing.set(merged.id,merged);if(state.storageReady)await db.put('tracks',merged).catch(()=>{});}
...
async function setupMediaSession(){
  ...
  safe('seekto',details=>{const pos=Math.max(0,Number(details?.seekTime)||0);if(state.currentEngine==='youtube'&&ytPlayer){try{ytPlayer.seekTo(pos,true);}catch{}}else if(state.currentEngine==='soundcloud'&&scWidget){try{scWidget.seekTo(pos*1000);}catch{}}else if(['local','direct'].includes(state.currentEngine)){const a=activeNativeAudio();a.currentTime=pos;}});
