...
const els = {
  ...
  miniFloatBtn: $('#miniFloatBtn'), floatingRealBtn: $('#floatingRealBtn'), remotePipBtn: $('#remotePipBtn'), floatMini: $('#floatMini'), fmDrag: $('#fmDrag'), fmLabel: $('#fmLabel'), fmPos: $('#fmPos'), fmExpand: $('#fmExpand'), fmClose: $('#fmClose'), fmStage: $('#fmStage'), fmTitle: $('#fmTitle'), fmArtist: $('#fmArtist'), fmProgress: $('#fmProgress'), fmTimeNow: $('#fmTimeNow'), fmTimeTotal: $('#fmTimeTotal'), fmPrev: $('#fmPrev'), fmPlay: $('#fmPlay'), fmNext: $('#fmNext'),
...
function bindLongPressReorder(row,front,track,options={}){
  if(!options.fromPlaylist||options.smart)return;let timer=0,sx=0,sy=0,active=false,targetId='',placeAfter=false,pointerId=null;
  ...
  const finish=async e=>{cancelTimer();if(!active)return;active=false;front._reorderActive=false;front._gestureConsumedUntil=performance.now()+500;row.classList.remove('reorder-source');document.body.classList.remove('playlist-reordering');clearTarget();try{if(pointerId!=null)front.releasePointerCapture(pointerId);}catch{}if(targetId&&targetId!==track.id)await reorderPlaylistTrack(state.activePlaylistId,track.id,targetId,placeAfter);targetId='';};
  front.addEventListener('pointerdown',e=>{if(e.target.closest('.row-actions'))return;sx=e.clientX;sy=e.clientY;pointerId=e.pointerId;cancelTimer();timer=setTimeout(()=>{active=true;front._reorderActive=true;row.classList.add('reorder-source');document.body.classList.add('playlist-reordering');try{front.setPointerCapture(pointerId);}catch{}navigator.vibrate?.(18);},430);},{passive:true});
  front.addEventListener('pointermove',e=>{const dx=e.clientX-sx,dy=e.clientY-sy;if(!active){if(Math.hypot(dx,dy)>9)cancelTimer();return;}e.preventDefault();const hit=document.elementFromPoint(e.clientX,e.clientY)?.closest?.('.track-row');clearTarget();if(hit&&hit!==row&&hit.closest('#playlistList')){targetId=hit.dataset.id||'';const r=hit.getBoundingClientRect();placeAfter=e.clientY>r.top+r.height/2;hit.classList.add('reorder-target');if(placeAfter)hit.classList.add('reorder-after');}if(e.clientY<90)window.scrollBy({top:-18,behavior:'auto'});else if(e.clientY>innerHeight-150)window.scrollBy({top:18,behavior:'auto'});},{passive:false});
  front.addEventListener('pointerup',finish);front.addEventListener('pointercancel',finish);
}
function makeTrackRow(track,options={fromPlaylist:false}){
  ...
  front.addEventListener('click',e=>{if(e.target.closest('.row-actions'))return;if(front._gestureConsumedUntil&&performance.now()<front._gestureConsumedUntil)return;const ctx=options.contextIds|| (options.fromPlaylist?(state.activeSmartId?getSmartTracks(state.activeSmartId).map(t=>t.id):(getActivePlaylist()?.trackIds||[])):getFilteredTracks().filter(playable).map(t=>t.id));if(playable(track))playTrack(track.id,ctx);else openTrackSheet(track,options);});
  ...
  let pt=0,lpx=0,lpy=0;
  front.addEventListener('pointerdown',e=>{if(e.target.closest('.row-actions'))return;lpx=e.clientX;lpy=e.clientY;if(options.fromPlaylist&&!options.smart)return;pt=setTimeout(()=>{if(!front._gestureActive&&!front._reorderActive)openTrackSheet(track,options);},520);},{passive:true});
  front.addEventListener('pointermove',e=>{if(Math.abs(e.clientX-lpx)>8||Math.abs(e.clientY-lpy)>8)clearTimeout(pt);},{passive:true});
  ['pointerup','pointercancel','pointerleave'].forEach(ev=>front.addEventListener(ev,()=>clearTimeout(pt),{passive:true}));return row;
}
...
function renderSearch(){
  ...
  const needle=q.toLowerCase();const tracks=state.tracks.filter(t=>[t.title,t.artist,t.album,t.genre,t.fileName,t.folder].join(' ').toLowerCase().includes(needle)).slice(0,60);const matchingTrackIds=new Set(tracks.map(t=>t.id));
  const playlistHits=state.playlists.map(pl=>{const nameMatch=(pl.name||'').toLowerCase().includes(needle),contained=(pl.trackIds||[]).filter(id=>matchingTrackIds.has(id));return{pl,nameMatch,contained,origin:pl.id===state.searchOriginPlaylistId};}).filter(x=>x.nameMatch||x.contained.length).sort((a,b)=>Number(b.origin)-Number(a.origin)||Number(b.nameMatch)-Number(a.nameMatch)||b.contained.length-a.contained.length).slice(0,18);
  const albums=getAlbumGroups().filter(g=>[g.name,g.artist].join(' ').toLowerCase().includes(needle)).slice(0,12);
  ...
  addSection('Canciones',tracks.slice(0,30).map(t=>makeTrackRow(t,{contextIds:tracks.filter(playable).map(x=>x.id)})));
  ...
  if(!tracks.length&&!playlistHits.length&&!albums.length){const empty=document.createElement('div');empty.className='search-empty';empty.innerHTML='<span>⌕</span><strong>Sin coincidencias</strong><small>Prueba otro nombre o pega un enlace.</small>';els.searchResults.appendChild(empty);}
}
function renderPlayer(){
  const t=getCurrentTrack();els.miniPlayer.classList.toggle('is-hidden',!t);if(!t)return;try{syncPipButton();}catch{}
...
function enterFloatMini(){
  ...
  toast('Mini reproductor interno · arrastra la barra para moverla · ⤢ vuelve a la app',4200);
}
...
// ==================================================================
// R10.15 · REPRODUCTOR FLOTANTE REAL (2 capas)
// CAPA A web: PiP nativo de video + snapshot de reproducción para el
// puente con la capa Android. Un solo estado de reproducción: la app
// web es SIEMPRE la fuente de verdad; Android solo refleja/controla.
// ==================================================================
function activeVideoElement(){if(state.currentEngine==='youtube'&&state.ytEngine==='native'&&state.ytNativeKind==='video'){const v=els.ytVideo;return v&&v.src?v:null;}return null;}
function syncPipButton(){if(!els.remotePipBtn)return;const v=activeVideoElement();const webOk=!!v&&!!document.pictureInPictureEnabled&&state.floatMini;const nativeOk=!window.MP_FLOATING?.isNative||window.MP_FLOATING?.capabilities?.pip===true;const show=state.currentEngine==='youtube'&&(webOk||(nativeOk&&state.ytEngine==='native'));els.remotePipBtn.classList.toggle('is-hidden',!show);}
async function requestPipWeb(){
  const native=window.MP_FLOATING;
  if(native?.isNative&&native.capabilities?.pip){try{native.send({type:'pip/request'});return;}catch{}}
  const v=activeVideoElement();
  if(!v||!document.pictureInPictureEnabled)return toast('PiP no disponible para esta fuente · usa el Mini reproductor interno',3400);
  try{if(document.pictureInPictureElement)await document.exitPictureInPicture();else await v.requestPictureInPicture();}catch(err){console.debug('pip',err);toast('El sistema no permitió PiP para este video',3200);}
}
function buildPlaybackSnapshot(){
  const t=getCurrentTrack();if(!t)return null;
  let duration=0,currentTime=0;
  if(state.currentEngine==='youtube'&&state.ytEngine==='native'){const el=ytNativeElement();duration=el?.duration||0;currentTime=el?.currentTime||0;}
  else if(state.currentEngine==='youtube'&&ytPlayer){try{duration=ytPlayer.getDuration()||0;currentTime=ytPlayer.getCurrentTime()||0;}catch{}}
  else if(state.currentEngine==='soundcloud'&&getCurrentTrack()){duration=getCurrentTrack().duration||0;}
  else{const a=activeNativeAudio();duration=a?.duration||0;currentTime=a?.currentTime||0;}
  return {id:t.id,title:t.title,artist:t.artist||sourceLabel(t),album:t.album||'',source:sourceLabel(t),sourceKind:t.sourceKind||'local',duration,currentTime,isPlaying:!!state.playing,volume:state.volume,canPip:!!activeVideoElement(),queueIndex:Math.max(0,state.queueIndex),queueLength:state.queueIds.length,podcast:!!isPodcastTrack(t)};
}
...
function stopAllEngines(){try{els.audio.pause();}catch{}try{els.directAudio?.pause();}catch{}if(ytPlayer){try{ytPlayer.stopVideo();}catch{}}if(scWidget){try{scWidget.pause();}catch{}}try{els.ytAudio?.pause();}catch{}try{els.ytVideo?.pause();}catch{}state.playing=false;syncMediaPlaybackState();}
...
function hasRecoveryData(){return !!(state.tracks.length||state.playlists.length||state.history.length||state.currentId||state.queueIds.length||state.baseQueueIds.length||state.manualQueueIds.length||state.repeatOneId);}
...
function maybeShowInstallCoach(){
  syncInstallUI();if(isInstalledDisplay()||state.installCoachShown)return;try{if(sessionStorage.getItem('mpf-install-coach-r10.1'))return;sessionStorage.setItem('mpf-install-coach-r10.1','1');}catch{}state.installCoachShown=true;setTimeout(()=>{if(!els.sheetDialog.open)openInstallSheet({automatic:true});},650);
}
...
function bindEvents(){
  ...
  els.navHome.onclick=()=>showView('home');els.navSearch.onclick=()=>showView('search');els.navLibrary.onclick=()=>{if(state.activeView==='library'&&state.libraryTab==='playlists'&&state.playlistDetailOpen){closePlaylistDetail();return;}showView('library');};
  ...
  [els.libraryTabPlaylists,els.libraryTabSongs,els.libraryTabAlbums].forEach(b=>b.onclick=()=>{if(b.dataset.tab==='playlists'&&state.libraryTab==='playlists'&&state.playlistDetailOpen)return closePlaylistDetail();setLibraryTab(b.dataset.tab);});
...
async function init(){
  ...
  try{window.dispatchEvent(new CustomEvent('mp:app-ready'));}catch{} // R10.15: arranque del puente flotante
}
// =============================================================
...
window.MP = Object.freeze({
  ...
  // R10.15: superficie para el puente del reproductor flotante real (capa Android)
  togglePlay, nextTrack, prevTrack, seekBy, getCurrentTrack, sourceLabel,
  buildPlaybackSnapshot, requestPipWeb, artworkUrlFor, isPodcastTrack
});
