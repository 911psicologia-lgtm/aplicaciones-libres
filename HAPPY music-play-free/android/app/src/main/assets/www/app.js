...
const BUILD = '2026.09.15-r10.16-live-radio';
const APP_VERSION = 'R10.16';
...
const els = {
  ...
  homeBtn: $('#homeBtn'), installBtn: $('#installBtn'), updateBtn: $('#updateBtn'), moreBtn: $('#moreBtn'), floatMiniBtn: $('#floatMiniBtn'),
...
function toggleFloatMini(){
  ...
  // R10.15 · APK: el botón «◱ Pantalla reducida flotante» abre la VENTANA FLOTANTE REAL nativa (▣)
  if(IS_NATIVE_APK&&window.HappyNative?.openFloat){try{const t=getCurrentTrack(),s=happyEngineSnapshot();window.HappyNative.openFloat(JSON.stringify({trackId:t?.id||'',posMs:Math.round((s.pos||0)*1000),playing:!!s.playing}));}catch{window.HappyNative.openFloat('');}return;}
