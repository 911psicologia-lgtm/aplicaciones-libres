...
public class FloatingPlayerService extends Service {
    ...
    /** R10.18 · HAPPY BOOST nativo — reenvía al NativeRadioEngine. Si el servicio
     *  no está activo, guarda el estado para aplicarlo cuando se cree. */
    private static boolean pendingBoostEnabled = false;
    public static void handleBoost(boolean enabled) {
        pendingBoostEnabled = enabled;
        FloatingPlayerService svc = instance;
        if (svc != null && svc.nativeEngine != null) {
            svc.nativeEngine.setBoostEnabled(enabled);
        }
    }

    public static boolean isBoostEnabled() {
        FloatingPlayerService svc = instance;
        if (svc != null && svc.nativeEngine != null) return svc.nativeEngine.isBoostEnabled();
        return pendingBoostEnabled;
    }
    ...
    public void onCreate() {
        ...
        // R10.18 · aplicar estado BOOST pendiente al servicio recién creado
        nativeEngine.setBoostEnabled(pendingBoostEnabled);
        setupMediaSession();
