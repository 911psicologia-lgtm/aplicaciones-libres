...
const st=windowObj.__SWARM_V2152_STATUS?.();
...
const checks={
 version:st.version==='2.15.2',worlds:st.worlds===20,bosses:st.bosses===20,
 familyCombat:st.familyCombat?.enabled===true&&st.familyCombat.fromWorld===11&&st.familyCombat.grammars===10&&st.familyCombat.minorEscort===true&&st.familyCombat.mediumCorridors===true&&st.familyCombat.majorCalls===true,
 micro:st.microSwarm?.enabled===true&&st.microSwarm.fromWorld===1&&st.microSwarm.countsForWave===false,
 rift:st.riftAlly?.duration===10,lastChance:st.lastChance?.manual===true&&st.lastChance?.seconds===5&&st.lastChance?.autoPurchase===false,
 bossAnim:boss&&Object.keys(boss.animatedBosses||{}).length===20,chase:audit?.chaseBonus?.enabled===true,pwa:st.pwa===true
};
const ok=Object.values(checks).every(Boolean);
console.log(JSON.stringify({ok,checks,status:st,bossAnimated:Object.keys(boss?.animatedBosses||{}).length},null,2));
process.exit(ok?0:4);
