...
const st=windowObj.__SWARM_V2151_STATUS?.();
...
const checks={
 version:st.version==='2.15.1',worlds:st.worlds===20,bosses:st.bosses===20,chapterII:st.chapterII?.active===true&&st.chapterII.assets===10,
 micro:st.microSwarm?.enabled===true&&st.microSwarm.fromWorld===1&&st.microSwarm.count[0]===4&&st.microSwarm.count[1]===10&&st.microSwarm.countsForWave===false&&st.microSwarm.formations===8&&st.microSwarm.forcedContinuity===true,
 rift:st.riftAlly?.duration===10&&st.riftAlly?.ch2Signatures===10,lastChance:st.lastChance?.manualLifeRescue===true&&st.lastChance?.autoPurchase===false&&st.lastChance?.allyContinues===true,
