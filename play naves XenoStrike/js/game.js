...
const KEY_META='swarm_rift_meta_v2144';
const KEY_RUN='swarm_rift_run_v2144';
const TAU=Math.PI*2;
...
for(const [key,file] of Object.entries({phase:'phase_lance.png',nanites:'nanite_swarm.png',riftmine:'rift_mine.png',echo:'temporal_echo.png',nova:'entomic_nova.png',ionweb:'ion_web.png',orbitblade:'orbit_blades.png'})){const im=new Image();im.src=`assets/powers_hangar/${file}?av=2144`;IMG.generatedPowers[key]=im;}
...
// v2.14.4 · DOCTRINAS, FUSIÓN ESCALONADA, CONTRAMEDIDAS Y SOBREFLUJO
...
function combatDamageMult(){return synergyDamageMult()*doctrineDamageMult()*fusionSurgeDamageMult();}
...
function bossCounterSpec(sector=G?.sector||storeContextSector?.()||1){return BOSS_COUNTERS[clamp(sector,1,10)]||BOSS_COUNTERS[1];}
...
function newRun(startSector=1,mode='campaign',runDifficultyChoice=null){
  ...
  G={screen:'GAME',mode,runDifficulty:runDiff,sector:clamp(startSector,1,Math.min(META.unlocked,activeSectorCount())),wave:1,score:0,hiScore:META.hiScore||0,credits:META.credits||0,
    ...
    heritageNext:null,activeCombos:{},comboSeenThisRun:{},powerDropHistory:[],fusionSurgeT:0,fusionSurgeCd:0,fusionSurgeTier:0,overflowPowerKey:null,overflowPowerMax:0,bossHitT:0,heartHitT:0,critWarned:false,lastBossDrop:null,frenzyT:0,frenzyWave:0,frenzyMult:1,frenzyKills:0,frenzyTarget:0,frenzyDone:false,frenzySupportUsed:false,bossWarningT:0,bossWarningText:'',
...
function enterSector(n,keepPlayer=false){
  ...
  G.enemies.length=0;G.bullets.length=0;G.eBullets.length=0;G.pickups.length=0;G.obstacles.length=0;G.frontThreats.length=0;G.particles.length=0;G.activeCombos={};G.fusionSurgeT=0;G.fusionSurgeCd=0;G.powerDropHistory=[];
...
function recycleOverflowPower(key,rank=1){
  ...
  grantCredits(credits,'power_overflow',true);grantRewardXp(xp,'power_overflow');
...
function updatePlayer(dt){
  ...
  const rate=Math.max(.052,.19*(1-up('rate')*.055)*(powerOn('overdrive')?(0.82-powerRank('overdrive')*.05):1)*(powerOn('burst')?(0.86-powerRank('burst')*.05):1)*(1-Math.min(.16,(lv-1)*.008))*(G.fusionSurgeT>0?.90:1));p.fire-=dt;if(p.fire<=0){p.fire=rate;firePlayer();}
...
function shopCatalog(tab=storeTab){
  ...
  const doctrine=hangarDoctrine(),policy=reservePolicy(),fusionSettings=[{kind:'doctrine',key:'hangar',name:`DOCTRINA · ${doctrine.name}`,icon:doctrine.icon,color:doctrine.color,desc:`${doctrine.desc} · tocar para cambiar`,cost:0,stock:doctrine.name,maxed:false},{kind:'policy',key:'reserve',name:`RESERVAS · ${policy.name}`,icon:policy.icon,color:policy.color,desc:`${policy.desc} · tocar para cambiar`,cost:0,stock:policy.name,maxed:false}];
  const mastery=Object.keys(COMBOS).map(key=>{const c=COMBOS[key],lv=comboMasteryLevel(key),n=comboMasteryCount(key),next=COMBO_MASTERY_THRESHOLDS[Math.min(4,lv+1)]||COMBO_MASTERY_THRESHOLDS[4];return {kind:'mastery',key,name:c.name,icon:'Σ',color:c.color,desc:`activaciones ${n} · ${lv>=4?'maestría máxima':`siguiente nivel en ${Math.max(0,next-n)}`}`,cost:0,stock:lv?`M${comboMasteryLabel(key)}`:'M—',level:lv,maxed:lv>=4};}).sort((a,b)=>(b.level||0)-(a.level||0)||comboMasteryCount(b.key)-comboMasteryCount(a.key));
...
function shopCanAfford(item){if(item.kind==='briefing'||item.kind==='doctrine'||item.kind==='policy')return true;if(item.kind==='mastery'||item.maxed)return false;if(item.kind==='power'&&storeInRun())return (G.xp||0)>=item.xp||(G.credits||0)>=item.cost;return (G.credits||0)>=item.cost;}
...
function drawGuide(){
  ...
  const pages=[
    ...
    {title:'COMBATE',cards:[['PODERES Y COMBOS','Cuatro poderes activos de base; Bahía XL permite cinco, el Bastidor hasta siete en reserva y Sobreflujo habilita un poder extra temporal (quinto o sexto según la Bahía XL). Ramas A/B y evoluciones VI/VII cambian función y VFX.','powers'],['AMENAZAS FRONTALES','Meteoros, arietes, cápsulas y restos llegan hacia cámara. Interceptarlos pronto paga mejor.','front'],['JEFES Y FASES',`Los ${activeSectorCount()} jefes activos mutan en tres fases y abren brevemente el núcleo tras ataques importantes.`,'boss']]},
    {title:'PROGRESIÓN',cards:[['REPETIR Y CHECKPOINTS','Entrena, repite sectores y salta a la arena del jefe con 50% de vida.','replay'],['HANGAR PERMANENTE','Casco, escudo, daño, motor, drones y otras mejoras persisten entre partidas.','hangar'],['XP, RANGOS Y HERENCIAS','Los poderes evolucionan hasta VII con Matriz; rangos VI/VII se ven distintos y los jefes transmiten herencias al siguiente sector.','progress']]}
