...
const KEY_META='swarm_rift_meta_v21525';
const KEY_RUN='swarm_rift_run_v21525';
const KEY_HIVE_TELEMETRY='swarm_rift_hive_telemetry_v21525';
const KEY_HIVE_TELEMETRY_PREV='swarm_rift_hive_telemetry_v21524';
const TAU=Math.PI*2;
...
function hiveArenaPrepare(b,p,sec){const st=hiveArenaPilotState(),plan=hiveArenaLanePlan(b,p);st.plan=plan;st.telegraphs++;st.lastReason='prepared';notify(`ARENA VIVA · ${plan.label} · CORREDOR MARCADO`,sec.accent,1.0);return plan;}
...
function hiveArenaMorphologyUpdate(b,p,sec,dt,pressure){const st=hiveArenaPilotState();if(!st)return false;if(st.plan){st.plan.telegraph=Math.max(0,st.plan.telegraph-dt);hiveArenaTelegraph(st.plan,sec,dt);if(st.plan.telegraph<=0){const plan=st.plan;st.plan=null;hiveArenaExecute(b,p,sec,plan);b.arenaCd=Math.max(HIVE_DIRECTOR_CONFIG.arenaCooldownFloor,(6.2-b.phase*.72-G.sector*.12)*pressure.cool+.35);}return true;}if(b.arenaCd>0)return false;const el=hiveArenaEligible(G.hiveDirector,b);if(!el.ok){st.lastReason=el.reason;st.deferred++;return false;}hiveArenaPrepare(b,p,sec);return true;}
// v2.15.25 · TACTICAL MEMORY + COUNTER-EVOLUTION + MICROSEQUENCES + FEINT BRANCHING
...
// v2.15.25 · COEVOLUTION MICROSEQUENCES + LEGIBLE FEINT BRANCHES
...
// v2.15.25: durante la apertura, una secuencia aprendida puede revelar DOS continuaciones
...
function hiveUpdate(dt){const h=G?.hiveDirector;if(!h||h.finished)return;hiveSpatialUpdate(dt);hiveTacticalPilotUpdate(dt);hiveBranchFeintUpdate(dt);h.sampleT=(h.sampleT||0)+Math.max(0,dt||0);if(h.sampleT<HIVE_DIRECTOR_CONFIG.sampleInterval)return;h.sampleT%=HIVE_DIRECTOR_CONFIG.sampleInterval;hiveSampleDirector();}
function hiveLoadTelemetry(){if(HIVE_TELEMETRY_CACHE)return HIVE_TELEMETRY_CACHE;let data=null,migrated=false;try{data=JSON.parse(localStorage.getItem(KEY_HIVE_TELEMETRY)||'null');if(!data){data=JSON.parse(localStorage.getItem(KEY_HIVE_TELEMETRY_PREV)||'null');migrated=!!data;}}catch(_){}if(!data||typeof data!=='object')data={schema:7,version:VERSION,fights:[]};data.schema=7;data.version=VERSION;data.migratedFrom=migrated?'2.15.24':(data.migratedFrom||null);data.fights=Array.isArray(data.fights)?data.fights.slice(-HIVE_DIRECTOR_CONFIG.maxFights):[];HIVE_TELEMETRY_CACHE=data;return data;}
...
function hiveTelemetrySummary(){const data=hiveLoadTelemetry(),f=data.fights,states={A0:0,A1:0,A2:0,A3:0};let sum=0,n=0,pilots=0,tactical=0,arena=0,counterTrials=0,counterLearned=0,counterRetired=0,sequences=0,mutated=0,abandoned=0,branches=0,branchCommits=0;for(const r of f){states[r.maxState]=(states[r.maxState]||0)+1;if(Number.isFinite(r.ttkReal)){sum+=r.ttkReal;n++;}pilots+=Number(r.signaturePilotEvents?.length||0);tactical+=Number(r.tacticalPilotEvents?.length||0);arena+=Number(r.arenaPilotEvents?.length||0);counterTrials+=Number(r.counterEvolution?.trials||0);counterLearned+=Number(r.counterEvolution?.successes||0);counterRetired+=Number(r.counterEvolution?.failures||0);sequences+=Number(r.counterEvolution?.sequences?.started||0);mutated+=Number(r.counterEvolution?.sequences?.mutated||0);abandoned+=Number(r.counterEvolution?.sequences?.abandoned||0);branches+=Number(r.counterEvolution?.sequences?.branches?.started||0);branchCommits+=Number(r.counterEvolution?.sequences?.branches?.committed||0);}return {schema:data.schema,stored:f.length,limit:HIVE_DIRECTOR_CONFIG.maxFights,migratedFrom:data.migratedFrom||null,states,averageTtk:n?+(sum/n).toFixed(2):null,signaturePilotEvents:pilots,tacticalPilotEvents:tactical,arenaPilotEvents:arena,counterEvolution:{trials:counterTrials,learned:counterLearned,retired:counterRetired,sequences:{started:sequences,mutated,abandoned,branches:{started:branches,committed:branchCommits}}},last:f.length?{world:f[f.length-1].world,boss:f[f.length-1].boss,ttk:f[f.length-1].ttkReal,maxState:f[f.length-1].maxState,result:f[f.length-1].result,fingerprint:f[f.length-1].dpsProfileFinal?.fingerprint||null}:null};}
