...
const KEY_META='swarm_rift_meta_v2136';
const KEY_RUN='swarm_rift_run_v2136';
const TAU=Math.PI*2;
...
for(const [bossKey] of Object.entries(IMG.bossAnim))for(const st of ['idle','attack','hurt','phase','death'])IMG.bossAnim[bossKey][st].src=`assets/boss_anim/${bossKey}/${st}.png?av=2136`;
...
const WORLD_ENEMY_FILES={
  ...
  LANGOSTAS:['assets/enemies_semantic/locust_minor.png','assets/enemies_semantic/locust_medium.png','assets/enemies_semantic/locust_major.png','assets/boss_reformulated/boss_colossus_reformulated.png'],
  MOSQUITOS:['assets/worlds7_10/enemies/mosquito_needler.png','assets/worlds7_10/enemies/mosquito_hemodrone.png','assets/worlds7_10/enemies/mosquito_bloodreaper.png','assets/boss_reformulated/boss_sanguina_reformulated.png'],
  TERMITAS:['assets/worlds7_10/enemies/termite_worker.png','assets/worlds7_10/enemies/termite_mandible_guard.png','assets/worlds7_10/enemies/termite_siegebuilder.png','assets/boss_reformulated/boss_architect_reformulated.png'],
  'LIBÉLULAS':['assets/worlds7_10/enemies/dragonfly_flashwing.png','assets/worlds7_10/enemies/dragonfly_prism_hunter.png','assets/worlds7_10/enemies/dragonfly_lance_predator.png','assets/boss_reformulated/boss_auralis_reformulated.png'],
  CIGARRAS:['assets/worlds7_10/enemies/cicada_nymph_echo.png','assets/worlds7_10/enemies/cicada_sonic_cantor.png','assets/worlds7_10/enemies/cicada_resonance_breaker.png','assets/boss_reformulated/boss_resonator_reformulated.png']
};
...
// v2.13.6 · PLAYLIST AMBIENTAL POR MUNDO
...
// v2.13.6 · PLAYLIST ADAPTATIVA DE BOSSES
...
function drawBossSignatureAttack(b){
  ...
  else if(b.pattern==='leap'){cx.strokeStyle=hexA(cfg.color,wind?.72:.88);cx.lineWidth=wind?3:9+b.phase*2;const r=wind?34+pulse*12:Math.max(18,clamp((prog-.18)/.82,0,1)*Math.max(W,H)*.78);cx.shadowColor=cfg.color;cx.shadowBlur=wind?10:24;cx.beginPath();cx.arc(s.targetX,s.targetY,r,0,TAU);cx.stroke();if(!wind){for(let i=0;i<12;i++){const a=i*TAU/12+G.elapsed*.35,ri=r*(.82+.13*Math.sin(G.elapsed*9+i));cx.strokeStyle=i%2?'#fff09a':'#b7ff58';cx.lineWidth=2+((i+b.phase)%3);cx.beginPath();cx.moveTo(s.targetX+Math.cos(a)*ri*.72,s.targetY+Math.sin(a)*ri*.72);cx.lineTo(s.targetX+Math.cos(a)*ri,s.targetY+Math.sin(a)*ri);cx.stroke();}}if(wind){cx.beginPath();cx.moveTo(s.targetX-22,s.targetY);cx.lineTo(s.targetX+22,s.targetY);cx.moveTo(s.targetX,s.targetY-22);cx.lineTo(s.targetX,s.targetY+22);cx.stroke();}cx.shadowBlur=0;}
  else if(b.pattern==='blood'){const ox=b.x-b.r*.42,oy=b.y-b.r*.03,len=Math.max(W,H)*1.2;beam(ox,oy,s.aim,len,10+b.phase*3,'#ff425f');if(!wind){cx.fillStyle=hexA('#ff425f',.22+pulse*.16);cx.beginPath();cx.arc(b.x,b.y,b.r*(.52+.13*pulse),0,TAU);cx.fill();for(let i=1;i<=7;i++){const u=((G.elapsed*.62+i/7)%1),px=ox+Math.cos(s.aim)*len*u,py=oy+Math.sin(s.aim)*len*u;cx.fillStyle=i%2?'#ffd5dc':'#ff3658';cx.shadowColor='#ff3658';cx.shadowBlur=12;cx.beginPath();cx.arc(px,py,2.2+(i%3),0,TAU);cx.fill();}cx.shadowBlur=0;}}
  else if(b.pattern==='architect'){const width=13+b.phase*3;for(const x of s.columns||[]){cx.shadowColor='#ffbd45';cx.shadowBlur=wind?6:18;cx.fillStyle=wind?hexA(cfg.color,.18):hexA(cfg.color,.50);cx.strokeStyle=hexA('#fff09a',wind?.32:.72);cx.lineWidth=2.5;cx.fillRect(x-width,0,width*2,H);cx.strokeRect(x-width,0,width*2,H);if(!wind){for(let j=0;j<5;j++){const yy=((G.elapsed*95+j*137)%H);cx.fillStyle=j%2?'#fff09a':'#ff9f2f';cx.fillRect(x-2,yy,4,10+j%3*4);}}cx.shadowBlur=0;}cx.fillStyle=hexA('#a6ff5f',.10);cx.fillRect(s.safeX-(36+b.phase*4),0,72+b.phase*8,H);}
  else if(b.pattern==='odonata'){const len=Math.max(W,H)*1.28,n=b.phase===1?2:b.phase===2?3:4;for(let i=0;i<n;i++){const dir=i%2?1:-1,a=s.base+(i-(n-1)/2)*.20+(wind?0:dir*prog*(.72+b.phase*.16));beam(b.x,b.y,a,len,7+b.phase*2,'#78ecff');if(!wind){for(let k=1;k<=5;k++){const u=((G.elapsed*.75+k/5+i*.13)%1),px=b.x+Math.cos(a)*len*u,py=b.y+Math.sin(a)*len*u;cx.fillStyle=k%2?'#ffffff':'#78ecff';cx.shadowColor='#78ecff';cx.shadowBlur=14;cx.beginPath();cx.arc(px,py,2.2+(k%2)*1.4,0,TAU);cx.fill();}}}if(!wind){cx.strokeStyle=hexA('#b8ffff',.65);cx.lineWidth=2;cx.beginPath();cx.arc(b.x,b.y,b.r*(.72+.10*Math.sin(G.elapsed*12)),0,TAU);cx.stroke();cx.shadowBlur=0;}}
  else if(b.pattern==='resonance'){const maxR=Math.max(W,H)*1.06,n=1+b.phase;for(let i=0;i<n;i++){const lp=wind?(.12+i*.065):clamp((prog-i*.16)/Math.max(.1,1-i*.16),0,1),r=Math.max(16,lp*maxR);cx.strokeStyle=hexA(i%2?'#f4d37a':'#d9a7ff',wind?.40:.78);cx.lineWidth=wind?2:8+b.phase*2;cx.shadowColor=i%2?'#f4d37a':'#d9a7ff';cx.shadowBlur=wind?0:20;cx.beginPath();cx.arc(b.x,b.y,r,0,TAU);cx.stroke();if(!wind){for(let k=0;k<8;k++){const a=k*TAU/8+G.elapsed*(i%2?.55:-.42);cx.fillStyle=k%2?'#fff09a':'#d9a7ff';cx.beginPath();cx.arc(b.x+Math.cos(a)*r,b.y+Math.sin(a)*r,2.4+(k%3),0,TAU);cx.fill();}}}if(!wind){cx.fillStyle=hexA('#fff09a',.16+.10*Math.sin(G.elapsed*10));cx.shadowColor='#d9a7ff';cx.shadowBlur=28;cx.beginPath();cx.arc(b.x,b.y,b.r*(.48+.08*Math.sin(G.elapsed*8)),0,TAU);cx.fill();}cx.shadowBlur=0;}
  cx.restore();
}
...
window.__SWARM_V2136_STATUS=()=>({version:VERSION,chase:{enabled:true,duration:G?.chase?.duration||68,freeMovement:'X+Y',shipScale:'larger',shieldStart:G?.chase?.shield??45,powerDrops:G?.chase?.powerDrops?.length||0,streak:G?.chase?.streak||0,bestStreak:G?.chase?.bestStreak||0},bossIdentity:{auralis:BOSS_ANIMATIONS['LIBÉLULAS'].motion,resonator:BOSS_ANIMATIONS.CIGARRAS.motion},music:{worldPool:(WORLD_PLAYLISTS[G?.sector||1]||[]).map(t=>t.label),bossPool:MusicX.bossPlaylist.map(t=>t.title),bossCurrent:MusicX.currentTrack?.title||'',worldCurrent:PreBossX.track?.label||''}});
})();
