...
const KEY_META='swarm_rift_meta_v21522';
const KEY_RUN='swarm_rift_run_v21522';
const KEY_HIVE_TELEMETRY='swarm_rift_hive_telemetry_v21522';
...
const HIVE_DIRECTOR_CONFIG={mode:'ARENA_MORPHOLOGY_PILOT',schema:4,sampleInterval:1.0,damageWindows:{burst:3,tactical:8,sustained:20},hysteresisSamples:3,minStateTime:4.0,stateCooldown:3.0,maxFights:40,historySamples:6,historyMin:3,historyBlend:.20,historyClamp:.12,signaturePilot:true,signaturePilotWorld:11,signaturePilotConfidence:.78,signaturePilotCooldownTarget:1.05,tacticalPilot:true,tacticalPilotWorld:11,tacticalPilotPhase:2,tacticalPilotConfidence:.68,tacticalSegmentMin:4.5,tacticalSegmentMax:7.0,tacticalNeutralMin:3.0,tacticalNeutralMax:4.8,tacticalActiveBeforeNeutral:2,tacticalAimMax:.115,tacticalYOffsetMax:.075,tacticalXOffsetMax:.035,arenaPilot:true,arenaPilotWorld:11,arenaPilotPhase:2,arenaPilotConfidence:.72,arenaTelegraph:.88,arenaSafeBand:.13,arenaCooldownFloor:3.1,arenaMaxEvents:12};
...
function hiveTacticalVector(b,p,identity=bossIdentity()){const tp=hiveTacticalPilotState(),sp=hiveSpatialState(),spec=hiveTacticalIdentitySpec(identity);if(!tp||!sp||!spec||!tp.active||tp.intensity<.03)return {active:false,variant:'NEUTRAL',intensity:0,xOffset:0,yOffset:0,aimAngle:Math.atan2(p.y-b.y,p.x-b.x),spreadScale:1,ringPhase:0,predictedY:p.y};const i=clamp(tp.intensity,0,1),pred=clamp(sp.predictedY,70,H-70),dy=clamp((pred-H*.5)/Math.max(1,H),-HIVE_DIRECTOR_CONFIG.tacticalYOffsetMax,HIVE_DIRECTOR_CONFIG.tacticalYOffsetMax),vySign=Math.sign(sp.vy||0),read=sp.read;let yMul=spec.y,xMul=spec.x,aimMul=spec.aim;if(read==='EDGE_ANCHOR')yMul*=.72;else if(read==='VERTICAL_WEAVE')aimMul*=1.10;else if(read==='EVASIVE'){aimMul*=.68;yMul*=.65;}const yOffset=H*dy*yMul*i,xDir=(identity.key==='VESPERA'||identity.key==='MIRAGE'||identity.key==='AURICULUS')?-1:1,xOffset=W*HIVE_DIRECTOR_CONFIG.tacticalXOffsetMax*xMul*i*xDir*(sp.dominant==='center'?.55:1),leadY=lerp(p.y,pred,clamp(aimMul*i,0,.86)),base=Math.atan2(p.y-b.y,p.x-b.x),lead=Math.atan2(leadY-b.y,p.x-b.x),delta=Math.atan2(Math.sin(lead-base),Math.cos(lead-base)),aimAngle=base+clamp(delta,-HIVE_DIRECTOR_CONFIG.tacticalAimMax,HIVE_DIRECTOR_CONFIG.tacticalAimMax);const spreadScale=clamp(lerp(1,spec.spread,i*(read==='VERTICAL_WEAVE'?.72:.48)),.82,1.22),ringPhase=(spec.ring||0)*i*(vySign||1);return {active:true,variant:tp.variant,intensity:+i.toFixed(3),xOffset,yOffset,aimAngle,spreadScale,ringPhase,predictedY:pred,spatialRead:read};}
...
function hiveArenaLanePlan(b,p){const id=bossIdentity(),spec=id?HIVE_ARENA_IDENTITY[id.key]:null,sp=hiveSpatialState(),lanes=[H*.20,H*.40,H*.60,H*.80],target=clamp(sp?.predictedY??p.y,70,H-70);let nearest=0,best=Infinity;for(let i=0;i<lanes.length;i++){const d=Math.abs(lanes[i]-target);if(d<best){best=d;nearest=i;}}let safe=nearest;if(spec&&sp?.confidence>.72&&sp.read!=='EVASIVE'&&sp.read!=='VERTICAL_WEAVE'){const candidate=clamp(nearest+(spec.bias||0),0,lanes.length-1);if(Math.abs(candidate-nearest)<=1)safe=candidate;}return {identity:id?.key||null,motif:spec?.motif||'READABLE_CORRIDOR',label:spec?.label||'CORREDOR ADAPTATIVO',lanes,safeIndex:safe,safeY:lanes[safe],spatial:sp?.read||'READING',spatialConfidence:+(sp?.confidence||0).toFixed(3),directorState:G?.hiveDirector?.state||'A1',directorConfidence:+(G?.hiveDirector?.confidence||0).toFixed(3),telegraph:HIVE_DIRECTOR_CONFIG.arenaTelegraph,pulse:0};}
...
function hiveSampleDirector(){
  ...
  const proposal=hiveProposedResponse(h.state,{ttk,dpsRatio:ratio,confidence,profile,experience,target,age,hold});h.proposedResponse=proposal;hiveMaybeSignaturePilot(h,b,age,profile,experience,hold);
...
function hiveUpdate(dt){const h=G?.hiveDirector;if(!h||h.finished)return;hiveSpatialUpdate(dt);hiveTacticalPilotUpdate(dt);h.sampleT=(h.sampleT||0)+Math.max(0,dt||0);if(h.sampleT<HIVE_DIRECTOR_CONFIG.sampleInterval)return;h.sampleT%=HIVE_DIRECTOR_CONFIG.sampleInterval;hiveSampleDirector();}
function hiveLoadTelemetry(){if(HIVE_TELEMETRY_CACHE)return HIVE_TELEMETRY_CACHE;let data=null,migrated=false;try{data=JSON.parse(localStorage.getItem(KEY_HIVE_TELEMETRY)||'null');if(!data){data=JSON.parse(localStorage.getItem(KEY_HIVE_TELEMETRY_PREV)||'null');migrated=!!data;}}catch(_){}if(!data||typeof data!=='object')data={schema:4,version:VERSION,fights:[]};data.schema=4;data.version=VERSION;data.migratedFrom=migrated?'2.15.21':(data.migratedFrom||null);data.fights=Array.isArray(data.fights)?data.fights.slice(-HIVE_DIRECTOR_CONFIG.maxFights):[];HIVE_TELEMETRY_CACHE=data;return data;}
...
function hiveTelemetrySummary(){const data=hiveLoadTelemetry(),f=data.fights,states={A0:0,A1:0,A2:0,A3:0};let sum=0,n=0,pilots=0,tactical=0,arena=0;for(const r of f){states[r.maxState]=(states[r.maxState]||0)+1;if(Number.isFinite(r.ttkReal)){sum+=r.ttkReal;n++;}pilots+=Number(r.signaturePilotEvents?.length||0);tactical+=Number(r.tacticalPilotEvents?.length||0);arena+=Number(r.arenaPilotEvents?.length||0);}return {schema:data.schema,stored:f.length,limit:HIVE_DIRECTOR_CONFIG.maxFights,migratedFrom:data.migratedFrom||null,states,averageTtk:n?+(sum/n).toFixed(2):null,signaturePilotEvents:pilots,tacticalPilotEvents:tactical,arenaPilotEvents:arena,last:f.length?{world:f[f.length-1].world,boss:f[f.length-1].boss,ttk:f[f.length-1].ttkReal,maxState:f[f.length-1].maxState,result:f[f.length-1].result,fingerprint:f[f.length-1].dpsProfileFinal?.fingerprint||null}:null};}
