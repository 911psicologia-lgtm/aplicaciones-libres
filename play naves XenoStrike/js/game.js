...
const KEY_META='swarm_rift_meta_v21530';
const KEY_RUN='swarm_rift_run_v21530';
const KEY_HIVE_TELEMETRY='swarm_rift_hive_telemetry_v21530';
const KEY_HIVE_TELEMETRY_PREV='swarm_rift_hive_telemetry_v21529';
const TAU=Math.PI*2;
...
// v2.15.30 · ADAPTIVE HIVE BOSS DIRECTOR — PACING ECOLOGY
// Continúa Stability Coordination sin añadir otra capa ofensiva. El Director ahora estima el
// momento rítmico de cada fase, evita iniciar intervenciones largas cuando una transición es
// inminente, suaviza la utilidad contextual y conserva fatiga por familias de intervención.
// Las reservas iguales se coalescen para reducir trabajo/ruido de coordinación.
// No aumenta HP, daño, cadencia, densidad ni muta la build. Tactical Identity sigue siendo sutil.
const HIVE_DIRECTOR_CONFIG={mode:'PACING_ECOLOGY',schema:12,sampleInterval:1.0,adaptiveSampling:true,sampleBalanced:1.18,sampleRecovery:1.45,damageWindows:{burst:3,tactical:8,sustained:20},hysteresisSamples:3,minStateTime:4.0,stateCooldown:3.0,maxFights:40,historySamples:6,historyMin:3,historyBlend:.20,historyClamp:.12,orchestrator:true,orchestratorContextual:true,orchestratorMinUtility:.50,orchestratorMajorUtility:.58,orchestratorBudgetMax:100,orchestratorBudgetStart:82,orchestratorRecoveryPerSec:5.5,orchestratorA1RecoveryBoost:1.25,orchestratorQuietMinor:.8,orchestratorQuietMajor:2.35,orchestratorRecentSec:14,orchestratorRepeatLimit:2,orchestratorPhaseTotalCap:5,orchestratorPhaseMajorCap:2,orchestratorPhaseEntryQuiet:1.15,orchestratorPhasePolicies:{1:{total:2,major:0,salience:28},2:{total:4,major:1,salience:62},3:{total:5,major:2,salience:82}},orchestratorPressureDecayPerSec:.13,orchestratorPressureA1DecayBoost:1.35,orchestratorPressureMajorGate:.72,orchestratorPressureMinorGate:.92,orchestratorPressureQuietMajor:1.25,orchestratorPressureQuietMinor:.45,orchestratorReservationDurations:{SIGNATURE_PRIORITY:2.2,ARENA_MORPHOLOGY:3.0,COUNTER_TRIAL:5.75,LEARNED_REUSE:4.65,MICRO_SEQUENCE:8.0,FEINT_BRANCH:1.55},orchestratorConflictMatrix:{SIGNATURE_PRIORITY:['ARENA_MORPHOLOGY','COUNTER_TRIAL','LEARNED_REUSE','MICRO_SEQUENCE','FEINT_BRANCH'],ARENA_MORPHOLOGY:['SIGNATURE_PRIORITY','COUNTER_TRIAL','LEARNED_REUSE','MICRO_SEQUENCE','FEINT_BRANCH'],COUNTER_TRIAL:['SIGNATURE_PRIORITY','ARENA_MORPHOLOGY','LEARNED_REUSE','MICRO_SEQUENCE','FEINT_BRANCH'],LEARNED_REUSE:['SIGNATURE_PRIORITY','ARENA_MORPHOLOGY','COUNTER_TRIAL','MICRO_SEQUENCE','FEINT_BRANCH'],MICRO_SEQUENCE:['SIGNATURE_PRIORITY','ARENA_MORPHOLOGY','COUNTER_TRIAL','LEARNED_REUSE'],FEINT_BRANCH:['SIGNATURE_PRIORITY','ARENA_MORPHOLOGY','COUNTER_TRIAL','LEARNED_REUSE']},orchestratorCosts:{SIGNATURE_PRIORITY:22,ARENA_MORPHOLOGY:24,COUNTER_TRIAL:16,LEARNED_REUSE:13,MICRO_SEQUENCE:30,FEINT_BRANCH:10},orchestratorTypeCooldown:{SIGNATURE_PRIORITY:5.5,ARENA_MORPHOLOGY:7.5,COUNTER_TRIAL:5.0,LEARNED_REUSE:4.5,MICRO_SEQUENCE:9.0,FEINT_BRANCH:0},orchestratorPhaseCaps:{SIGNATURE_PRIORITY:1,ARENA_MORPHOLOGY:2,COUNTER_TRIAL:2,LEARNED_REUSE:2,MICRO_SEQUENCE:1,FEINT_BRANCH:1},orchestratorPerformanceCost:{FULL:1,BALANCED:1.08,RECOVERY:1.18},orchestratorPerformanceRecovery:{FULL:1,BALANCED:.9,RECOVERY:.72},orchestratorRecoveryDefers:['ARENA_MORPHOLOGY','FEINT_BRANCH'],orchestratorContextCacheSec:.25,orchestratorUtilityAlpha:.42,orchestratorFamilyDecayPerSec:.16,orchestratorFamilyGate:.78,orchestratorFamilyPhaseCarry:.72,orchestratorFamilyAdds:{SPECTACLE:.30,SPATIAL:.34,ADAPTIVE:.24,SEQUENCE:.36},orchestratorPhaseEntryRhythm:1.75,orchestratorPhaseExitLookahead:3.2,orchestratorPhaseExitDefers:['ARENA_MORPHOLOGY','COUNTER_TRIAL','LEARNED_REUSE','MICRO_SEQUENCE'],orchestratorDurationSafety:.55,orchestratorForecastFamilyGate:.94,orchestratorMaintenanceInterval:.10,orchestratorRhythmSpacing:{ENTRY:1.55,BUILD:1.05,CLIMAX:.82,EXIT_RISK:1.35},orchestratorSpacingPerformance:{FULL:1,BALANCED:1.08,RECOVERY:1.18},orchestratorSpacingPressureGain:.68,signaturePilot:true,signaturePilotWorld:11,signaturePilotConfidence:.78,signaturePilotCooldownTarget:1.05,tacticalPilot:true,tacticalPilotWorld:11,tacticalPilotPhase:2,tacticalPilotConfidence:.68,tacticalSegmentMin:4.5,tacticalSegmentMax:7.0,tacticalNeutralMin:3.0,tacticalNeutralMax:4.8,tacticalActiveBeforeNeutral:2,tacticalAimMax:.115,tacticalYOffsetMax:.075,tacticalXOffsetMax:.035,arenaPilot:true,arenaPilotWorld:11,arenaPilotPhase:2,arenaPilotConfidence:.72,arenaTelegraph:.88,arenaSafeBand:.13,arenaCooldownFloor:3.1,arenaMaxEvents:12,counterEvolution:true,counterEvolutionWorld:11,counterEvolutionPhase:2,counterEvolutionConfidence:.74,counterEvolutionRepeatSamples:4,counterEvolutionTrialSec:5.5,counterEvolutionLearnedSec:4.4,counterEvolutionCooldown:6.5,counterEvolutionMaxTrialsPerPhase:2,counterEvolutionMaxReuses:2,counterEvolutionSuccessDpsDrop:.10,counterEvolutionMaxEvents:36,microSequence:true,microSequenceWorld:11,microSequencePhase:2,microSequenceConfidence:.78,microSequenceMaxPerPhase:1,microSequenceDecodeSamples:2,microSequenceMutationSec:1.35,microSequenceCooldown:8.0,microSequencePressureRecovery:1.12,branchFeint:true,branchFeintWorld:11,branchFeintPhase:2,branchFeintConfidence:.80,branchFeintMaxPerPhase:1,branchTelegraphSec:.90,branchCommitDelay:.34,branchDecodeGrace:.42,branchReactionRatio:.045,branchLaneOffset:.145,branchPulseSec:.18};
...
function hiveRollingDps(windowSec=HIVE_DIRECTOR_CONFIG.damageWindows.sustained,bossOnly=false){
...
function hiveMedian(values){const a=values.filter(Number.isFinite).sort((x,y)=>x-y),n=a.length;if(!n)return null;const m=Math.floor(n/2);return n%2?a[m]:(a[m-1]+a[m])/2;}
function hiveHistoricalBaseline(world=G?.sector||1,difficulty=G?.runDifficulty||'normal'){
  const fights=hiveLoadTelemetry().fights.filter(r=>r&&r.world===world&&(r.difficulty||'normal')===difficulty&&r.result==='victory'&&Number.isFinite(Number(r.dpsDuringBoss))).slice(-HIVE_DIRECTOR_CONFIG.historySamples);
  const medianDps=hiveMedian(fights.map(r=>Number(r.dpsDuringBoss))),medianTtk=hiveMedian(fights.map(r=>Number(r.ttkReal)));return {samples:fights.length,medianDps:Number.isFinite(medianDps)?+medianDps.toFixed(3):null,medianTtk:Number.isFinite(medianTtk)?+medianTtk.toFixed(2):null,eligible:fights.length>=HIVE_DIRECTOR_CONFIG.historyMin};
}
function hiveCalibratedExpected(base,history){base=Math.max(.001,Number(base)||.001);if(!history?.eligible||!Number.isFinite(history.medianDps))return {expected:base,weight:0,delta:0};const blended=base*(1-HIVE_DIRECTOR_CONFIG.historyBlend)+history.medianDps*HIVE_DIRECTOR_CONFIG.historyBlend,lo=base*(1-HIVE_DIRECTOR_CONFIG.historyClamp),hi=base*(1+HIVE_DIRECTOR_CONFIG.historyClamp),expected=clamp(blended,lo,hi);return {expected:+expected.toFixed(4),weight:HIVE_DIRECTOR_CONFIG.historyBlend,delta:+((expected/base)-1).toFixed(4)};}
...
function hiveDpsProfile(expectedDps=1,age=0){
  const burst=hiveRollingDps(HIVE_DIRECTOR_CONFIG.damageWindows.burst,true),tactical=hiveRollingDps(HIVE_DIRECTOR_CONFIG.damageWindows.tactical,true),sustained=hiveRollingDps(HIVE_DIRECTOR_CONFIG.damageWindows.sustained,true),expected=Math.max(.001,Number(expectedDps)||.001);
  const effective=age<5?(burst*.34+tactical*.46+sustained*.20):(burst*.15+tactical*.30+sustained*.55),mx=Math.max(burst,tactical,sustained),mn=Math.min(burst,tactical,sustained),spread=mx>0?(mx-mn)/mx:0,burstRatio=sustained>0?burst/sustained:(burst>0?2:1),trend=sustained>0?tactical/sustained:1;
  const br=burst/expected,tr=tactical/expected,sr=sustained/expected;let fingerprint='STEADY';if(br>=1.65&&burstRatio>=1.48&&sr<1.38)fingerprint='BURST_SPIKE';else if(sr>=1.55&&tr>=1.45)fingerprint='SUSTAINED_DOMINATION';else if(tr>=1.24&&tactical>sustained+1)fingerprint='RAMP_UP';else if(tr<=.76&&sustained>0)fingerprint='COOLING';else if(sr<.55&&age>=7)fingerprint='LOW_OUTPUT';
  const first=HIVE_DAMAGE.firstBoss,coverage=first===null?0:clamp((hiveNowSec()-first+1)/HIVE_DIRECTOR_CONFIG.damageWindows.sustained,0,1),agreement=1-clamp(spread,.0,.75),confidence=clamp(.25+clamp(age/12,0,1)*.34+coverage*.25+agreement*.16,.25,1);
  return {burst:+burst.toFixed(3),tactical:+tactical.toFixed(3),sustained:+sustained.toFixed(3),effective:+effective.toFixed(3),burstRatio:+burstRatio.toFixed(3),trend:+trend.toFixed(3),spread:+spread.toFixed(3),burstVsExpected:+br.toFixed(3),tacticalVsExpected:+tr.toFixed(3),sustainedVsExpected:+sr.toFixed(3),coverage:+coverage.toFixed(3),confidence:+confidence.toFixed(3),fingerprint};
}
function hiveRawAdaptiveState(dps,ttk,powerIndex,age,target,initial=false,confidence=1,profile=null){
  const expected=Number(target?.expectedDps)||1,ratio=Math.max(0,Number(dps)||0)/expected,t=Number(ttk),conf=clamp(Number(confidence)||0,0,1),burstOnly=profile?.fingerprint==='BURST_SPIKE'&&(profile?.sustainedVsExpected||0)<1.38;
  if(Number.isFinite(t)&&dps>0&&conf>=.62&&!burstOnly&&(t<=Math.min(25,target.low*.42)||(ratio>=2.5&&t<target.low*.72)))return 'A3';
  if(Number.isFinite(t)&&dps>0&&conf>=.55&&t<target.low*.82&&ratio>=1.35)return burstOnly&&age<12?'A1':'A2';
  if(!initial&&age>=6&&conf>=.52&&dps>0&&(t>target.high*1.45||ratio<.48))return 'A0';
  if(initial){if(powerIndex>=2.6)return 'A3';if(powerIndex>=1.65)return 'A2';if(powerIndex<.68)return 'A0';}
  if(!initial&&age>=9&&conf>=.52&&dps<=.01&&powerIndex<.70)return 'A0';return 'A1';
}
...
function hiveFairnessHold(b=G?.boss){const reasons=[];if(!b)return {active:false,reasons};if((b.entry||0)>0)reasons.push('boss_entry');if((b.phaseTransitionT||0)>0)reasons.push('phase_transition');if((G?.recoveryGraceT||0)>0)reasons.push('player_recovery_grace');if((G?.lastChanceT||0)>0)reasons.push('last_chance');if(G?.mode==='bossRush')reasons.push('boss_rush');return {active:reasons.length>0,reasons};}
// v2.15.30 · PACING ECOLOGY COORDINATOR
// Añade headroom temporal por duración real, fatiga proyectada y separación dinámica entre intervenciones.
// El mantenimiento interno se agrupa a baja frecuencia para reducir filtros/decays por frame sin cambiar el combate.
// FEINT_BRANCH conserva compatibilidad jerárquica con su MICRO_SEQUENCE padre.
function hiveOrchestratorState(){const h=G?.hiveDirector;if(!h)return null;if(!h.orchestrator)h.orchestrator={budget:HIVE_DIRECTOR_CONFIG.orchestratorBudgetStart,quietUntil:0,recent:[],events:[],spent:{},denied:{},commits:0,phaseCounts:{},phaseTotals:{},phaseMajors:{},phaseSpend:{},lastByType:{},lastUtility:{},utilityEma:{},contextCache:null,familyFatigue:{},performanceDeferrals:0,contextDenials:0,conflictDeferrals:0,pressureDeferrals:0,phaseSalienceDenials:0,familyFatigueDenials:0,phaseExitDeferrals:0,phaseEntryDeferrals:0,headroomDenials:0,forecastFatigueDenials:0,rhythmSpacingDenials:0,coalescedReservations:0,pressure:0,reservations:[],lastPhase:Number(G?.boss?.phase||0),phaseEntries:0,lastCommitAt:-999,maintenanceAcc:0,maintenancePasses:0};const st=h.orchestrator;st.phaseSpend=st.phaseSpend||{};st.reservations=Array.isArray(st.reservations)?st.reservations:[];st.utilityEma=st.utilityEma||{};st.familyFatigue=st.familyFatigue||{};st.contextCache=st.contextCache||null;st.pressure=clamp(Number(st.pressure)||0,0,1);for(const k of ['conflictDeferrals','pressureDeferrals','phaseSalienceDenials','familyFatigueDenials','phaseExitDeferrals','phaseEntryDeferrals','headroomDenials','forecastFatigueDenials','rhythmSpacingDenials','coalescedReservations','phaseEntries','maintenancePasses'])st[k]=Number(st[k])||0;if(!Number.isFinite(Number(st.lastCommitAt)))st.lastCommitAt=-999;st.maintenanceAcc=Math.max(0,Number(st.maintenanceAcc)||0);if(!Number.isFinite(Number(st.lastPhase)))st.lastPhase=Number(G?.boss?.phase||0);return st;}
function hiveOrchestratorAge(){const h=G?.hiveDirector;return h?Math.max(0,(G?.elapsed||0)-h.startedAt):0;}
function hiveOrchestratorCost(type){return Math.max(0,Number(HIVE_DIRECTOR_CONFIG.orchestratorCosts?.[type]||0));}
function hiveOrchestratorTier(){return ['FULL','BALANCED','RECOVERY'].includes(PERF?.tier)?PERF.tier:'FULL';}
function hiveOrchestratorAdjustedCost(type,cost=hiveOrchestratorCost(type)){const tier=hiveOrchestratorTier(),factor=Number(HIVE_DIRECTOR_CONFIG.orchestratorPerformanceCost?.[tier]||1);return +(Math.max(0,cost)*factor).toFixed(3);}
function hiveOrchestratorPhasePolicy(phase=Number(G?.boss?.phase||1)){const p=HIVE_DIRECTOR_CONFIG.orchestratorPhasePolicies?.[phase]||HIVE_DIRECTOR_CONFIG.orchestratorPhasePolicies?.[3]||{};return {total:Math.min(HIVE_DIRECTOR_CONFIG.orchestratorPhaseTotalCap,Number(p.total??HIVE_DIRECTOR_CONFIG.orchestratorPhaseTotalCap)),major:Math.min(HIVE_DIRECTOR_CONFIG.orchestratorPhaseMajorCap,Number(p.major??HIVE_DIRECTOR_CONFIG.orchestratorPhaseMajorCap)),salience:Math.max(0,Number(p.salience??Infinity))};}
function hiveOrchestratorFamily(type){if(type==='SIGNATURE_PRIORITY')return 'SPECTACLE';if(type==='ARENA_MORPHOLOGY')return 'SPATIAL';if(type==='COUNTER_TRIAL'||type==='LEARNED_REUSE')return 'ADAPTIVE';if(type==='MICRO_SEQUENCE'||type==='FEINT_BRANCH')return 'SEQUENCE';return 'OTHER';}
function hiveOrchestratorRhythm(){const h=G?.hiveDirector,b=G?.boss,phase=Number(b?.phase||1),exp=h?.experience||{},elapsed=Math.max(0,Number(exp.phaseElapsed??b?.phaseElapsed??0)||0),phaseMin=Math.max(1,Number(exp.phaseMin)||bossPhaseMinDuration(G?.sector||1,phase)||1),timeToNext=Number(exp.timeToNextPhase),nativeGate=!!exp.nativeGateProtecting,exitRisk=phase<3&&!nativeGate&&Number.isFinite(timeToNext)&&timeToNext<=HIVE_DIRECTOR_CONFIG.orchestratorPhaseExitLookahead;let zone='BUILD';if(elapsed<HIVE_DIRECTOR_CONFIG.orchestratorPhaseEntryRhythm)zone='ENTRY';else if(exitRisk)zone='EXIT_RISK';else if(elapsed>=phaseMin*.62)zone='CLIMAX';return {zone,phase,elapsed:+elapsed.toFixed(2),phaseMin:+phaseMin.toFixed(2),progress:+clamp(elapsed/phaseMin,0,1.5).toFixed(3),timeToNext:Number.isFinite(timeToNext)?+timeToNext.toFixed(2):null,nativeGate,exitRisk};}
function hiveOrchestratorReserveDuration(type,major=false,reserveFor=null){return Math.max(.15,Number(reserveFor??HIVE_DIRECTOR_CONFIG.orchestratorReservationDurations?.[type]??(major?2.0:.8)));}
function hiveOrchestratorHeadroom(type,rhythm=hiveOrchestratorRhythm(),major=false,reserveFor=null){if(type==='SIGNATURE_PRIORITY'||hiveOrchestratorIsNestedBranch(type)||rhythm?.nativeGate||rhythm?.phase>=3||!Number.isFinite(Number(rhythm?.timeToNext)))return {ok:true,remaining:Number(rhythm?.timeToNext),required:0};const dur=hiveOrchestratorReserveDuration(type,major,reserveFor),required=dur+Math.max(0,Number(HIVE_DIRECTOR_CONFIG.orchestratorDurationSafety)||0),remaining=Math.max(0,Number(rhythm.timeToNext));return {ok:remaining+1e-6>=required,remaining:+remaining.toFixed(2),required:+required.toFixed(2),duration:+dur.toFixed(2)};}
function hiveOrchestratorSpacing(type,major=false,rhythm=hiveOrchestratorRhythm(),pressure=0){if(hiveOrchestratorIsNestedBranch(type))return 0;const zone=rhythm?.zone||'BUILD',base=Number(HIVE_DIRECTOR_CONFIG.orchestratorRhythmSpacing?.[zone]??1),tier=hiveOrchestratorTier(),perf=Number(HIVE_DIRECTOR_CONFIG.orchestratorSpacingPerformance?.[tier]||1),pressureMul=1+clamp(Number(pressure)||0,0,1)*Math.max(0,Number(HIVE_DIRECTOR_CONFIG.orchestratorSpacingPressureGain)||0),majorMul=major?1.08:1;return +(Math.max(0,base*perf*pressureMul*majorMul)).toFixed(3);}
function hiveOrchestratorForecastFatigue(type,major=false){const st=hiveOrchestratorState(),family=hiveOrchestratorFamily(type);if(!st||family==='OTHER'||hiveOrchestratorIsNestedBranch(type))return {ok:true,family,current:0,projected:0};const current=clamp(Number(st.familyFatigue?.[family])||0,0,1),add=Number(HIVE_DIRECTOR_CONFIG.orchestratorFamilyAdds?.[family]??(major?.30:.18)),projected=clamp(current+add,0,1),gate=clamp(Number(HIVE_DIRECTOR_CONFIG.orchestratorForecastFamilyGate)||.94,.5,1);return {ok:projected<=gate+1e-6,family,current:+current.toFixed(3),projected:+projected.toFixed(3),gate:+gate.toFixed(3)};}

function hiveOrchestratorIsNestedBranch(type){return type==='FEINT_BRANCH'&&G?.hiveDirector?.counterEvolution?.active?.mode==='MICRO_SEQUENCE';}
function hiveOrchestratorConflict(type,age=hiveOrchestratorAge()){const st=hiveOrchestratorState();if(!st)return null;const nested=hiveOrchestratorIsNestedBranch(type),conflicts=HIVE_DIRECTOR_CONFIG.orchestratorConflictMatrix?.[type]||[];for(const r of st.reservations||[]){if(!r||r.until<=age)continue;if(nested&&r.type==='MICRO_SEQUENCE')continue;if(conflicts.includes(r.type))return r;}return null;}
function hiveOrchestratorReserve(type,age,reserveFor=null,major=false){const st=hiveOrchestratorState();if(!st)return null;const dur=hiveOrchestratorReserveDuration(type,major,reserveFor),until=age+dur;st.reservations=(st.reservations||[]).filter(r=>r.until>age);const same=st.reservations.find(r=>r.type===type&&r.phase===Number(G?.boss?.phase||0));if(same){same.until=+Math.max(same.until,until).toFixed(2);same.major=!!(same.major||major);st.coalescedReservations=(st.coalescedReservations||0)+1;return same;}const item={type,at:+age.toFixed(2),until:+until.toFixed(2),phase:Number(G?.boss?.phase||0),major:!!major};st.reservations.push(item);if(st.reservations.length>8)st.reservations.shift();return item;}
function hiveOrchestratorContext(type){const st=hiveOrchestratorState(),age=hiveOrchestratorAge(),cacheSec=Math.max(.05,HIVE_DIRECTOR_CONFIG.orchestratorContextCacheSec||.25),stamp=Math.floor(age/cacheSec),phase=Number(G?.boss?.phase||1);if(st?.contextCache?.stamp===stamp&&st.contextCache.phase===phase&&st.contextCache.values?.[type])return st.contextCache.values[type];const h=G?.hiveDirector,b=G?.boss,sp=hiveSpatialState(),ce=h?.counterEvolution,exp=h?.experience||{},state=h?.state||'A1',conf=clamp(Number(h?.confidence)||0,0,1),stateScore=state==='A3'?1:state==='A2'?.72:state==='A1'?.24:.12,spConf=clamp(Number(sp?.confidence)||0,0,1),risk=clamp(Number(exp.risk)||0,0,2)/2,coverage=clamp(Number(exp.coverage)||0,0,1),repeat=clamp(Number(ce?.streak||0)/Math.max(1,HIVE_DIRECTOR_CONFIG.counterEvolutionRepeatSamples),0,1),rhythm=hiveOrchestratorRhythm();let raw=.24*conf+.20*stateScore+.12*spConf+.10*clamp(phase/3,0,1)+.12*risk+.08*(1-coverage);let why=['director',`rhythm_${rhythm.zone.toLowerCase()}`];if(type==='SIGNATURE_PRIORITY'){raw+=exp.signatureDebt?.34:0;why.push(exp.signatureDebt?'signature_debt':'no_signature_debt');}else if(type==='ARENA_MORPHOLOGY'){raw+=.18*spConf+.08*(state==='A3'?1:.6);why.push('spatial_pressure');}else if(type==='COUNTER_TRIAL'){raw+=.22*repeat+.12*spConf;why.push('repeated_tactic');}else if(type==='LEARNED_REUSE'){raw+=ce?.learned?.[ce?.currentTactic]?.28:.06;raw+=.12*repeat;why.push('learned_memory');}else if(type==='MICRO_SEQUENCE'){raw+=ce?.learned?.[ce?.currentTactic]?.24:.04;raw+=.12*risk+.08*(state==='A3'?1:.5);why.push('coevolution_sequence');}else if(type==='FEINT_BRANCH'){raw+=ce?.active?.mode==='MICRO_SEQUENCE'?.42:.04;raw+=.10*spConf;why.push('sequence_branch');}raw=clamp(raw,0,1);const prev=Number(st?.utilityEma?.[type]),alpha=clamp(Number(HIVE_DIRECTOR_CONFIG.orchestratorUtilityAlpha)||.42,.05,1),smooth=Number.isFinite(prev)?prev*(1-alpha)+raw*alpha:raw;if(st){st.utilityEma[type]=smooth;if(!st.contextCache||st.contextCache.stamp!==stamp||st.contextCache.phase!==phase)st.contextCache={stamp,phase,values:{}};const out={score:+clamp(smooth,0,1).toFixed(3),rawScore:+raw.toFixed(3),why:why.join('+'),state,phase,confidence:+conf.toFixed(3),spatial:+spConf.toFixed(3),risk:+risk.toFixed(3),coverage:+coverage.toFixed(3),repeat:+repeat.toFixed(3),rhythm};st.contextCache.values[type]=out;return out;}return {score:+raw.toFixed(3),rawScore:+raw.toFixed(3),why:why.join('+'),state,phase,confidence:+conf.toFixed(3),spatial:+spConf.toFixed(3),risk:+risk.toFixed(3),coverage:+coverage.toFixed(3),repeat:+repeat.toFixed(3),rhythm};}
function hiveOrchestratorTick(dt){const st=hiveOrchestratorState();if(!st||!HIVE_DIRECTOR_CONFIG.orchestrator)return;const d=Math.max(0,Number(dt)||0),age=hiveOrchestratorAge(),phase=Number(G?.boss?.phase||0);if(st.lastPhase!==phase){st.lastPhase=phase;st.phaseEntries=(st.phaseEntries||0)+1;st.pressure=clamp((st.pressure||0)*.55,0,1);for(const k of Object.keys(st.familyFatigue||{}))st.familyFatigue[k]=clamp((Number(st.familyFatigue[k])||0)*HIVE_DIRECTOR_CONFIG.orchestratorFamilyPhaseCarry,0,1);st.reservations=[];st.contextCache=null;st.maintenanceAcc=0;st.quietUntil=Math.max(st.quietUntil||0,age+HIVE_DIRECTOR_CONFIG.orchestratorPhaseEntryQuiet);}st.maintenanceAcc=(st.maintenanceAcc||0)+d;const interval=Math.max(.02,Number(HIVE_DIRECTOR_CONFIG.orchestratorMaintenanceInterval)||.10);if(st.maintenanceAcc+1e-9<interval)return;const step=st.maintenanceAcc;st.maintenanceAcc=0;st.maintenancePasses=(st.maintenancePasses||0)+1;const tier=hiveOrchestratorTier(),tierFactor=Number(HIVE_DIRECTOR_CONFIG.orchestratorPerformanceRecovery?.[tier]||1),a1=G?.hiveDirector?.state==='A1',a1Boost=a1?HIVE_DIRECTOR_CONFIG.orchestratorA1RecoveryBoost:1,decay=HIVE_DIRECTOR_CONFIG.orchestratorPressureDecayPerSec*(a1?HIVE_DIRECTOR_CONFIG.orchestratorPressureA1DecayBoost:1);st.pressure=clamp((st.pressure||0)-step*decay,0,1);const familyDecay=step*HIVE_DIRECTOR_CONFIG.orchestratorFamilyDecayPerSec*(a1?1.2:1);for(const k of Object.keys(st.familyFatigue||{})){st.familyFatigue[k]=clamp((Number(st.familyFatigue[k])||0)-familyDecay,0,1);if(st.familyFatigue[k]<=.001)delete st.familyFatigue[k];}st.budget=clamp((st.budget||0)+step*HIVE_DIRECTOR_CONFIG.orchestratorRecoveryPerSec*tierFactor*a1Boost,0,HIVE_DIRECTOR_CONFIG.orchestratorBudgetMax);st.recent=(st.recent||[]).filter(e=>age-e.at<=HIVE_DIRECTOR_CONFIG.orchestratorRecentSec);st.reservations=(st.reservations||[]).filter(r=>r.until>age);}
function hiveOrchestratorPermit(type,{major=false,cost=hiveOrchestratorCost(type),ignoreQuiet=false,skipContext=false,reserveFor=null}={}){const st=hiveOrchestratorState(),age=hiveOrchestratorAge();if(!HIVE_DIRECTOR_CONFIG.orchestrator||!st)return {ok:true,reason:'disabled',cost:0,utility:1};const phase=Number(G?.boss?.phase||0),tier=hiveOrchestratorTier(),adjusted=hiveOrchestratorAdjustedCost(type,cost),context=hiveOrchestratorContext(type),minUtility=major?HIVE_DIRECTOR_CONFIG.orchestratorMajorUtility:HIVE_DIRECTOR_CONFIG.orchestratorMinUtility,nested=hiveOrchestratorIsNestedBranch(type),rhythm=context.rhythm||hiveOrchestratorRhythm(),family=hiveOrchestratorFamily(type),familyFatigue=clamp(Number(st.familyFatigue?.[family])||0,0,1);if(tier==='RECOVERY'&&HIVE_DIRECTOR_CONFIG.orchestratorRecoveryDefers.includes(type))return {ok:false,reason:'performance_recovery',cost:adjusted,utility:context.score};if(!skipContext&&HIVE_DIRECTOR_CONFIG.orchestratorContextual&&context.score<minUtility)return {ok:false,reason:'low_context_utility',cost:adjusted,utility:context.score};if(major&&!nested&&rhythm.zone==='ENTRY')return {ok:false,reason:'phase_entry_rhythm',cost:adjusted,utility:context.score,rhythm};if(!nested&&rhythm.zone==='EXIT_RISK'&&HIVE_DIRECTOR_CONFIG.orchestratorPhaseExitDefers.includes(type))return {ok:false,reason:'phase_exit_risk',cost:adjusted,utility:context.score,rhythm};const headroom=hiveOrchestratorHeadroom(type,rhythm,major,reserveFor);if(!nested&&!headroom.ok)return {ok:false,reason:'phase_duration_headroom',cost:adjusted,utility:context.score,rhythm,headroom};if(!nested&&family!=='OTHER'&&familyFatigue>=HIVE_DIRECTOR_CONFIG.orchestratorFamilyGate)return {ok:false,reason:'family_fatigue',cost:adjusted,utility:context.score,family,familyFatigue:+familyFatigue.toFixed(3)};const fatigueForecast=hiveOrchestratorForecastFatigue(type,major);if(!fatigueForecast.ok)return {ok:false,reason:'forecast_family_fatigue',cost:adjusted,utility:context.score,...fatigueForecast};const conflict=hiveOrchestratorConflict(type,age);if(conflict)return {ok:false,reason:`reservation_conflict_${conflict.type}`,cost:adjusted,utility:context.score,conflict};if(!ignoreQuiet&&age<(st.quietUntil||0))return {ok:false,reason:'quiet_window',cost:adjusted,utility:context.score};const pressure=clamp(Number(st.pressure)||0,0,1),pressureGate=major?HIVE_DIRECTOR_CONFIG.orchestratorPressureMajorGate:HIVE_DIRECTOR_CONFIG.orchestratorPressureMinorGate;if(!nested&&pressure>=pressureGate)return {ok:false,reason:'pressure_recovery',cost:adjusted,utility:context.score,pressure:+pressure.toFixed(3)};const spacing=hiveOrchestratorSpacing(type,major,rhythm,pressure),sinceLast=age-Number(st.lastCommitAt??-999);if(!nested&&sinceLast+1e-6<spacing)return {ok:false,reason:'rhythm_spacing',cost:adjusted,utility:context.score,spacing,sinceLast:+Math.max(0,sinceLast).toFixed(3),rhythm};const last=Number(st.lastByType?.[type]);const typeCd=Number(HIVE_DIRECTOR_CONFIG.orchestratorTypeCooldown?.[type]||0);if(Number.isFinite(last)&&age-last<typeCd)return {ok:false,reason:'type_cooldown',cost:adjusted,utility:context.score};const phaseMap=st.phaseCounts?.[phase]||{},phaseType=Number(phaseMap[type]||0),phaseCap=Number(HIVE_DIRECTOR_CONFIG.orchestratorPhaseCaps?.[type]??999),phaseTotal=Number(st.phaseTotals?.[phase]||0),phaseMajor=Number(st.phaseMajors?.[phase]||0),phaseSpent=Number(st.phaseSpend?.[phase]||0),policy=hiveOrchestratorPhasePolicy(phase);if(phaseType>=phaseCap)return {ok:false,reason:'phase_type_cap',cost:adjusted,utility:context.score};if(phaseTotal>=policy.total)return {ok:false,reason:'phase_attention_cap',cost:adjusted,utility:context.score};if(major&&phaseMajor>=policy.major)return {ok:false,reason:'phase_major_cap',cost:adjusted,utility:context.score};if(phaseSpent+adjusted>policy.salience+1e-6)return {ok:false,reason:'phase_salience_cap',cost:adjusted,utility:context.score,phaseSpent:+phaseSpent.toFixed(2),phaseSalience:policy.salience};if((st.budget||0)+1e-6<adjusted)return {ok:false,reason:'salience_budget',cost:adjusted,utility:context.score};const repeats=(st.recent||[]).filter(e=>e.type===type&&age-e.at<=HIVE_DIRECTOR_CONFIG.orchestratorRecentSec).length;if(repeats>=HIVE_DIRECTOR_CONFIG.orchestratorRepeatLimit)return {ok:false,reason:'repeat_fatigue',cost:adjusted,utility:context.score};if(major&&G?.boss?.phaseTransitionT>0)return {ok:false,reason:'phase_transition',cost:adjusted,utility:context.score};return {ok:true,reason:'eligible',cost:adjusted,utility:context.score,context,phasePolicy:policy,pressure:+pressure.toFixed(3),family,familyFatigue:+familyFatigue.toFixed(3),fatigueForecast,headroom,spacing,rhythm};}
function hiveOrchestratorDeny(type,reason){const st=hiveOrchestratorState();if(!st)return false;st.denied[type]=(st.denied[type]||0)+1;if(reason==='performance_recovery')st.performanceDeferrals=(st.performanceDeferrals||0)+1;if(reason==='low_context_utility')st.contextDenials=(st.contextDenials||0)+1;if(String(reason).startsWith('reservation_conflict_'))st.conflictDeferrals=(st.conflictDeferrals||0)+1;if(reason==='pressure_recovery')st.pressureDeferrals=(st.pressureDeferrals||0)+1;if(reason==='phase_salience_cap')st.phaseSalienceDenials=(st.phaseSalienceDenials||0)+1;if(reason==='family_fatigue')st.familyFatigueDenials=(st.familyFatigueDenials||0)+1;if(reason==='forecast_family_fatigue')st.forecastFatigueDenials=(st.forecastFatigueDenials||0)+1;if(reason==='phase_exit_risk')st.phaseExitDeferrals=(st.phaseExitDeferrals||0)+1;if(reason==='phase_entry_rhythm')st.phaseEntryDeferrals=(st.phaseEntryDeferrals||0)+1;if(reason==='phase_duration_headroom')st.headroomDenials=(st.headroomDenials||0)+1;if(reason==='rhythm_spacing')st.rhythmSpacingDenials=(st.rhythmSpacingDenials||0)+1;st.lastDenied={type,reason,at:+hiveOrchestratorAge().toFixed(2),tier:hiveOrchestratorTier(),pressure:+(st.pressure||0).toFixed(3),rhythm:hiveOrchestratorRhythm().zone};return false;}
function hiveOrchestratorCommit(type,{major=false,cost=hiveOrchestratorCost(type),reason='adaptive_intervention',ignoreQuiet=false,skipContext=false,reserveFor=null}={}){const st=hiveOrchestratorState();if(!st||!HIVE_DIRECTOR_CONFIG.orchestrator)return true;const permit=hiveOrchestratorPermit(type,{major,cost,ignoreQuiet,skipContext,reserveFor});if(!permit.ok)return hiveOrchestratorDeny(type,permit.reason);const age=hiveOrchestratorAge(),phase=Number(G?.boss?.phase||0),spend=permit.cost,utility=permit.utility,family=permit.family||hiveOrchestratorFamily(type),rhythm=permit.rhythm||hiveOrchestratorRhythm();st.budget=clamp(st.budget-spend,0,HIVE_DIRECTOR_CONFIG.orchestratorBudgetMax);const pressureAdd=(major?.28:.12)+Math.min(.18,spend/200);st.pressure=clamp((st.pressure||0)+pressureAdd,0,1);if(family!=='OTHER'){const add=Number(HIVE_DIRECTOR_CONFIG.orchestratorFamilyAdds?.[family]??(major?.30:.18));st.familyFatigue[family]=clamp((Number(st.familyFatigue[family])||0)+add,0,1);}const quietBase=major?HIVE_DIRECTOR_CONFIG.orchestratorQuietMajor:HIVE_DIRECTOR_CONFIG.orchestratorQuietMinor,pressureQuiet=(st.pressure||0)*(major?HIVE_DIRECTOR_CONFIG.orchestratorPressureQuietMajor:HIVE_DIRECTOR_CONFIG.orchestratorPressureQuietMinor);st.quietUntil=Math.max(st.quietUntil||0,age+quietBase+pressureQuiet);st.spent[type]=+(Number(st.spent[type]||0)+spend).toFixed(2);st.commits++;st.lastCommitAt=age;st.lastByType[type]=age;st.lastUtility[type]=utility;st.phaseCounts[phase]=st.phaseCounts[phase]||{};st.phaseCounts[phase][type]=(st.phaseCounts[phase][type]||0)+1;st.phaseTotals[phase]=(st.phaseTotals[phase]||0)+1;st.phaseSpend[phase]=+(Number(st.phaseSpend[phase]||0)+spend).toFixed(2);if(major)st.phaseMajors[phase]=(st.phaseMajors[phase]||0)+1;const reservation=hiveOrchestratorReserve(type,age,reserveFor,major);const ev={type,at:+age.toFixed(2),phase,cost:+spend.toFixed(2),utility:+utility.toFixed(3),rawUtility:permit.context?.rawScore??null,budgetAfter:+st.budget.toFixed(2),phaseSpendAfter:st.phaseSpend[phase],pressureAfter:+st.pressure.toFixed(3),family,familyFatigueAfter:+(st.familyFatigue?.[family]||0).toFixed(3),rhythm:rhythm.zone,spacing:permit.spacing??0,headroom:permit.headroom||null,fatigueForecast:permit.fatigueForecast||null,reserveUntil:reservation?.until??null,major:!!major,tier:hiveOrchestratorTier(),reason};st.recent.push(ev);st.events.push(ev);if(st.recent.length>12)st.recent.shift();if(st.events.length>42)st.events.shift();return true;}
function hiveOrchestratorSnapshot(){const st=hiveOrchestratorState(),age=hiveOrchestratorAge(),phase=Number(G?.boss?.phase||0);return st?{budget:+(st.budget||0).toFixed(2),max:HIVE_DIRECTOR_CONFIG.orchestratorBudgetMax,quietRemaining:+Math.max(0,(st.quietUntil||0)-age).toFixed(2),sinceLastCommit:+Math.max(0,age-Number(st.lastCommitAt??age)).toFixed(2),pressure:+(st.pressure||0).toFixed(3),rhythm:hiveOrchestratorRhythm(),familyFatigue:Object.fromEntries(Object.entries(st.familyFatigue||{}).map(([k,v])=>[k,+Number(v||0).toFixed(3)])),utilityEma:Object.fromEntries(Object.entries(st.utilityEma||{}).map(([k,v])=>[k,+Number(v||0).toFixed(3)])),phasePolicy:hiveOrchestratorPhasePolicy(phase),reservations:(st.reservations||[]).filter(r=>r.until>age).map(r=>({...r,remaining:+Math.max(0,r.until-age).toFixed(2)})),recent:[...(st.recent||[])],spent:{...(st.spent||{})},denied:{...(st.denied||{})},commits:st.commits||0,phaseCounts:JSON.parse(JSON.stringify(st.phaseCounts||{})),phaseTotals:{...(st.phaseTotals||{})},phaseMajors:{...(st.phaseMajors||{})},phaseSpend:{...(st.phaseSpend||{})},lastUtility:{...(st.lastUtility||{})},performanceDeferrals:st.performanceDeferrals||0,contextDenials:st.contextDenials||0,conflictDeferrals:st.conflictDeferrals||0,pressureDeferrals:st.pressureDeferrals||0,phaseSalienceDenials:st.phaseSalienceDenials||0,familyFatigueDenials:st.familyFatigueDenials||0,forecastFatigueDenials:st.forecastFatigueDenials||0,phaseExitDeferrals:st.phaseExitDeferrals||0,phaseEntryDeferrals:st.phaseEntryDeferrals||0,headroomDenials:st.headroomDenials||0,rhythmSpacingDenials:st.rhythmSpacingDenials||0,coalescedReservations:st.coalescedReservations||0,phaseEntries:st.phaseEntries||0,maintenancePasses:st.maintenancePasses||0,maintenancePending:+(st.maintenanceAcc||0).toFixed(3),lastDenied:st.lastDenied||null,performanceTier:hiveOrchestratorTier()}:null;}
function hiveDirectorSampleInterval(){if(!HIVE_DIRECTOR_CONFIG.adaptiveSampling)return HIVE_DIRECTOR_CONFIG.sampleInterval;if(PERF?.tier==='RECOVERY')return HIVE_DIRECTOR_CONFIG.sampleRecovery;if(PERF?.tier==='BALANCED')return HIVE_DIRECTOR_CONFIG.sampleBalanced;return HIVE_DIRECTOR_CONFIG.sampleInterval;}

function hiveSpatialState(){
  const h=G?.hiveDirector;if(!h)return null;if(!h.spatial)h.spatial={elapsed:0,sampleT:0,samples:0,lastX:null,lastY:null,lastVy:0,vySmooth:0,travel:0,reversals:0,lastVySign:0,zones:{upper:0,center:0,lower:0},verticalEdgeT:0,centerT:0,read:'READING',confidence:0,dominant:'center',predictedY:G?.player?.y||H*.5,travelRate:0,reversalRate:0,edgeRatio:0,centerRatio:0};return h.spatial;
}
function hiveSpatialUpdate(dt){
  const h=G?.hiveDirector,p=G?.player;if(!h||h.finished||!p||!G?.boss)return;const s=hiveSpatialState(),d=Math.max(0,Number(dt)||0);s.elapsed+=d;s.sampleT+=d;if(s.sampleT<.20)return;const step=s.sampleT;s.sampleT=0;const x=Number(p.x)||0,y=Number(p.y)||0;if(s.lastY!==null){const dy=y-s.lastY,vy=dy/Math.max(.04,step),sign=Math.abs(vy)>H*.055?Math.sign(vy):0;s.travel+=Math.abs(dy);s.lastVy=vy;s.vySmooth=lerp(s.vySmooth||0,vy,.38);if(sign&&s.lastVySign&&sign!==s.lastVySign)s.reversals++;if(sign)s.lastVySign=sign;}s.lastX=x;s.lastY=y;s.samples++;
  const zone=y<H*.34?'upper':y>H*.66?'lower':'center';s.zones[zone]=(s.zones[zone]||0)+step;if(y<H*.23||y>H*.77)s.verticalEdgeT+=step;if(y>=H*.38&&y<=H*.62)s.centerT+=step;const total=Math.max(.001,s.zones.upper+s.zones.center+s.zones.lower),sorted=Object.entries(s.zones).sort((a,b)=>b[1]-a[1]),dom=sorted[0],domRatio=dom[1]/total,travelRate=(s.travel/Math.max(1,H))/Math.max(.25,s.elapsed),reversalRate=s.reversals/Math.max(.5,s.elapsed),edgeRatio=s.verticalEdgeT/Math.max(.25,s.elapsed),centerRatio=s.centerT/Math.max(.25,s.elapsed);let read='MOBILE_MIX';if(s.elapsed<3.5||s.samples<10)read='READING';else if(edgeRatio>.52&&travelRate<.48)read='EDGE_ANCHOR';else if(reversalRate>.48&&travelRate>.30)read='VERTICAL_WEAVE';else if(centerRatio>.58&&travelRate<.22)read='CENTER_HOLD';else if(domRatio>.66&&travelRate<.30)read='LANE_ANCHOR';else if(reversalRate>.78||travelRate>.72)read='EVASIVE';const confidence=read==='READING'?clamp(s.elapsed/5*.35,0,.35):clamp(.42+Math.min(.28,Math.abs(domRatio-.34)*.55)+Math.min(.18,Math.abs(travelRate-.32)*.30)+Math.min(.12,reversalRate*.12),.42,.96);s.read=read;s.confidence=confidence;s.dominant=dom[0];s.travelRate=travelRate;s.reversalRate=reversalRate;s.edgeRatio=edgeRatio;s.centerRatio=centerRatio;s.predictedY=clamp(y+(s.vySmooth||0)*.26,70,H-70);
}
function hiveSpatialSnapshot(){const s=hiveSpatialState();if(!s)return null;return {read:s.read,confidence:+s.confidence.toFixed(3),dominant:s.dominant,predictedY:+s.predictedY.toFixed(1),vy:+(s.vySmooth||0).toFixed(2),travelRate:+s.travelRate.toFixed(3),reversalRate:+s.reversalRate.toFixed(3),edgeRatio:+s.edgeRatio.toFixed(3),centerRatio:+s.centerRatio.toFixed(3),samples:s.samples};}
const HIVE_TACTICAL_IDENTITY={KHEPRI:{variant:'ORBIT_TILT',aim:.56,y:.62,x:.52,spread:1.08,ring:.28},VESPERA:{variant:'VECTOR_INTERCEPT',aim:.95,y:.76,x:.62,spread:.88,ring:.08},REGENT:{variant:'PINCER_TRACK',aim:.72,y:.88,x:.34,spread:1.18,ring:.12},PYROLUX:{variant:'FIGURE_EIGHT_PHASE',aim:.58,y:.58,x:.44,spread:1.12,ring:.42},CAUSTIC:{variant:'DISTILLERY_CROSSWIND',aim:.52,y:.46,x:.28,spread:1.20,ring:.36},ATLAS:{variant:'VEIL_OFFSET',aim:.60,y:.70,x:.40,spread:.94,ring:.50},MIRAGE:{variant:'MIRROR_OPPOSITION',aim:.78,y:.82,x:.56,spread:.84,ring:.60},PIT:{variant:'AMBUSH_OFFSET',aim:.66,y:.86,x:.30,spread:1.14,ring:.22},AURICULUS:{variant:'TAIL_LEAD',aim:.90,y:.64,x:.50,spread:.90,ring:.16},APIS:{variant:'ROYAL_PRESSURE',aim:.64,y:.60,x:.36,spread:1.10,ring:.30}};
function hiveTacticalIdentitySpec(identity=bossIdentity()){return identity?HIVE_TACTICAL_IDENTITY[identity.key]||null:null;}
function hiveTacticalPilotState(){const h=G?.hiveDirector;if(!h)return null;if(!h.tacticalPilot)h.tacticalPilot={active:false,variant:'NEUTRAL',segmentT:0,intensity:0,activeSegments:0,totalSegments:0,neutralSegments:0,events:[],lastReason:'init'};return h.tacticalPilot;}
function hiveTacticalEligible(h=G?.hiveDirector,b=G?.boss){if(!HIVE_DIRECTOR_CONFIG.tacticalPilot||!h||!b||h.finished||h.finalizing||G?.mode!=='campaign'||G.sector<HIVE_DIRECTOR_CONFIG.tacticalPilotWorld||b.phase<HIVE_DIRECTOR_CONFIG.tacticalPilotPhase||!bossIdentity())return {ok:false,reason:'scope'};const hold=hiveFairnessHold(b);if(hold.active)return {ok:false,reason:hold.reasons.join('+')};if(h.state!=='A2'&&h.state!=='A3')return {ok:false,reason:'state'};if((h.confidence||0)<HIVE_DIRECTOR_CONFIG.tacticalPilotConfidence)return {ok:false,reason:'director_confidence'};if(h.dpsProfile?.fingerprint==='BURST_SPIKE')return {ok:false,reason:'burst_guard'};if(b.signatureAttack||b.telegraphT>0||b.specialT>0||b.chargeT>0)return {ok:false,reason:'attack_lock'};const sp=hiveSpatialState();if(!sp||sp.read==='READING'||sp.confidence<.46)return {ok:false,reason:'spatial_confidence'};return {ok:true,reason:'eligible'};}
function hiveTacticalChooseVariant(identity,sp){const spec=hiveTacticalIdentitySpec(identity);if(!spec)return 'NEUTRAL';if(sp?.read==='EVASIVE')return `${spec.variant}_STABILIZE`;if(sp?.read==='EDGE_ANCHOR')return `${spec.variant}_EDGE`;if(sp?.read==='VERTICAL_WEAVE')return `${spec.variant}_WEAVE`;if(sp?.read==='CENTER_HOLD')return `${spec.variant}_CENTER`;if(sp?.read==='LANE_ANCHOR')return `${spec.variant}_LANE`;return spec.variant;}
function hiveTacticalPilotUpdate(dt){
  const h=G?.hiveDirector,b=G?.boss,identity=bossIdentity();if(!h||!b||h.finished||!identity)return;const tp=hiveTacticalPilotState(),eligible=hiveTacticalEligible(h,b),d=Math.max(0,Number(dt)||0);tp.segmentT=Math.max(0,(tp.segmentT||0)-d);if(!eligible.ok){tp.intensity=lerp(tp.intensity||0,0,clamp(d*4.5,0,1));tp.active=false;tp.lastReason=eligible.reason;return;}if(tp.segmentT<=0){const forceNeutral=tp.activeSegments>=HIVE_DIRECTOR_CONFIG.tacticalActiveBeforeNeutral;if(forceNeutral){tp.variant='NEUTRAL';tp.active=false;tp.activeSegments=0;tp.neutralSegments++;tp.segmentT=rnd(HIVE_DIRECTOR_CONFIG.tacticalNeutralMin,HIVE_DIRECTOR_CONFIG.tacticalNeutralMax);}else{const sp=hiveSpatialState();tp.variant=hiveTacticalChooseVariant(identity,sp);tp.active=true;tp.activeSegments++;tp.totalSegments++;tp.segmentT=rnd(HIVE_DIRECTOR_CONFIG.tacticalSegmentMin,HIVE_DIRECTOR_CONFIG.tacticalSegmentMax);const ev={at:+Math.max(0,(G.elapsed||0)-h.startedAt).toFixed(2),phase:b.phase,world:G.sector,boss:identity.key,variant:tp.variant,spatial:sp.read,spatialConfidence:+sp.confidence.toFixed(3),directorState:h.state,directorConfidence:+(h.confidence||0).toFixed(3)};tp.events.push(ev);if(tp.events.length>18)tp.events.shift();} }tp.intensity=lerp(tp.intensity||0,tp.active?1:0,clamp(d*(tp.active?1.8:2.8),0,1));tp.lastReason='eligible';
}
function hiveTacticalVector(b,p,identity=bossIdentity()){const tp=hiveTacticalPilotState(),sp=hiveSpatialState(),spec=hiveTacticalIdentitySpec(identity),ce=hiveCounterEvolutionEffect();if(!tp||!sp||!spec||!tp.active||tp.intensity<.03)return {active:false,variant:'NEUTRAL',intensity:0,xOffset:0,yOffset:0,aimAngle:Math.atan2(p.y-b.y,p.x-b.x),spreadScale:1,ringPhase:0,predictedY:p.y,counterEvolution:ce?.id||null};const i=clamp(tp.intensity,0,1),pred=clamp(sp.predictedY,70,H-70),dy=clamp((pred-H*.5)/Math.max(1,H),-HIVE_DIRECTOR_CONFIG.tacticalYOffsetMax,HIVE_DIRECTOR_CONFIG.tacticalYOffsetMax),vySign=Math.sign(sp.vy||0),read=sp.read;let yMul=spec.y*(ce?.yMul||1),xMul=spec.x*(ce?.xMul||1),aimMul=spec.aim*(ce?.aimMul||1);if(read==='EDGE_ANCHOR')yMul*=.72;else if(read==='VERTICAL_WEAVE')aimMul*=1.10;else if(read==='EVASIVE'){aimMul*=.68;yMul*=.65;}const yOffset=H*dy*yMul*i,xDir=(identity.key==='VESPERA'||identity.key==='MIRAGE'||identity.key==='AURICULUS')?-1:1,xOffset=W*HIVE_DIRECTOR_CONFIG.tacticalXOffsetMax*xMul*i*xDir*(sp.dominant==='center'?.55:1),leadY=lerp(p.y,pred,clamp(aimMul*i,0,.86)),base=Math.atan2(p.y-b.y,p.x-b.x),lead=Math.atan2(leadY-b.y,p.x-b.x),delta=Math.atan2(Math.sin(lead-base),Math.cos(lead-base)),aimAngle=base+clamp(delta,-HIVE_DIRECTOR_CONFIG.tacticalAimMax,HIVE_DIRECTOR_CONFIG.tacticalAimMax);const spreadTarget=clamp(spec.spread*(ce?.spreadMul||1),.82,1.22),spreadScale=clamp(lerp(1,spreadTarget,i*(read==='VERTICAL_WEAVE'?.72:.48)),.82,1.22),ringPhase=((spec.ring||0)+(ce?.ringOffset||0))*i*(vySign||1);return {active:true,variant:tp.variant+(ce?`+CE_${ce.id}`:''),intensity:+i.toFixed(3),xOffset,yOffset,aimAngle,spreadScale,ringPhase,predictedY:pred,spatialRead:read,counterEvolution:ce?.id||null};}
function hiveTacticalSpread(base,tactical){return base*(tactical?.active?tactical.spreadScale:1);}
function hiveTacticalRingPhase(base,tactical){return base+(tactical?.active?tactical.ringPhase:0);}
// v2.15.22 · Boss arena becomes a readable extension of boss identity without extra projectile density.
const HIVE_ARENA_IDENTITY={
  KHEPRI:{motif:'GRAVITY_CORRIDOR',label:'ÓRBITA DE GRAVEDAD',bias:0},VESPERA:{motif:'LANCER_RUNWAY',label:'CORREDOR DE LANZA',bias:1},REGENT:{motif:'MANDIBLE_GATE',label:'PUERTA MANDIBULAR',bias:-1},PYROLUX:{motif:'BEACON_ARC',label:'ARCO BIOLUMÍNICO',bias:1},CAUSTIC:{motif:'CAUSTIC_CROSSFLOW',label:'FLUJO CÁUSTICO',bias:-1},ATLAS:{motif:'VEIL_PASSAGE',label:'PASO DEL VELO',bias:0},MIRAGE:{motif:'MIRROR_GAP',label:'BRECHA ESPEJO',bias:-1},PIT:{motif:'AMBUSH_CHANNEL',label:'CANAL DE EMBOSCADA',bias:1},AURICULUS:{motif:'PRISMATIC_RUNWAY',label:'PISTA PRISMÁTICA',bias:0},APIS:{motif:'ROYAL_CELL',label:'CELDA REAL',bias:0}
};
function hiveArenaPilotState(){const h=G?.hiveDirector;if(!h)return null;if(!h.arenaPilot)h.arenaPilot={plan:null,events:[],telegraphs:0,executed:0,deferred:0,lastReason:'init'};return h.arenaPilot;}
function hiveArenaEligible(h=G?.hiveDirector,b=G?.boss){if(!HIVE_DIRECTOR_CONFIG.arenaPilot||!h||!b||h.finished||h.finalizing||G?.mode!=='campaign'||G.sector<HIVE_DIRECTOR_CONFIG.arenaPilotWorld||b.phase<HIVE_DIRECTOR_CONFIG.arenaPilotPhase||!bossIdentity())return {ok:false,reason:'scope'};if(h.counterEvolution?.active)return {ok:false,reason:'counter_evolution_lock'};const hold=hiveFairnessHold(b);if(hold.active)return {ok:false,reason:hold.reasons.join('+')};if(h.state!=='A2'&&h.state!=='A3')return {ok:false,reason:'state'};if((h.confidence||0)<HIVE_DIRECTOR_CONFIG.arenaPilotConfidence)return {ok:false,reason:'director_confidence'};if(h.dpsProfile?.fingerprint==='BURST_SPIKE')return {ok:false,reason:'burst_guard'};if(b.signatureAttack||b.telegraphT>0||b.specialT>0||b.chargeT>0)return {ok:false,reason:'attack_lock'};const orch=hiveOrchestratorPermit('ARENA_MORPHOLOGY',{major:true});if(!orch.ok)return {ok:false,reason:`orchestrator_${orch.reason}`};const sp=hiveSpatialState();if(!sp||sp.read==='READING'||sp.confidence<.46)return {ok:false,reason:'spatial_confidence'};return {ok:true,reason:'eligible'};}
function hiveArenaLanePlan(b,p){const id=bossIdentity(),spec=id?HIVE_ARENA_IDENTITY[id.key]:null,sp=hiveSpatialState(),ce=hiveCounterEvolutionEffect(),lanes=[H*.20,H*.40,H*.60,H*.80],target=clamp(sp?.predictedY??p.y,70,H-70);let nearest=0,best=Infinity;for(let i=0;i<lanes.length;i++){const d=Math.abs(lanes[i]-target);if(d<best){best=d;nearest=i;}}let safe=nearest;if(spec&&sp?.confidence>.72&&sp.read!=='EVASIVE'&&sp.read!=='VERTICAL_WEAVE'){const combinedBias=clamp((spec.bias||0)+(ce?.arenaBias||0),-1,1),candidate=clamp(nearest+combinedBias,0,lanes.length-1);if(Math.abs(candidate-nearest)<=1)safe=candidate;}return {identity:id?.key||null,motif:spec?.motif||'READABLE_CORRIDOR',label:spec?.label||'CORREDOR ADAPTATIVO',lanes,safeIndex:safe,safeY:lanes[safe],spatial:sp?.read||'READING',spatialConfidence:+(sp?.confidence||0).toFixed(3),directorState:G?.hiveDirector?.state||'A1',directorConfidence:+(G?.hiveDirector?.confidence||0).toFixed(3),counterEvolution:ce?.id||null,telegraph:HIVE_DIRECTOR_CONFIG.arenaTelegraph,pulse:0};}
function hiveArenaYOutside(plan,index=0){const candidates=plan.lanes.filter((_,i)=>i!==plan.safeIndex);return candidates[index%candidates.length];}
function hiveArenaPrepare(b,p,sec){const st=hiveArenaPilotState(),plan=hiveArenaLanePlan(b,p);if(!hiveOrchestratorCommit('ARENA_MORPHOLOGY',{major:true,reason:'readable_arena_recomposition'})){st.lastReason='orchestrator_denied';st.deferred++;return null;}st.plan=plan;st.telegraphs++;st.lastReason='prepared';notify(`ARENA VIVA · ${plan.label} · CORREDOR MARCADO`,sec.accent,1.0);return plan;}
function hiveArenaTelegraph(plan,sec,dt){plan.pulse=(plan.pulse||0)-dt;if(plan.pulse<=0){plan.pulse=.16;const y=plan.safeY;G.particles.push({kind:'halo',x:W*.34,y,r:12,vr:155,life:.42,max:.42,col:sec.accent});G.particles.push({kind:'halo',x:W*.58,y,r:10,vr:145,life:.42,max:.42,col:sec.accent});G.particles.push({kind:'halo',x:W*.80,y,r:8,vr:135,life:.42,max:.42,col:sec.accent});}}
function hiveArenaExecute(b,p,sec,plan){
  const phase=b.phase,ys=plan.lanes.filter((_,i)=>i!==plan.safeIndex),fx=commanderProjectileFx(b,'arena',true);
  if(b.pattern==='storm'){for(const y of ys)for(let k=0;k<6;k++)spawnEnemyBullet(W-k*85,y,Math.PI,300+phase*25,8+phase*.6,4,'#ffe66f',b,fx);}
  else if(b.pattern==='titan'){for(let i=0;i<2+phase;i++){const x=W*.48+i*W*.11,y=hiveArenaYOutside(plan,i);G.particles.push({kind:'shock',x,y,r:10,vr:300,life:.65,max:.65,col:sec.accent});ringBullets(x,y,6+phase,125+phase*18,7+phase*.5,sec.accent,b.t+i*.25,b,'arena');}b.guardT=Math.max(b.guardT,.55);}
  else if(b.pattern==='blade'){for(let i=0;i<ys.length;i++){const y=ys[i],dir=i%2?1:-1;G.particles.push({kind:'slash',x:W*.55,y,a:dir>0?0:Math.PI,len:W*.86,life:.42,max:.42,col:'#ff9a55'});for(let k=0;k<5;k++)spawnEnemyBullet(dir>0?0:W,y+(k-2)*12,dir>0?0:Math.PI,340+phase*24,9,4,'#ff9a55',b,fx);}}
  else if(b.pattern==='moth'){for(let i=0;i<3;i++){const x=[W*.34,W*.56,W*.76][i],y=hiveArenaYOutside(plan,i);G.particles.push({kind:'halo',x,y,r:16,vr:190,life:1,max:1,col:'#e9b7ff'});ringBullets(x,y,7+phase,110+phase*18,7,'#e9b7ff',b.t,b,'arena');}}
  else if(b.pattern==='queen'){for(let i=0;i<1+phase;i++)spawnEnemy(pick(sec.forms.slice(0,Math.min(3,phase+1))),W*.72,hiveArenaYOutside(plan,i),'summon');for(const y of ys)for(let k=0;k<4;k++)spawnEnemyBullet(W-k*70,y,Math.PI,250+phase*18,8,5,'#ff746b',b,fx);}
  else if(b.pattern==='leap'){for(let i=0;i<3;i++){const x=[W*.38,W*.58,W*.78][i],y=hiveArenaYOutside(plan,i);G.particles.push({kind:'shock',x,y,r:12,vr:430,life:.68,max:.68,col:sec.accent});ringBullets(x,y,7+phase*2,155+phase*20,8,sec.accent,b.t,b,'arena');}}
  else if(b.pattern==='blood'){for(const y of ys)for(let k=-1;k<=1;k++)spawnEnemyBullet(W+10,y,Math.PI+k*.035,230+phase*22,8+phase, '#ff4965',b,fx);if(phase>=2&&G.obstacles.length<5){const old=G.sector;spawnObstacle();G.sector=old;}G.particles.push({kind:'halo',x:W*.58,y:plan.safeY,r:18,vr:220,life:.8,max:.8,col:'#ff6175'});}
  else if(b.pattern==='architect'){for(let i=0;i<Math.min(3,phase+1);i++){const hp=150+G.sector*28;G.obstacles.push({x:W*.55+i*W*.12,y:hiveArenaYOutside(plan,i),r:34,type:i===2?'resinnode':'resinwall',hp,maxHp:hp,vx:-28,vy:0,entry:'arena',rot:0,t:0,col:sec.base,contact:18+G.sector,assetIndex:0});}b.guardT=Math.max(b.guardT,.8+phase*.25);}
  else if(b.pattern==='odonata'){for(const y of ys){G.particles.push({kind:'slash',x:W*.52,y,a:0,len:W*.92,life:.48,max:.48,col:'#78ecff'});for(let k=0;k<7;k++)spawnEnemyBullet(W-k*95,y+rnd(-5,5),Math.PI,380+phase*28,8+phase*.6,4,'#9af7ff',b,fx);}}
  else if(b.pattern==='resonance'){for(let i=0;i<3;i++){const x=[W*.32,W*.48,W*.66][i],y=hiveArenaYOutside(plan,i);G.particles.push({kind:'shock',x,y,r:10,vr:360,life:.7,max:.7,col:'#d9a7ff'});ringBullets(x,y,8+phase*2,125+phase*18,7+phase*.7,'#d9a7ff',b.t,b,'arena');}if(phase===3&&G.enemies.length<5){const legacy=SECTORS[rndI(0,8)];spawnEnemy(legacy.forms[1],b.x-50,hiveArenaYOutside(plan,1),'summon');G.enemies[G.enemies.length-1].family=legacy.family;}}
  notify(`ARENA · ${plan.label}`,sec.accent,1.0);
  const st=hiveArenaPilotState();if(st){st.executed++;const ev={at:+Math.max(0,(G.elapsed||0)-(G.hiveDirector?.startedAt||0)).toFixed(2),phase:b.phase,world:G.sector,boss:plan.identity,motif:plan.motif,safeIndex:plan.safeIndex,safeY:+plan.safeY.toFixed(1),spatial:plan.spatial,directorState:plan.directorState};st.events.push(ev);if(st.events.length>HIVE_DIRECTOR_CONFIG.arenaMaxEvents)st.events.shift();}
}
function hiveArenaMorphologyUpdate(b,p,sec,dt,pressure){const st=hiveArenaPilotState();if(!st)return false;if(st.plan){st.plan.telegraph=Math.max(0,st.plan.telegraph-dt);hiveArenaTelegraph(st.plan,sec,dt);if(st.plan.telegraph<=0){const plan=st.plan;st.plan=null;hiveArenaExecute(b,p,sec,plan);b.arenaCd=Math.max(HIVE_DIRECTOR_CONFIG.arenaCooldownFloor,(6.2-b.phase*.72-G.sector*.12)*pressure.cool+.35);}return true;}if(b.arenaCd>0)return false;const el=hiveArenaEligible(G.hiveDirector,b);if(!el.ok){st.lastReason=el.reason;st.deferred++;return false;}return !!hiveArenaPrepare(b,p,sec);}
// v2.15.30 · TACTICAL MEMORY + COUNTER-EVOLUTION + MICROSEQUENCES + FEINT BRANCHING
// Memoria exclusivamente intra-combate. Prueba respuestas geométricas acotadas,
// mide si cambian la táctica repetida o reducen presión ofensiva y retira las que fallan.
// Nunca altera HP, daño, cadencia, ranks, powers ni persistencia de la build.
const HIVE_COUNTER_EVOLUTION_IDENTITY={
  KHEPRI:{responses:[{id:'GRAVITY_CROSSCUT',label:'CORTE GRAVITACIONAL',aimMul:1.08,yMul:1.08,xMul:1.10,spreadMul:1.03,ringOffset:.08,arenaBias:0},{id:'PERIAPSIS_SHIFT',label:'DESFASE DE PERIAPSIS',aimMul:.96,yMul:1.15,xMul:.94,spreadMul:1.06,ringOffset:.13,arenaBias:1},{id:'ORBITAL_FEINT',label:'AMAGO ORBITAL',aimMul:1.04,yMul:.90,xMul:1.14,spreadMul:.97,ringOffset:-.09,arenaBias:-1}]},
  VESPERA:{responses:[{id:'LANCER_CROSSVECTOR',label:'VECTOR CRUZADO',aimMul:1.12,yMul:1.05,xMul:1.12,spreadMul:.96,ringOffset:.02,arenaBias:1},{id:'NEEDLE_OFFSET',label:'AGUJA DESFASADA',aimMul:1.06,yMul:.92,xMul:1.08,spreadMul:.92,ringOffset:-.04,arenaBias:0},{id:'RUNWAY_REVERSAL',label:'INVERSIÓN DE CORREDOR',aimMul:.98,yMul:1.12,xMul:.94,spreadMul:1.04,ringOffset:.05,arenaBias:-1}]},
  REGENT:{responses:[{id:'MANDIBLE_CROSSLOCK',label:'CRUCE MANDIBULAR',aimMul:1.05,yMul:1.12,xMul:1.04,spreadMul:1.07,ringOffset:.03,arenaBias:-1},{id:'PINCER_DELAY',label:'PINZA DIFERIDA',aimMul:.94,yMul:1.16,xMul:.96,spreadMul:1.10,ringOffset:.07,arenaBias:0},{id:'REGENT_FEINT',label:'AMAGO REGENTE',aimMul:1.07,yMul:.91,xMul:1.10,spreadMul:.98,ringOffset:-.05,arenaBias:1}]},
  PYROLUX:{responses:[{id:'BEACON_PHASEBREAK',label:'RUPTURA DE FASE',aimMul:1.04,yMul:1.06,xMul:1.05,spreadMul:1.08,ringOffset:.12,arenaBias:1},{id:'FIGURE8_REVERSAL',label:'OCHO INVERSO',aimMul:.97,yMul:1.12,xMul:1.08,spreadMul:1.03,ringOffset:-.14,arenaBias:0},{id:'LUMEN_CROSS',label:'CRUCE LUMÍNICO',aimMul:1.09,yMul:.94,xMul:.96,spreadMul:.95,ringOffset:.08,arenaBias:-1}]},
  CAUSTIC:{responses:[{id:'ACID_CROSSFLOW',label:'CONTRAFLUJO ÁCIDO',aimMul:1.02,yMul:1.08,xMul:1.04,spreadMul:1.10,ringOffset:.08,arenaBias:-1},{id:'DISTILLATE_OFFSET',label:'DESTILADO DESFASADO',aimMul:.96,yMul:1.13,xMul:.92,spreadMul:1.06,ringOffset:-.10,arenaBias:0},{id:'CAUSTIC_FEINT',label:'AMAGO CÁUSTICO',aimMul:1.08,yMul:.92,xMul:1.08,spreadMul:.97,ringOffset:.05,arenaBias:1}]},
  ATLAS:{responses:[{id:'VEIL_CROSSING',label:'CRUCE DEL VELO',aimMul:1.05,yMul:1.10,xMul:1.06,spreadMul:.96,ringOffset:.12,arenaBias:0},{id:'ATLAS_PARALLAX',label:'PARALAJE ATLAS',aimMul:.98,yMul:.94,xMul:1.12,spreadMul:1.05,ringOffset:-.11,arenaBias:1},{id:'VEIL_REVERSAL',label:'VELO INVERSO',aimMul:1.07,yMul:1.04,xMul:.94,spreadMul:1.02,ringOffset:.07,arenaBias:-1}]},
  MIRAGE:{responses:[{id:'MIRROR_COUNTERPOSE',label:'CONTRAPOSICIÓN ESPEJO',aimMul:1.10,yMul:1.08,xMul:1.10,spreadMul:.94,ringOffset:.10,arenaBias:-1},{id:'PHASMID_PARALLAX',label:'PARALAJE FÁSMIDO',aimMul:1.03,yMul:.92,xMul:1.14,spreadMul:.98,ringOffset:-.12,arenaBias:1},{id:'FALSE_GAP',label:'BRECHA FALSA',aimMul:.96,yMul:1.14,xMul:.95,spreadMul:1.08,ringOffset:.04,arenaBias:0}]},
  PIT:{responses:[{id:'AMBUSH_CROSSCUT',label:'CORTE DE EMBOSCADA',aimMul:1.08,yMul:1.12,xMul:1.04,spreadMul:1.05,ringOffset:.05,arenaBias:1},{id:'PIT_DELAY',label:'EMBOSCADA DIFERIDA',aimMul:.95,yMul:1.16,xMul:.92,spreadMul:1.09,ringOffset:-.08,arenaBias:0},{id:'CHANNEL_REVERSAL',label:'CANAL INVERSO',aimMul:1.05,yMul:.92,xMul:1.10,spreadMul:.97,ringOffset:.09,arenaBias:-1}]},
  AURICULUS:{responses:[{id:'PRISMATIC_LEAD',label:'ADELANTO PRISMÁTICO',aimMul:1.11,yMul:1.04,xMul:1.08,spreadMul:.94,ringOffset:.06,arenaBias:0},{id:'TAIL_CROSS',label:'CRUCE DE COLA',aimMul:1.06,yMul:1.12,xMul:.96,spreadMul:.98,ringOffset:-.07,arenaBias:1},{id:'AURICULAR_FEINT',label:'AMAGO AURICULAR',aimMul:.97,yMul:.93,xMul:1.12,spreadMul:1.06,ringOffset:.09,arenaBias:-1}]},
  APIS:{responses:[{id:'ROYAL_CROSSPRESSURE',label:'PRESIÓN REAL CRUZADA',aimMul:1.06,yMul:1.08,xMul:1.04,spreadMul:1.07,ringOffset:.08,arenaBias:0},{id:'CELL_ROTATION',label:'ROTACIÓN DE CELDA',aimMul:.98,yMul:1.13,xMul:.94,spreadMul:1.04,ringOffset:-.10,arenaBias:1},{id:'IMPERIAL_FEINT',label:'AMAGO IMPERIAL',aimMul:1.08,yMul:.92,xMul:1.10,spreadMul:.98,ringOffset:.06,arenaBias:-1}]}
 };
// v2.15.30 · COEVOLUTION MICROSEQUENCES + LEGIBLE FEINT BRANCHES
// Una respuesta aprendida puede convertirse en una secuencia de tres actos. Cada acto
// modula únicamente la geometría ya autorizada; no añade proyectiles, daño, HP ni cadencia.
// Si el jugador cambia de táctica o recupera presión de forma sostenida, la secuencia muta
// una sola vez hacia un cierre alternativo y después se abandona si vuelve a ser descifrada.
// v2.15.30: durante la apertura, una secuencia aprendida puede revelar DOS continuaciones
// plausibles. Ambas se telegraphan; tras una única lectura de reacción se fija una rama.
// Después del commit no hay seguimiento correctivo: la decisión queda congelada.
const HIVE_MICRO_SEQUENCE_IDENTITY={
  KHEPRI:{id:'ORBITAL_TRIAD',label:'TRÍADA ORBITAL',steps:[{id:'OPEN',label:'APERTURA DE ÓRBITA',duration:1.25,aim:.97,y:.96,x:1.03,spread:1.03,ring:-.04,arena:0},{id:'PRESS',label:'PERIAPSIS',duration:1.75,aim:1.04,y:1.04,x:1.02,spread:.98,ring:.06,arena:0},{id:'CLOSE',label:'SALIDA GRAVITACIONAL',duration:1.35,aim:.99,y:.97,x:.98,spread:1.02,ring:-.03,arena:-1}]},
  VESPERA:{id:'LANCER_CADENCE',label:'CADENCIA DE LANZA',steps:[{id:'OPEN',label:'MARCA VECTORIAL',duration:1.20,aim:1.02,y:.97,x:1.03,spread:1.02,ring:0,arena:0},{id:'PRESS',label:'INTERCEPCIÓN',duration:1.80,aim:1.04,y:1.02,x:1.04,spread:.97,ring:.02,arena:1},{id:'CLOSE',label:'SALIDA DE AGUJA',duration:1.35,aim:.98,y:1.02,x:.98,spread:1.01,ring:-.03,arena:0}]},
  REGENT:{id:'MANDIBLE_TRIPTYCH',label:'TRÍPTICO MANDIBULAR',steps:[{id:'OPEN',label:'APERTURA DE PINZA',duration:1.30,aim:.98,y:.96,x:1.02,spread:1.04,ring:0,arena:0},{id:'PRESS',label:'CRUCE DE MANDÍBULAS',duration:1.70,aim:1.03,y:1.04,x:1.01,spread:1.03,ring:.03,arena:-1},{id:'CLOSE',label:'LIBERACIÓN',duration:1.35,aim:.98,y:.98,x:.97,spread:.99,ring:-.02,arena:0}]},
  PYROLUX:{id:'BEACON_PULSE_CHAIN',label:'CADENA DE PULSOS',steps:[{id:'OPEN',label:'CEBO BIOLUMÍNICO',duration:1.20,aim:.98,y:1.01,x:.99,spread:1.03,ring:.04,arena:0},{id:'PRESS',label:'PULSO CRUZADO',duration:1.85,aim:1.03,y:1.03,x:1.02,spread:.98,ring:.06,arena:1},{id:'CLOSE',label:'ECO DE FARO',duration:1.30,aim:.99,y:.98,x:1.01,spread:1.01,ring:-.05,arena:0}]},
  CAUSTIC:{id:'DISTILLATION_CYCLE',label:'CICLO DE DESTILACIÓN',steps:[{id:'OPEN',label:'CEBO CÁUSTICO',duration:1.30,aim:.97,y:1.02,x:.99,spread:1.04,ring:.02,arena:0},{id:'PRESS',label:'CONTRAFLUJO',duration:1.70,aim:1.02,y:1.04,x:1.03,spread:1.02,ring:.05,arena:-1},{id:'CLOSE',label:'PURGA',duration:1.35,aim:.98,y:.97,x:.98,spread:.98,ring:-.04,arena:0}]},
  ATLAS:{id:'VEIL_PARALLAX_CHAIN',label:'CADENA DEL VELO',steps:[{id:'OPEN',label:'DESFASE DEL VELO',duration:1.25,aim:.98,y:.97,x:1.03,spread:.99,ring:.04,arena:0},{id:'PRESS',label:'PARALAJE',duration:1.75,aim:1.03,y:1.03,x:1.04,spread:1.01,ring:.05,arena:1},{id:'CLOSE',label:'CIERRE DEL VELO',duration:1.35,aim:.98,y:1.01,x:.97,spread:1.02,ring:-.04,arena:0}]},
  MIRAGE:{id:'PHASMID_DECEPTION_CHAIN',label:'CADENA DE ENGAÑO',steps:[{id:'OPEN',label:'FALSA LECTURA',duration:1.20,aim:.97,y:.98,x:1.03,spread:1.03,ring:-.04,arena:0},{id:'PRESS',label:'CONTRAPOSICIÓN',duration:1.80,aim:1.04,y:1.03,x:1.04,spread:.97,ring:.06,arena:-1},{id:'CLOSE',label:'BRECHA REAL',duration:1.35,aim:.99,y:.98,x:.98,spread:1.02,ring:.02,arena:0}]},
  PIT:{id:'AMBUSH_THREE_BEAT',label:'EMBOSCADA EN TRES TIEMPOS',steps:[{id:'OPEN',label:'SEÑUELO',duration:1.25,aim:.98,y:.97,x:1.01,spread:1.03,ring:0,arena:0},{id:'PRESS',label:'CIERRE DE CANAL',duration:1.75,aim:1.03,y:1.04,x:1.03,spread:1.01,ring:.04,arena:1},{id:'CLOSE',label:'RETIRADA DE FOSA',duration:1.35,aim:.98,y:.98,x:.98,spread:.98,ring:-.03,arena:0}]},
  AURICULUS:{id:'PRISMATIC_TAIL_CHAIN',label:'CADENA PRISMÁTICA',steps:[{id:'OPEN',label:'DESTELLO DE COLA',duration:1.20,aim:1.01,y:.98,x:1.02,spread:1.01,ring:.03,arena:0},{id:'PRESS',label:'PINZA PRISMÁTICA',duration:1.80,aim:1.04,y:1.03,x:1.03,spread:.97,ring:.04,arena:1},{id:'CLOSE',label:'DESCARGA LATERAL',duration:1.35,aim:.98,y:1.01,x:.98,spread:1.03,ring:-.04,arena:0}]},
  APIS:{id:'ROYAL_COMMAND_CHAIN',label:'CADENA DE MANDO REAL',steps:[{id:'OPEN',label:'ORDEN DE CELDA',duration:1.30,aim:.98,y:.98,x:1.01,spread:1.03,ring:.02,arena:0},{id:'PRESS',label:'PRESIÓN IMPERIAL',duration:1.70,aim:1.03,y:1.04,x:1.02,spread:1.02,ring:.05,arena:0},{id:'CLOSE',label:'REPLIEGUE REAL',duration:1.35,aim:.99,y:.97,x:.98,spread:.99,ring:-.03,arena:-1}]}
};
function hiveMicroSequenceSpec(identity=bossIdentity()){return identity?HIVE_MICRO_SEQUENCE_IDENTITY[identity.key]||null:null;}
function hiveMicroSequenceDuration(spec){return (spec?.steps||[]).reduce((a,x)=>a+Math.max(.1,Number(x.duration)||0),0);}
function hiveBranchFeintEligible(st,h,b,response,seqSpec){if(!HIVE_DIRECTOR_CONFIG.branchFeint||!st||!h||!b||!response?.learned||!seqSpec)return false;if(G?.sector<HIVE_DIRECTOR_CONFIG.branchFeintWorld||b.phase<HIVE_DIRECTOR_CONFIG.branchFeintPhase||(h.confidence||0)<HIVE_DIRECTOR_CONFIG.branchFeintConfidence)return false;if((st.branchPhaseUse?.[b.phase]||0)>=HIVE_DIRECTOR_CONFIG.branchFeintMaxPerPhase)return false;return hiveCounterEvolutionResponses(bossIdentity()).some(r=>r.id!==response.id&&!(st.retired?.[st.currentTactic]||{})[r.id]);}
function hiveBranchFeintPlan(st,h,b,age,tactic,response,seqSpec){if(!hiveBranchFeintEligible(st,h,b,response,seqSpec))return null;const alt=hiveMicroSequenceAlternate(st,tactic,response.id);if(!alt)return null;const sp=hiveSpatialState(),baseY=clamp(sp?.predictedY??G?.player?.y??H*.5,70,H-70),off=H*HIVE_DIRECTOR_CONFIG.branchLaneOffset,openDur=Math.max(.5,Number(seqSpec.steps?.[0]?.duration)||1.2);return {state:'WAIT',revealAt:age+Math.max(.48,openDur*.52),telegraphUntil:null,effectAt:null,startedAt:null,baselineY:baseY,baselineTactic:hiveCounterEvolutionTactic(h.dpsProfile),candidateA:{...response},candidateB:{...alt},previewYA:clamp(baseY-off,70,H-70),previewYB:clamp(baseY+off,70,H-70),committed:null,committedPath:null,reaction:null,pulse:0};}
function hiveBranchFeintVisual(branch,dt){if(!branch||!G?.particles)return;branch.pulse=(branch.pulse||0)-Math.max(0,dt||0);if(branch.pulse>0)return;branch.pulse=HIVE_DIRECTOR_CONFIG.branchPulseSec;const sec=SECTORS?.[(G?.sector||1)-1],col=sec?.accent||'#b8f4ff',x1=W*.61,x2=W*.82;if(branch.state==='TELEGRAPH'){for(const y of [branch.previewYA,branch.previewYB]){G.particles.push({kind:'halo',x:x1,y,r:8,vr:120,life:.34,max:.34,col});G.particles.push({kind:'halo',x:x2,y,r:7,vr:105,life:.34,max:.34,col});}}else if(branch.state==='COMMIT'&&branch.committed){const y=branch.committedPath==='A'?branch.previewYA:branch.previewYB;G.particles.push({kind:'halo',x:W*.70,y,r:10,vr:150,life:.34,max:.34,col});G.particles.push({kind:'halo',x:W*.88,y,r:8,vr:130,life:.34,max:.34,col});}}
function hiveBranchFeintUpdate(dt){const h=G?.hiveDirector,st=h?.counterEvolution,a=st?.active,q=a?.sequence,b=G?.boss,sp=hiveSpatialState();if(!h||!st||!a||a.mode!=='MICRO_SEQUENCE'||!q?.branch||!b)return;const br=q.branch,age=Math.max(0,(G.elapsed||0)-h.startedAt);if(br.state==='WAIT'&&age>=br.revealAt){const op=hiveOrchestratorPermit('FEINT_BRANCH',{major:false,ignoreQuiet:true});if(!op.ok){br.state='CANCELLED';st.lastReason=`branch_${op.reason}`;hiveOrchestratorDeny('FEINT_BRANCH',op.reason);return;}if(!hiveOrchestratorCommit('FEINT_BRANCH',{major:false,reason:'legible_dual_intent',ignoreQuiet:true})){br.state='CANCELLED';st.lastReason='branch_orchestrator_denied';return;}br.state='TELEGRAPH';br.startedAt=age;br.telegraphUntil=age+HIVE_DIRECTOR_CONFIG.branchTelegraphSec;br.baselineY=clamp(sp?.predictedY??G?.player?.y??br.baselineY,70,H-70);const off=H*HIVE_DIRECTOR_CONFIG.branchLaneOffset;br.previewYA=clamp(br.baselineY-off,70,H-70);br.previewYB=clamp(br.baselineY+off,70,H-70);st.branchPhaseUse[b.phase]=(st.branchPhaseUse[b.phase]||0)+1;st.branchesStarted++;st.events.push({type:'BRANCH_REVEAL',at:+age.toFixed(2),phase:b.phase,tactic:a.tactic,sequence:q.spec.id,a:br.candidateA.id,b:br.candidateB.id,baselineY:+br.baselineY.toFixed(1),previewYA:+br.previewYA.toFixed(1),previewYB:+br.previewYB.toFixed(1)});if(st.events.length>HIVE_DIRECTOR_CONFIG.counterEvolutionMaxEvents)st.events.shift();notify(`BIFURCACIÓN · ${br.candidateA.label} / ${br.candidateB.label}`,'#b8f4ff',.95);}if(br.state==='TELEGRAPH'){hiveBranchFeintVisual(br,dt);if(age<br.telegraphUntil)return;const currentY=clamp(sp?.predictedY??G?.player?.y??br.baselineY,70,H-70),delta=(currentY-br.baselineY)/Math.max(1,H),currentTactic=hiveCounterEvolutionTactic(h.dpsProfile);let reaction='HOLD',path='A';if(delta<=-HIVE_DIRECTOR_CONFIG.branchReactionRatio){reaction='UP';path='B';}else if(delta>=HIVE_DIRECTOR_CONFIG.branchReactionRatio){reaction='DOWN';path='A';}else if(currentTactic!=='READING'&&currentTactic!==br.baselineTactic){reaction='TACTIC_SHIFT';path=(b.phase+G.sector)%2?'A':'B';}else path=(sp?.dominant==='top'||sp?.dominant==='upper')?'B':(sp?.dominant==='bottom'||sp?.dominant==='lower')?'A':((G.sector+b.phase)%2?'A':'B');br.reaction=reaction;br.committedPath=path;br.committed=path==='A'?{...br.candidateA}:{...br.candidateB};br.state='COMMIT';br.effectAt=age+HIVE_DIRECTOR_CONFIG.branchCommitDelay;st.branchesCommitted++;st.events.push({type:'BRANCH_COMMIT',at:+age.toFixed(2),phase:b.phase,tactic:a.tactic,sequence:q.spec.id,reaction,path,response:br.committed.id,currentY:+currentY.toFixed(1),effectAt:+br.effectAt.toFixed(2)});if(st.events.length>HIVE_DIRECTOR_CONFIG.counterEvolutionMaxEvents)st.events.shift();st.lastReason='branch_committed';notify(`RAMA ${path} · ${br.committed.label}`,SECTORS?.[G.sector-1]?.accent||'#b8f4ff',.72);}else if(br.state==='COMMIT'&&age<br.effectAt)hiveBranchFeintVisual(br,dt);}
function hiveBranchFeintEffect(active,age=Math.max(0,(G?.elapsed||0)-(G?.hiveDirector?.startedAt||0))){const br=active?.sequence?.branch;if(!br||br.state!=='COMMIT'||!br.committed||age<(br.effectAt||Infinity))return null;return {...br.committed,branchPath:br.committedPath,branchReaction:br.reaction};}
function hiveMicroSequenceStage(active,age=Math.max(0,(G?.elapsed||0)-(G?.hiveDirector?.startedAt||0))){const q=active?.sequence;if(!q)return null;const spec=q.spec;if(!spec?.steps?.length)return null;if(q.forceClosure){const step=spec.steps[spec.steps.length-1];return {...step,index:spec.steps.length-1,elapsed:Math.max(0,age-(q.mutatedAt||age)),sequenceId:spec.id};}let t=Math.max(0,age-active.startedAt),acc=0;for(let i=0;i<spec.steps.length;i++){const step=spec.steps[i],dur=Math.max(.1,Number(step.duration)||0);if(t<acc+dur||i===spec.steps.length-1)return {...step,index:i,elapsed:Math.max(0,t-acc),sequenceId:spec.id};acc+=dur;}return null;}
function hiveMicroSequenceEligible(st,h,b,tactic,response){if(!HIVE_DIRECTOR_CONFIG.microSequence||!response?.learned||!st||!h||!b)return false;if(G?.sector<HIVE_DIRECTOR_CONFIG.microSequenceWorld||b.phase<HIVE_DIRECTOR_CONFIG.microSequencePhase||(h.confidence||0)<HIVE_DIRECTOR_CONFIG.microSequenceConfidence)return false;if((st.sequencePhaseUse?.[b.phase]||0)>=HIVE_DIRECTOR_CONFIG.microSequenceMaxPerPhase||st.sequenceRetired?.[tactic])return false;if(h.arenaPilot?.plan||b.signatureAttack||b.telegraphT>0||b.specialT>0||b.chargeT>0)return false;return !!hiveMicroSequenceSpec(bossIdentity());}
function hiveMicroSequenceAlternate(st,tactic,currentId){const list=hiveCounterEvolutionResponses(bossIdentity()),retired=st?.retired?.[tactic]||{};return list.find(r=>r.id!==currentId&&!retired[r.id])||null;}
function hiveMicroSequenceObserve(st,h,b,age,profile){const a=st?.active,q=a?.sequence;if(!a||a.mode!=='MICRO_SEQUENCE'||!q)return false;const stage=hiveMicroSequenceStage(a,age);if(stage&&stage.index!==q.lastStageIndex){q.lastStageIndex=stage.index;st.events.push({type:'SEQUENCE_STEP',at:+age.toFixed(2),phase:b.phase,tactic:a.tactic,response:a.response.id,sequence:q.spec.id,step:stage.id,label:stage.label});if(st.events.length>HIVE_DIRECTOR_CONFIG.counterEvolutionMaxEvents)st.events.shift();}const br=q.branch;if(br&&(br.state==='TELEGRAPH'||(br.state==='COMMIT'&&age<(br.effectAt||0)+HIVE_DIRECTOR_CONFIG.branchDecodeGrace)))return false;const tactic=hiveCounterEvolutionTactic(profile),dps=Math.max(0,hiveRollingDps(3,true)),changed=tactic!=='READING'&&tactic!==a.tactic,recovered=dps>=a.baselineDps*HIVE_DIRECTOR_CONFIG.microSequencePressureRecovery;if(changed||recovered)q.decodeSamples=(q.decodeSamples||0)+1;else q.decodeSamples=Math.max(0,(q.decodeSamples||0)-1);if(q.decodeSamples<HIVE_DIRECTOR_CONFIG.microSequenceDecodeSamples)return false;if(!q.mutated){const alt=hiveMicroSequenceAlternate(st,a.tactic,a.response.id);if(alt){q.mutated=true;q.mutationResponse={...alt};q.forceClosure=true;q.mutatedAt=age;q.decodeSamples=0;a.endsAt=Math.min(a.endsAt,age+HIVE_DIRECTOR_CONFIG.microSequenceMutationSec);st.sequencesMutated++;st.events.push({type:'SEQUENCE_MUTATE',at:+age.toFixed(2),phase:b.phase,tactic:a.tactic,sequence:q.spec.id,from:a.response.id,to:alt.id,reason:changed?'tactic_changed':'pressure_recovered'});if(st.events.length>HIVE_DIRECTOR_CONFIG.counterEvolutionMaxEvents)st.events.shift();st.lastReason='sequence_mutated';return false;}}st.sequenceRetired[a.tactic]=true;st.sequencesAbandoned++;st.events.push({type:'SEQUENCE_ABANDON',at:+age.toFixed(2),phase:b.phase,tactic:a.tactic,sequence:q.spec.id,response:a.response.id,reason:changed?'decoded_tactic':'decoded_pressure'});if(st.events.length>HIVE_DIRECTOR_CONFIG.counterEvolutionMaxEvents)st.events.shift();st.active=null;st.cooldownUntil=age+HIVE_DIRECTOR_CONFIG.microSequenceCooldown;st.lastReason='sequence_abandoned';return true;}
function hiveCounterEvolutionState(){const h=G?.hiveDirector;if(!h)return null;if(!h.counterEvolution)h.counterEvolution={currentTactic:'READING',streak:0,active:null,learned:{},retired:{},reuseCount:{},phaseTrials:{},sequencePhaseUse:{},sequenceRetired:{},branchPhaseUse:{},events:[],successes:0,failures:0,aborted:0,sequencesStarted:0,sequencesCompleted:0,sequencesMutated:0,sequencesAbandoned:0,branchesStarted:0,branchesCommitted:0,cooldownUntil:0,lastReason:'init'};return h.counterEvolution;}
function hiveCounterEvolutionTactic(profile=G?.hiveDirector?.dpsProfile){const sp=hiveSpatialState();if(!sp||sp.read==='READING')return 'READING';if(profile?.fingerprint==='SUSTAINED_DOMINATION'&&sp.read==='MOBILE_MIX')return 'SUSTAINED_PRESSURE';return sp.read;}
function hiveCounterEvolutionResponses(identity=bossIdentity()){return identity?HIVE_COUNTER_EVOLUTION_IDENTITY[identity.key]?.responses||[]:[];}
function hiveCounterEvolutionPickResponse(st,tactic,identity=bossIdentity()){const list=hiveCounterEvolutionResponses(identity);if(!list.length)return null;const learnedId=st.learned[tactic],reuse=Number(st.reuseCount[tactic]||0);if(learnedId&&reuse<HIVE_DIRECTOR_CONFIG.counterEvolutionMaxReuses){const learned=list.find(r=>r.id===learnedId);if(learned)return {...learned,learned:true};}const retired=st.retired[tactic]||{};const offset={EDGE_ANCHOR:0,VERTICAL_WEAVE:1,CENTER_HOLD:2,LANE_ANCHOR:0,EVASIVE:1,MOBILE_MIX:2,SUSTAINED_PRESSURE:0}[tactic]||0;for(let i=0;i<list.length;i++){const r=list[(i+offset)%list.length];if(!retired[r.id])return {...r,learned:false};}return null;}
function hiveCounterEvolutionEffect(){const st=hiveCounterEvolutionState(),a=st?.active;if(!a||!a.response)return null;if(a.mode!=='MICRO_SEQUENCE'||!a.sequence)return {...a.response,mode:a.mode,targetTactic:a.tactic};const stage=hiveMicroSequenceStage(a),branch=hiveBranchFeintEffect(a),base=a.sequence.mutationResponse&&a.sequence.forceClosure?a.sequence.mutationResponse:(branch||a.response);if(!stage)return {...base,mode:a.mode,targetTactic:a.tactic,microSequence:a.sequence.spec.id,branchPath:branch?.branchPath||null,branchReaction:branch?.branchReaction||null};return {...base,aimMul:clamp((base.aimMul||1)*(stage.aim||1),.88,1.18),yMul:clamp((base.yMul||1)*(stage.y||1),.86,1.20),xMul:clamp((base.xMul||1)*(stage.x||1),.86,1.20),spreadMul:clamp((base.spreadMul||1)*(stage.spread||1),.86,1.18),ringOffset:clamp((base.ringOffset||0)+(stage.ring||0),-.22,.22),arenaBias:clamp((base.arenaBias||0)+(stage.arena||0),-1,1),mode:a.mode,targetTactic:a.tactic,microSequence:a.sequence.spec.id,sequenceStage:stage.id,sequenceStageLabel:stage.label,sequenceMutated:!!a.sequence.mutated,branchPath:branch?.branchPath||null,branchReaction:branch?.branchReaction||null};}
function hiveCounterEvolutionAbort(st,age,reason){if(!st?.active)return;st.events.push({type:'ABORT',at:+age.toFixed(2),phase:G?.boss?.phase||0,tactic:st.active.tactic,response:st.active.response?.id||null,reason});if(st.events.length>HIVE_DIRECTOR_CONFIG.counterEvolutionMaxEvents)st.events.shift();st.active=null;st.aborted++;st.cooldownUntil=age+2.5;st.lastReason=reason;}
function hiveCounterEvolutionStart(st,h,b,age,tactic,response){const phase=b.phase,useSequence=hiveMicroSequenceEligible(st,h,b,tactic,response),seqSpec=useSequence?hiveMicroSequenceSpec(bossIdentity()):null,mode=useSequence?'MICRO_SEQUENCE':(response.learned?'LEARNED_REUSE':'TRIAL'),duration=useSequence?hiveMicroSequenceDuration(seqSpec):(response.learned?HIVE_DIRECTOR_CONFIG.counterEvolutionLearnedSec:HIVE_DIRECTOR_CONFIG.counterEvolutionTrialSec),orchType=useSequence?'MICRO_SEQUENCE':(response.learned?'LEARNED_REUSE':'COUNTER_TRIAL'),major=useSequence,permit=hiveOrchestratorPermit(orchType,{major});if(!permit.ok){st.lastReason=`orchestrator_${permit.reason}`;hiveOrchestratorDeny(orchType,permit.reason);return false;}if(!hiveOrchestratorCommit(orchType,{major,reason:mode.toLowerCase(),reserveFor:duration})){st.lastReason='orchestrator_denied';return false;}const tp=hiveTacticalPilotState();if(tp?.active)tp.segmentT=Math.max(tp.segmentT||0,duration+.25);st.active={mode,tactic,response:{...response},phase,startedAt:age,endsAt:age+duration,baselineDps:Math.max(.001,hiveRollingDps(3,true)||h.dpsProfile?.tactical||h.effectiveDps||.001),baselinePlayerDamage:h.damageToPlayer||0,baselineSpatial:hiveSpatialSnapshot(),sequence:useSequence?{spec:seqSpec,decodeSamples:0,lastStageIndex:-1,mutated:false,mutationResponse:null,forceClosure:false,mutatedAt:null,branch:null}:null};if(useSequence)st.active.sequence.branch=hiveBranchFeintPlan(st,h,b,age,tactic,response,seqSpec);if(response.learned){st.reuseCount[tactic]=(st.reuseCount[tactic]||0)+1;if(useSequence){st.sequencePhaseUse[phase]=(st.sequencePhaseUse[phase]||0)+1;st.sequencesStarted++;}}else st.phaseTrials[phase]=(st.phaseTrials[phase]||0)+1;st.events.push({type:'START',mode,at:+age.toFixed(2),phase,tactic,response:response.id,label:response.label,sequence:seqSpec?.id||null,sequenceLabel:seqSpec?.label||null,baselineDps:+st.active.baselineDps.toFixed(3)});if(st.events.length>HIVE_DIRECTOR_CONFIG.counterEvolutionMaxEvents)st.events.shift();st.lastReason=useSequence?'micro_sequence_active':'active';notify(`${useSequence?'COEVOLUCIÓN':response.learned?'MEMORIA TÁCTICA':'CONTRAEVOLUCIÓN'} · ${useSequence?seqSpec.label:response.label}`,'#b8f4ff',useSequence?1.05:.9);return true;}
function hiveCounterEvolutionFinish(st,h,b,age,profile){const a=st?.active;if(!a)return;const currentTactic=hiveCounterEvolutionTactic(profile),currentDps=Math.max(0,hiveRollingDps(3,true)),dpsDrop=a.baselineDps>0?clamp(1-currentDps/a.baselineDps,-1,1):0,tacticChanged=currentTactic!=='READING'&&currentTactic!==a.tactic,playerDamageDelta=Math.max(0,(h.damageToPlayer||0)-a.baselinePlayerDamage);if(a.mode==='MICRO_SEQUENCE'){st.sequencesCompleted++;st.events.push({type:'SEQUENCE_END',outcome:'COMPLETED',at:+age.toFixed(2),phase:b.phase,tactic:a.tactic,response:a.response.id,sequence:a.sequence?.spec?.id||null,mutated:!!a.sequence?.mutated,currentTactic,dpsDrop:+dpsDrop.toFixed(3)});if(st.events.length>HIVE_DIRECTOR_CONFIG.counterEvolutionMaxEvents)st.events.shift();st.active=null;st.cooldownUntil=age+HIVE_DIRECTOR_CONFIG.microSequenceCooldown;st.lastReason='micro_sequence_complete';return;}if(a.mode==='LEARNED_REUSE'){st.events.push({type:'REUSE_END',at:+age.toFixed(2),phase:b.phase,tactic:a.tactic,response:a.response.id,currentTactic,dpsDrop:+dpsDrop.toFixed(3)});if(st.events.length>HIVE_DIRECTOR_CONFIG.counterEvolutionMaxEvents)st.events.shift();st.active=null;st.cooldownUntil=age+HIVE_DIRECTOR_CONFIG.counterEvolutionCooldown;st.lastReason='learned_reuse_complete';return;}const success=tacticChanged||dpsDrop>=HIVE_DIRECTOR_CONFIG.counterEvolutionSuccessDpsDrop;if(success){st.learned[a.tactic]=a.response.id;st.successes++;}else{st.retired[a.tactic]=st.retired[a.tactic]||{};st.retired[a.tactic][a.response.id]=true;st.failures++;}st.events.push({type:'RESULT',outcome:success?'LEARNED':'RETIRED',at:+age.toFixed(2),phase:b.phase,tactic:a.tactic,response:a.response.id,currentTactic,tacticChanged,dpsDrop:+dpsDrop.toFixed(3),playerDamageDelta:+playerDamageDelta.toFixed(2)});if(st.events.length>HIVE_DIRECTOR_CONFIG.counterEvolutionMaxEvents)st.events.shift();st.active=null;st.cooldownUntil=age+HIVE_DIRECTOR_CONFIG.counterEvolutionCooldown;st.lastReason=success?'learned':'retired';}
function hiveCounterEvolutionSample(h,b,age,profile,hold){const st=hiveCounterEvolutionState();if(!st||!HIVE_DIRECTOR_CONFIG.counterEvolution)return;if(st.active){if(hold.active||b.phase!==st.active.phase||b.signatureAttack||b.telegraphT>0||b.specialT>0||b.chargeT>0){hiveCounterEvolutionAbort(st,age,hold.active?hold.reasons.join('+'):'attack_or_phase_lock');return;}if(st.active.mode==='MICRO_SEQUENCE'&&hiveMicroSequenceObserve(st,h,b,age,profile))return;if(st.active&&age>=st.active.endsAt)hiveCounterEvolutionFinish(st,h,b,age,profile);return;}const tactic=hiveCounterEvolutionTactic(profile);if(tactic===st.currentTactic)st.streak++;else{st.currentTactic=tactic;st.streak=1;}if(G?.mode!=='campaign'||G.sector<HIVE_DIRECTOR_CONFIG.counterEvolutionWorld||b.phase<HIVE_DIRECTOR_CONFIG.counterEvolutionPhase){st.lastReason='scope';return;}if(hold.active){st.lastReason=hold.reasons.join('+');return;}if(h.state!=='A2'&&h.state!=='A3'){st.lastReason='state';return;}if((h.confidence||0)<HIVE_DIRECTOR_CONFIG.counterEvolutionConfidence){st.lastReason='director_confidence';return;}const sp=hiveSpatialState();if(!sp||sp.read==='READING'||sp.confidence<.50){st.lastReason='spatial_confidence';return;}if(profile?.fingerprint==='BURST_SPIKE'){st.lastReason='burst_guard';return;}const tp=hiveTacticalPilotState();if(!tp?.active||(tp.intensity||0)<.25){st.lastReason='tactical_neutral';return;}if(b.signatureAttack||b.telegraphT>0||b.specialT>0||b.chargeT>0){st.lastReason='attack_lock';return;}if(age<st.cooldownUntil){st.lastReason='cooldown';return;}if(st.streak<HIVE_DIRECTOR_CONFIG.counterEvolutionRepeatSamples){st.lastReason='observing_repeat';return;}const phase=b.phase;if((st.phaseTrials[phase]||0)>=HIVE_DIRECTOR_CONFIG.counterEvolutionMaxTrialsPerPhase&&!st.learned[tactic]){st.lastReason='phase_trial_budget';return;}const response=hiveCounterEvolutionPickResponse(st,tactic,bossIdentity());if(!response){st.lastReason='responses_exhausted';return;}if(hiveCounterEvolutionStart(st,h,b,age,tactic,response))st.streak=0;}
function hiveCounterEvolutionSnapshot(){const st=hiveCounterEvolutionState();if(!st)return null;const stage=st.active?.mode==='MICRO_SEQUENCE'?hiveMicroSequenceStage(st.active):null;return {currentTactic:st.currentTactic,streak:st.streak,active:st.active?{mode:st.active.mode,tactic:st.active.tactic,response:st.active.response?.id||null,label:st.active.response?.label||null,phase:st.active.phase,endsAt:+st.active.endsAt.toFixed(2),sequence:st.active.sequence?.spec?.id||null,sequenceStage:stage?.id||null,sequenceMutated:!!st.active.sequence?.mutated,branch:st.active.sequence?.branch?{state:st.active.sequence.branch.state,path:st.active.sequence.branch.committedPath||null,reaction:st.active.sequence.branch.reaction||null,a:st.active.sequence.branch.candidateA?.id||null,b:st.active.sequence.branch.candidateB?.id||null}:null}:null,learned:{...st.learned},retired:Object.fromEntries(Object.entries(st.retired).map(([k,v])=>[k,Object.keys(v||{})])),reuseCount:{...st.reuseCount},phaseTrials:{...st.phaseTrials},sequencePhaseUse:{...st.sequencePhaseUse},sequenceRetired:{...st.sequenceRetired},branchPhaseUse:{...st.branchPhaseUse},branchesStarted:st.branchesStarted,branchesCommitted:st.branchesCommitted,successes:st.successes,failures:st.failures,aborted:st.aborted,sequencesStarted:st.sequencesStarted,sequencesCompleted:st.sequencesCompleted,sequencesMutated:st.sequencesMutated,sequencesAbandoned:st.sequencesAbandoned,lastReason:st.lastReason,events:[...st.events]};}
function hiveExperienceSnapshot(h,b,age,profile){
  const phase=clamp(Number(b?.phase)||1,1,3),phaseElapsed=Math.max(0,Number(b?.phaseElapsed)||0),minPhase=bossPhaseMinDuration(G?.sector||1,phase),phaseMinRemaining=Math.max(0,minPhase-phaseElapsed),sigReliable=(G?.sector||1)>=11||b?.pattern==='blade',sigPhase=Number(h?.signatureByPhase?.[phase]||0),signatureDebt=sigReliable&&phaseElapsed>=Math.min(9,minPhase*.58)&&sigPhase===0?1:0;
  const nextRatio=phase===1?.68:phase===2?.34:0,hpToNext=phase<3?Math.max(0,(Number(b?.hp)||0)-(Number(b?.maxHp)||1)*nextRatio):0,timeToNext=phase<3&&profile.effective>0?hpToNext/profile.effective:null,nativeGateProtecting=phase<3&&phaseMinRemaining>0&&Number.isFinite(timeToNext)&&timeToNext<phaseMinRemaining+.8;
  const phaseCount=Math.max(1,h?.phasesReached?.length||1),sigCount=Number(h?.signaturesExecuted||0),timeCoverage=clamp(age/Math.max(1,h?.target?.low||45),0,1),coverage=clamp((phaseCount/3)*.55+Math.min(sigCount,2)/2*.30+timeCoverage*.15,0,1),phaseDebt=Math.max(0,3-phaseCount),risk=(signatureDebt?1:0)+(phaseDebt>0&&Number.isFinite(h?.projectedTtk)&&h.projectedTtk<(h?.target?.low||45)*.58?1:0);
  return {phase,phaseElapsed:+phaseElapsed.toFixed(2),phaseMin:+minPhase.toFixed(2),phaseMinRemaining:+phaseMinRemaining.toFixed(2),phaseCount,phaseDebt,signatureTrackingReliable:sigReliable,signaturesThisPhase:sigPhase,signatureDebt,nativeGateProtecting,timeToNextPhase:Number.isFinite(timeToNext)?+timeToNext.toFixed(2):null,coverage:+coverage.toFixed(3),risk};
}
function hivePhaseSkipRisk(ttk,phase=1,nativeGate=false){if(nativeGate||!Number.isFinite(ttk))return 0;if(phase<=1)return ttk<8?2:ttk<16?1:0;if(phase===2)return ttk<8?1:0;return 0;}
function hiveCounterfactualMultiplier(ttk,age,target,state,profile){if(!Number.isFinite(ttk)||ttk<=0)return 1;if(profile?.fingerprint==='BURST_SPIKE'&&(profile?.sustainedVsExpected||0)<1.38)return 1;const desiredTotal=state==='A3'?Math.max(24,target.low*.56):target.low*.70,desiredRemaining=Math.max(5,desiredTotal-age);if(ttk>=desiredRemaining*.92)return 1;const raw=clamp(ttk/desiredRemaining,.70,.95);if(raw>=.93)return 1;if(raw>=.82)return .88;if(raw>=.74)return .78;return .70;}
function hiveProposedResponse(state,ctx={}){
  const p=G?.player||{hp:1,maxHp:1,shield:0,maxShield:1},hp=p.hp/Math.max(1,p.maxHp),sh=p.shield/Math.max(1,p.maxShield),sector=G?.sector||1,ttk=ctx.ttk,dpsRatio=ctx.dpsRatio||0,confidence=ctx.confidence??1,profile=ctx.profile||null,experience=ctx.experience||null,target=ctx.target||hiveTargetWindow(sector),age=ctx.age||0,hold=ctx.hold||{active:false,reasons:[]};
  if(state==='A0'){const help=(hp<.42||sh<.20)?'shield_or_repair':dpsRatio<.58?'damage_or_rate':'context_power';return {type:'SYMBIOTIC_ASSIST',wouldApply:false,resource:help,maxConcurrent:1,maxPerBoss:2,cooldownSec:12,confidence:+confidence.toFixed(3),reason:'underpowered_or_low_survival'};}
  if(state==='A1')return {type:'NONE',wouldApply:false,reason:'healthy_range',confidence:+confidence.toFixed(3)};
  if(confidence<.56)return {type:'OBSERVE_MORE',wouldApply:false,deferred:true,reason:'low_confidence',confidence:+confidence.toFixed(3)};
  const severe=state==='A3',mult=hiveCounterfactualMultiplier(ttk,age,target,state,profile),nativeGate=!!experience?.nativeGateProtecting,phaseRisk=hivePhaseSkipRisk(ttk,G?.boss?.phase||1,nativeGate),burstOnly=profile?.fingerprint==='BURST_SPIKE',sustained=profile?.fingerprint==='SUSTAINED_DOMINATION'||(profile?.sustainedVsExpected||0)>=1.65,aggressiveEligible=severe&&confidence>=.82&&sustained&&!hold.active;
  const sequence=['signature_priority','defensive_mobility'];if(mult<1)sequence.push('reactive_armor_shadow');if(phaseRisk>0)sequence.push('soft_gate_shadow');if(aggressiveEligible&&sector>=11&&G?.mode!=='bossRush')sequence.push('rift_disruption_shadow');
  return {type:burstOnly?'BURST_OBSERVATION':(severe?'DOMINATION_RESPONSE':'OVERPOWER_RESPONSE'),wouldApply:false,confidence:+confidence.toFixed(3),fingerprint:profile?.fingerprint||'UNKNOWN',receivedDamageMultiplier:mult,durationSec:mult<1?(severe?5:4):0,estimatedTtkGainSec:mult<1&&Number.isFinite(ttk)?+(ttk*(1/mult-1)).toFixed(2):0,signaturePriority:severe?'high':'moderate',defensiveMobility:true,tacticalSummonsExistingOnly:true,responseSequence:sequence,softGateSec:phaseRisk>0?(severe?.9:.65):0,phaseSkipRisk:phaseRisk,phaseProtectionSource:nativeGate?'native_phase_gate':(phaseRisk?'shadow_candidate':'none'),experienceDebt:experience?{phaseDebt:experience.phaseDebt,signatureDebt:experience.signatureDebt,coverage:experience.coverage,risk:experience.risk}:null,fairnessHold:hold.active?hold.reasons:[],suppression:sector>=11&&aggressiveEligible?{name:'RIFT_DISRUPTION',durationSec:3.5,telegraph:true,dodgeableCandidate:true,preserveIcons:true,restoreExactState:true,mutatesBuild:false}:null,rebirthEligible:sector>=11&&aggressiveEligible&&Number.isFinite(ttk)&&ttk<25&&((experience?.phaseDebt||0)>0||(experience?.signatureDebt||0)>0)};
}
...
function hiveStartBoss(b){
  if(!G||!b||G.mode==='training')return null;const target=hiveTargetWindow(G.sector),preDps=hiveRollingDps(20,false),build=hiveBuildSnapshot(),startHpRatio=clamp(b.hp/Math.max(1,b.maxHp),.35,1);target.low=+(target.low*startHpRatio).toFixed(2);target.high=+(target.high*startHpRatio).toFixed(2);target.expectedDpsBase=b.hp/((target.low+target.high)/2);const history=hiveHistoricalBaseline(G.sector,G.runDifficulty||'normal'),cal=hiveCalibratedExpected(target.expectedDpsBase,history);target.expectedDps=cal.expected;target.historyCalibration={samples:history.samples,medianDps:history.medianDps,medianTtk:history.medianTtk,weight:cal.weight,delta:cal.delta};const initialTtk=preDps>0?b.hp/preDps:null,index=hivePowerIndex(preDps,target.expectedDps,build),state=hiveRawAdaptiveState(preDps,initialTtk,index,0,target,true,1,null),now=G.elapsed||0;
  hiveResetBossDamageMeter();G.hiveDirector={mode:HIVE_DIRECTOR_CONFIG.mode,schema:11,world:G.sector,boss:b.name,sessionId:G.hiveSessionId||hiveSessionId(),startedAt:now,sampleT:0,target,history,state,initialState:state,maxState:state,stateSince:now,lastTransitionAt:now,candidate:null,candidateSamples:0,transitions:[],preDps:+preDps.toFixed(3),effectiveDps:0,dpsProfile:null,dpsPeak:0,dpsRatio:0,powerIndex:index,powerIndexInitial:index,powerIndexMax:index,ttkInitial:Number.isFinite(initialTtk)?+initialTtk.toFixed(2):null,ttk:null,ttkMin:null,projectedTtk:null,confidence:0,confidenceMax:0,buildInitial:build,buildCurrent:build,proposedResponse:hiveProposedResponse(state,{ttk:initialTtk,dpsRatio:target.expectedDps?preDps/target.expectedDps:0,target,confidence:1}),proposalHistory:[],damageReceivedByBoss:0,damageToPlayer:0,phasesReached:[b.phase||1],currentPhase:b.phase||1,phaseEnteredAt:{[b.phase||1]:now},phaseDurations:{},signaturesExecuted:0,signatureByPhase:{1:0,2:0,3:0},signatureTimeline:[],signaturePilotUsedByPhase:{},signaturePilotEvents:[],spatial:null,tacticalPilot:null,arenaPilot:null,counterEvolution:null,orchestrator:{budget:HIVE_DIRECTOR_CONFIG.orchestratorBudgetStart,quietUntil:0,recent:[],events:[],spent:{},denied:{},commits:0,phaseCounts:{},phaseTotals:{},phaseMajors:{},phaseSpend:{},lastByType:{},lastUtility:{},utilityEma:{},contextCache:null,familyFatigue:{},performanceDeferrals:0,contextDenials:0,conflictDeferrals:0,pressureDeferrals:0,phaseSalienceDenials:0,familyFatigueDenials:0,phaseExitDeferrals:0,phaseEntryDeferrals:0,coalescedReservations:0,pressure:0,reservations:[],lastPhase:Number(b.phase||1),phaseEntries:0},softGatesProposed:0,suppressionsProposed:0,maxReductionProposed:1,rebirthProposed:null,experience:null,experienceRiskMax:0,coverageMin:1,fairnessDeferrals:0,nativeGateDeferrals:0,fingerprintHistory:[],finished:false};return G.hiveDirector;
}
function hiveRecordPhase(phase){const h=G?.hiveDirector;if(!h||h.finished)return;const now=G.elapsed||0,prev=h.currentPhase||Math.max(1,phase-1);if(h.phaseEnteredAt?.[prev]!=null)h.phaseDurations[prev]=+Math.max(0,now-h.phaseEnteredAt[prev]).toFixed(2);h.currentPhase=phase;h.phaseEnteredAt[phase]=now;if(!h.phasesReached.includes(phase))h.phasesReached.push(phase);}
function hiveRecordSignature(label='signature'){const h=G?.hiveDirector;if(h&&!h.finished){const phase=G?.boss?.phase||h.currentPhase||1;h.signaturesExecuted=(h.signaturesExecuted||0)+1;h.signatureByPhase[phase]=(h.signatureByPhase[phase]||0)+1;h.signatureTimeline.push({at:+Math.max(0,(G.elapsed||0)-h.startedAt).toFixed(2),phase,label:String(label||'signature').slice(0,64)});if(h.signatureTimeline.length>18)h.signatureTimeline.shift();}}
...
function hiveMaybeSignaturePilot(h,b,age,profile,experience,hold){
  if(!HIVE_DIRECTOR_CONFIG.signaturePilot||!h||!b||h.finished||h.finalizing||G.sector<HIVE_DIRECTOR_CONFIG.signaturePilotWorld||G.mode!=='campaign'||h.state!=='A3'||h.confidence<HIVE_DIRECTOR_CONFIG.signaturePilotConfidence||hold.active||!experience?.signatureDebt||profile?.fingerprint!=='SUSTAINED_DOMINATION'||b.phase<2||h.counterEvolution?.active)return false;
  const ph=b.phase;if(h.signaturePilotUsedByPhase[ph]||b.signatureAttack||b.telegraphT>0||b.specialT>0||b.chargeT>0||b.entry>0||(b.phaseTransitionT||0)>0)return false;const op=hiveOrchestratorPermit('SIGNATURE_PRIORITY',{major:true});if(!op.ok){hiveOrchestratorDeny('SIGNATURE_PRIORITY',op.reason);return false;}const cfg=bossSignatureConfig(b);if(!cfg||!Number.isFinite(b.signatureAttackCd)||b.signatureAttackCd<=HIVE_DIRECTOR_CONFIG.signaturePilotCooldownTarget)return false;
  const before=b.signatureAttackCd;if(!hiveOrchestratorCommit('SIGNATURE_PRIORITY',{major:true,reason:'signature_debt_sustained_A3'}))return false;b.signatureAttackCd=Math.min(b.signatureAttackCd,HIVE_DIRECTOR_CONFIG.signaturePilotCooldownTarget);h.signaturePilotUsedByPhase[ph]=true;h.signaturePilotEvents.push({at:+age.toFixed(2),phase:ph,before:+before.toFixed(2),after:+b.signatureAttackCd.toFixed(2),confidence:h.confidence,fingerprint:profile.fingerprint,reason:'signature_debt_sustained_A3'});return true;
}
function hiveSampleDirector(){
  const b=G?.boss,h=G?.hiveDirector;if(!b||!h||h.finished||b.dead)return;const now=G.elapsed||0,age=Math.max(0,now-h.startedAt),target=h.target,build=hiveBuildSnapshot(),profile=hiveDpsProfile(target.expectedDps,age),dps=profile.effective,ttk=dps>0?Math.max(0,b.hp)/dps:null,projected=Number.isFinite(ttk)?age+ttk:null,index=hivePowerIndex(dps,target.expectedDps,build),ratio=target.expectedDps>0?dps/target.expectedDps:0,confidence=profile.confidence,raw=hiveRawAdaptiveState(dps,projected,index,age,target,false,confidence,profile),hold=hiveFairnessHold(b);
  h.effectiveDps=+dps.toFixed(3);h.dpsProfile=profile;h.dpsPeak=Math.max(h.dpsPeak||0,profile.burst,profile.tactical,profile.sustained);h.dpsRatio=+ratio.toFixed(3);h.ttk=Number.isFinite(ttk)?+ttk.toFixed(2):null;h.projectedTtk=Number.isFinite(projected)?+projected.toFixed(2):null;if(Number.isFinite(ttk))h.ttkMin=h.ttkMin===null?h.ttk:Math.min(h.ttkMin,h.ttk);h.powerIndex=index;h.powerIndexMax=Math.max(h.powerIndexMax,index);h.confidence=confidence;h.confidenceMax=Math.max(h.confidenceMax||0,confidence);h.buildCurrent=build;hiveHysteresisStep(h,raw,now);const experience=hiveExperienceSnapshot(h,b,age,profile);h.experience=experience;h.experienceRiskMax=Math.max(h.experienceRiskMax||0,experience.risk||0);h.coverageMin=Math.min(h.coverageMin??1,experience.coverage);if(hold.active)h.fairnessDeferrals++;if(experience.nativeGateProtecting)h.nativeGateDeferrals++;h.fingerprintHistory.push(profile.fingerprint);if(h.fingerprintHistory.length>20)h.fingerprintHistory.shift();
  const proposal=hiveProposedResponse(h.state,{ttk,dpsRatio:ratio,confidence,profile,experience,target,age,hold});h.proposedResponse=proposal;hiveMaybeSignaturePilot(h,b,age,profile,experience,hold);hiveCounterEvolutionSample(h,b,age,profile,hold);
  const sig=JSON.stringify([h.state,proposal.type,proposal.receivedDamageMultiplier||1,proposal.resource||'',proposal.suppression?.name||'',proposal.phaseSkipRisk||0,proposal.fingerprint||'',experience.signatureDebt,experience.nativeGateProtecting]);if(h._proposalSig!==sig){h._proposalSig=sig;h.proposalHistory.push({at:+age.toFixed(2),phase:b.phase,state:h.state,confidence,profile:{...profile},experience:{...experience},proposal:{...proposal}});if(h.proposalHistory.length>20)h.proposalHistory.shift();}
  if((proposal.phaseSkipRisk||0)>0)h.softGatesProposed=Math.max(h.softGatesProposed,proposal.phaseSkipRisk);if(proposal.suppression)h.suppressionsProposed=Math.max(h.suppressionsProposed,1);if(proposal.receivedDamageMultiplier&&proposal.receivedDamageMultiplier<1)h.maxReductionProposed=Math.min(h.maxReductionProposed,proposal.receivedDamageMultiplier);
}
function hiveUpdate(dt){const h=G?.hiveDirector;if(!h||h.finished)return;hiveOrchestratorTick(dt);hiveSpatialUpdate(dt);hiveTacticalPilotUpdate(dt);hiveBranchFeintUpdate(dt);h.sampleT=(h.sampleT||0)+Math.max(0,dt||0);const sampleInterval=hiveDirectorSampleInterval();if(h.sampleT<sampleInterval)return;h.sampleT%=sampleInterval;hiveSampleDirector();}
function hiveLoadTelemetry(){if(HIVE_TELEMETRY_CACHE)return HIVE_TELEMETRY_CACHE;let data=null,migrated=false;try{data=JSON.parse(localStorage.getItem(KEY_HIVE_TELEMETRY)||'null');if(!data){data=JSON.parse(localStorage.getItem(KEY_HIVE_TELEMETRY_PREV)||'null');migrated=!!data;}}catch(_){}if(!data||typeof data!=='object')data={schema:12,version:VERSION,fights:[]};data.schema=12;data.version=VERSION;data.migratedFrom=migrated?'2.15.29':(data.migratedFrom||null);data.fights=Array.isArray(data.fights)?data.fights.slice(-HIVE_DIRECTOR_CONFIG.maxFights):[];HIVE_TELEMETRY_CACHE=data;return data;}
...
function hiveTelemetrySummary(){const data=hiveLoadTelemetry(),f=data.fights,states={A0:0,A1:0,A2:0,A3:0};let sum=0,n=0,pilots=0,tactical=0,arena=0,orchestratorCommits=0,orchestratorDenied=0,counterTrials=0,counterLearned=0,counterRetired=0,sequences=0,mutated=0,abandoned=0,branches=0,branchCommits=0;for(const r of f){states[r.maxState]=(states[r.maxState]||0)+1;if(Number.isFinite(r.ttkReal)){sum+=r.ttkReal;n++;}pilots+=Number(r.signaturePilotEvents?.length||0);tactical+=Number(r.tacticalPilotEvents?.length||0);arena+=Number(r.arenaPilotEvents?.length||0);orchestratorCommits+=Number(r.orchestrator?.commits||0);orchestratorDenied+=Object.values(r.orchestrator?.denied||{}).reduce((a,v)=>a+(Number(v)||0),0);counterTrials+=Number(r.counterEvolution?.trials||0);counterLearned+=Number(r.counterEvolution?.successes||0);counterRetired+=Number(r.counterEvolution?.failures||0);sequences+=Number(r.counterEvolution?.sequences?.started||0);mutated+=Number(r.counterEvolution?.sequences?.mutated||0);abandoned+=Number(r.counterEvolution?.sequences?.abandoned||0);branches+=Number(r.counterEvolution?.sequences?.branches?.started||0);branchCommits+=Number(r.counterEvolution?.sequences?.branches?.committed||0);}return {schema:data.schema,stored:f.length,limit:HIVE_DIRECTOR_CONFIG.maxFights,migratedFrom:data.migratedFrom||null,states,averageTtk:n?+(sum/n).toFixed(2):null,signaturePilotEvents:pilots,tacticalPilotEvents:tactical,arenaPilotEvents:arena,orchestrator:{commits:orchestratorCommits,denied:orchestratorDenied},counterEvolution:{trials:counterTrials,learned:counterLearned,retired:counterRetired,sequences:{started:sequences,mutated,abandoned,branches:{started:branches,committed:branchCommits}}},last:f.length?{world:f[f.length-1].world,boss:f[f.length-1].boss,ttk:f[f.length-1].ttkReal,maxState:f[f.length-1].maxState,result:f[f.length-1].result,fingerprint:f[f.length-1].dpsProfileFinal?.fingerprint||null}:null};}
function hiveFinalizeBoss(result='victory'){
  const h=G?.hiveDirector,b=G?.boss;if(!h||h.finished)return null;h.finalizing=true;hiveSampleDirector();const now=G.elapsed||0,duration=Math.max(.01,now-h.startedAt),ph=h.currentPhase||b?.phase||1;if(h.phaseEnteredAt?.[ph]!=null)h.phaseDurations[ph]=+Math.max(0,now-h.phaseEnteredAt[ph]).toFixed(2);const sustainedA3=h.maxState==='A3'&&(h.dpsProfile?.fingerprint==='SUSTAINED_DOMINATION'||(h.dpsProfile?.sustainedVsExpected||0)>=1.65),experience=h.experience||{},extreme=sustainedA3&&duration<25&&h.confidence>=.82,rebirth=G.sector>=11&&extreme&&G.mode==='campaign'&&((experience.phaseDebt||0)>0||(experience.signatureDebt||0)>0)?{wouldActivate:true,returnHpRatio:G.sector===20?.50:.40,reason:'extreme_sustained_fast_kill_with_experience_debt',behaviorMustChange:true,requiredChanges:['locomotion','pattern','signaturePriority','aggression'],maxPerBoss:1}:null;h.rebirthProposed=rebirth;h.finished=true;
  const rec={schema:12,version:VERSION,mode:HIVE_DIRECTOR_CONFIG.mode,world:h.world,boss:h.boss,timestamp:Date.now(),sessionId:h.sessionId,runMode:G.mode||'campaign',difficulty:G.runDifficulty||'normal',target:h.target,historyBaseline:h.history,dpsPreBoss:h.preDps,dpsDuringBoss:h.effectiveDps,dpsPeak:+(h.dpsPeak||0).toFixed(3),dpsProfileFinal:h.dpsProfile,powerIndexInitial:h.powerIndexInitial,powerIndexMax:h.powerIndexMax,confidenceFinal:h.confidence,confidenceMax:h.confidenceMax,ttkInitial:h.ttkInitial,ttkMinimum:h.ttkMin,ttkProjectedTotal:h.projectedTtk,ttkReal:+duration.toFixed(2),stateInitial:h.initialState,maxState:h.maxState,stateChanges:h.transitions.slice(),fingerprintHistory:[...h.fingerprintHistory],helpsProposed:h.proposalHistory.filter(x=>x.state==='A0').map(x=>({at:x.at,phase:x.phase,resource:x.proposal.resource,maxConcurrent:x.proposal.maxConcurrent,maxPerBoss:x.proposal.maxPerBoss,cooldownSec:x.proposal.cooldownSec})),reactiveArmorProposed:h.proposalHistory.filter(x=>x.proposal.receivedDamageMultiplier&&x.proposal.receivedDamageMultiplier<1).map(x=>({at:x.at,phase:x.phase,multiplier:x.proposal.receivedDamageMultiplier,durationSec:x.proposal.durationSec,ttkGain:x.proposal.estimatedTtkGainSec,fingerprint:x.proposal.fingerprint})),maxReductionProposed:h.maxReductionProposed,maxDamageReductionFraction:+(1-h.maxReductionProposed).toFixed(3),suppressionsProposed:h.proposalHistory.filter(x=>x.proposal.suppression).map(x=>({at:x.at,phase:x.phase,...x.proposal.suppression})),softGatesProposed:h.proposalHistory.filter(x=>(x.proposal.phaseSkipRisk||0)>0).map(x=>({at:x.at,phase:x.phase,durationSec:x.proposal.softGateSec,risk:x.proposal.phaseSkipRisk,protection:x.proposal.phaseProtectionSource})),softGateRiskMax:h.softGatesProposed,rebirthProposed:rebirth?{phase:b?.phase||null,...rebirth}:null,signaturePilotEvents:[...h.signaturePilotEvents],orchestrator:hiveOrchestratorSnapshot(),spatialFinal:hiveSpatialSnapshot(),tacticalPilotEvents:[...(h.tacticalPilot?.events||[])],tacticalSegments:h.tacticalPilot?.totalSegments||0,tacticalNeutralSegments:h.tacticalPilot?.neutralSegments||0,arenaPilotEvents:[...(h.arenaPilot?.events||[])],arenaPilotExecuted:h.arenaPilot?.executed||0,arenaPilotDeferred:h.arenaPilot?.deferred||0,counterEvolution:{trials:(h.counterEvolution?.events||[]).filter(e=>e.type==='START'&&e.mode==='TRIAL').length,reuses:(h.counterEvolution?.events||[]).filter(e=>e.type==='START'&&e.mode==='LEARNED_REUSE').length,successes:h.counterEvolution?.successes||0,failures:h.counterEvolution?.failures||0,aborted:h.counterEvolution?.aborted||0,sequences:{started:h.counterEvolution?.sequencesStarted||0,completed:h.counterEvolution?.sequencesCompleted||0,mutated:h.counterEvolution?.sequencesMutated||0,abandoned:h.counterEvolution?.sequencesAbandoned||0,phaseUse:{...(h.counterEvolution?.sequencePhaseUse||{})},retired:{...(h.counterEvolution?.sequenceRetired||{})},branches:{started:h.counterEvolution?.branchesStarted||0,committed:h.counterEvolution?.branchesCommitted||0,phaseUse:{...(h.counterEvolution?.branchPhaseUse||{})}}},learned:{...(h.counterEvolution?.learned||{})},retired:Object.fromEntries(Object.entries(h.counterEvolution?.retired||{}).map(([k,v])=>[k,Object.keys(v||{})])),events:[...(h.counterEvolution?.events||[])]},duration:+duration.toFixed(2),damageToPlayer:+(h.damageToPlayer||0).toFixed(2),damageReceivedByBoss:+(h.damageReceivedByBoss||0).toFixed(2),phasesReached:[...h.phasesReached],phaseDurations:{...h.phaseDurations},signaturesExecuted:h.signaturesExecuted||0,signatureByPhase:{...h.signatureByPhase},signatureTimeline:[...h.signatureTimeline],experienceFinal:experience,experienceRiskMax:h.experienceRiskMax,nativeGateDeferrals:h.nativeGateDeferrals,fairnessDeferrals:h.fairnessDeferrals,result,buildInitial:h.buildInitial,buildFinal:h.buildCurrent};h.record=rec;hivePersistFight(rec);return rec;
}
function hiveDirectorStatus(){const h=G?.hiveDirector,tp=h?.tacticalPilot,ce=h?.counterEvolution,ceSnap=hiveCounterEvolutionSnapshot();return {version:VERSION,mode:HIVE_DIRECTOR_CONFIG.mode,gameplayIntervention:'PACING_ECOLOGY + RHYTHM_FORECASTING + STABILITY_COORDINATION + CONTEXTUAL_ORCHESTRATION + SIGNATURE_PRIORITY + TACTICAL_IDENTITY_GEOMETRY + ARENA_MORPHOLOGY + COUNTER_EVOLUTION + COEVOLUTION_MICROSEQUENCES + LEGIBLE_FEINT_BRANCHING',inactiveInterventions:['reactive_armor','symbiotic_assist','rift_disruption','rift_rebirth','extra_soft_gates'],sampleInterval:HIVE_DIRECTOR_CONFIG.sampleInterval,currentSampleInterval:hiveDirectorSampleInterval(),orchestrator:hiveOrchestratorSnapshot(),damageWindows:{...HIVE_DIRECTOR_CONFIG.damageWindows},powerIndex:h?.powerIndex??null,effectiveDps:h?.effectiveDps??0,dpsProfile:h?.dpsProfile||null,confidence:h?.confidence??0,ttk:h?.ttk??null,projectedTtk:h?.projectedTtk??null,adaptiveState:h?.state||null,experience:h?.experience||null,historicalCalibration:h?.target?.historyCalibration||null,spatial:h?hiveSpatialSnapshot():null,tacticalIdentityPilot:{enabled:HIVE_DIRECTOR_CONFIG.tacticalPilot,active:!!tp?.active,variant:tp?.variant||'NEUTRAL',intensity:+(tp?.intensity||0).toFixed(3),segments:tp?.totalSegments||0,neutralSegments:tp?.neutralSegments||0,events:tp?.events?[...tp.events]:[]},signaturePilot:h?{enabled:HIVE_DIRECTOR_CONFIG.signaturePilot,usedByPhase:{...h.signaturePilotUsedByPhase},events:[...h.signaturePilotEvents]}:{enabled:HIVE_DIRECTOR_CONFIG.signaturePilot},arenaMorphologyPilot:{enabled:HIVE_DIRECTOR_CONFIG.arenaPilot,plan:h?.arenaPilot?.plan||null,executed:h?.arenaPilot?.executed||0,deferred:h?.arenaPilot?.deferred||0,events:h?.arenaPilot?.events?[...h.arenaPilot.events]:[]},counterEvolution:{enabled:HIVE_DIRECTOR_CONFIG.counterEvolution,sessionOnly:true,currentTactic:ce?.currentTactic||'READING',active:ceSnap?.active||null,learned:{...(ce?.learned||{})},successes:ce?.successes||0,failures:ce?.failures||0,aborted:ce?.aborted||0,lastReason:ce?.lastReason||'inactive',microSequences:{enabled:HIVE_DIRECTOR_CONFIG.microSequence,identityMatrix:Object.keys(HIVE_MICRO_SEQUENCE_IDENTITY).length,started:ce?.sequencesStarted||0,completed:ce?.sequencesCompleted||0,mutated:ce?.sequencesMutated||0,abandoned:ce?.sequencesAbandoned||0,maxPerPhase:HIVE_DIRECTOR_CONFIG.microSequenceMaxPerPhase,feintBranching:{enabled:HIVE_DIRECTOR_CONFIG.branchFeint,telegraphSec:HIVE_DIRECTOR_CONFIG.branchTelegraphSec,commitDelay:HIVE_DIRECTOR_CONFIG.branchCommitDelay,maxPerPhase:HIVE_DIRECTOR_CONFIG.branchFeintMaxPerPhase,started:ce?.branchesStarted||0,committed:ce?.branchesCommitted||0,phaseUse:{...(ce?.branchPhaseUse||{})}}}},hysteresis:h?{candidate:h.candidate,candidateSamples:h.candidateSamples,required:HIVE_DIRECTOR_CONFIG.hysteresisSamples,minStateTime:HIVE_DIRECTOR_CONFIG.minStateTime,cooldown:HIVE_DIRECTOR_CONFIG.stateCooldown}:null,proposedResponse:h?.proposedResponse||null,currentBoss:h?{world:h.world,name:h.boss,phase:G?.boss?.phase||0}:null,telemetrySummary:hiveTelemetrySummary()};}
function hiveDirectorReport(){const h=G?.hiveDirector;return {status:hiveDirectorStatus(),runtime:h?{target:h.target,history:h.history,dpsPeak:h.dpsPeak,transitions:[...h.transitions],fingerprints:[...h.fingerprintHistory],phaseDurations:{...h.phaseDurations},signatureByPhase:{...h.signatureByPhase},spatial:hiveSpatialSnapshot(),tacticalPilot:h.tacticalPilot?{...h.tacticalPilot,events:[...h.tacticalPilot.events]}:null,counterEvolution:hiveCounterEvolutionSnapshot(),proposalHistory:h.proposalHistory.map(x=>({at:x.at,phase:x.phase,state:x.state,confidence:x.confidence,fingerprint:x.profile?.fingerprint,experience:x.experience,proposal:x.proposal}))}:null,telemetry:hiveTelemetrySummary()};}
function hiveDirectorSelfTest(){
  const target={low:70,high:110,expectedDps:10},cases={A0:hiveRawAdaptiveState(3,180,.5,10,target,false,1,{fingerprint:'LOW_OUTPUT',sustainedVsExpected:.3}),A1:hiveRawAdaptiveState(10,90,1,10,target,false,1,{fingerprint:'STEADY',sustainedVsExpected:1}),A2:hiveRawAdaptiveState(16,50,1.7,10,target,false,1,{fingerprint:'STEADY',sustainedVsExpected:1.6}),A3:hiveRawAdaptiveState(30,20,3,10,target,false,1,{fingerprint:'SUSTAINED_DOMINATION',sustainedVsExpected:3})};const burstCapped=hiveRawAdaptiveState(30,20,3,6,target,false,.8,{fingerprint:'BURST_SPIKE',sustainedVsExpected:1.05});const lowConfidence=hiveRawAdaptiveState(30,20,3,10,target,false,.4,{fingerprint:'SUSTAINED_DOMINATION',sustainedVsExpected:3});const ctx={state:'A1',maxState:'A1',stateSince:0,lastTransitionAt:0,candidate:null,candidateSamples:0,transitions:[]};const h1=hiveHysteresisStep(ctx,'A2',5),h2=hiveHysteresisStep(ctx,'A2',6),h3=hiveHysteresisStep(ctx,'A2',7),passStates=Object.entries(cases).every(([k,v])=>k===v);return {states:cases,statesPass:passStates,burstGuard:burstCapped!=='A3',confidenceGuard:lowConfidence!=='A3',hysteresis:{first:h1,second:h2,third:h3,state:ctx.state,pass:!h1&&!h2&&h3&&ctx.state==='A2'},heavyCountermeasuresShadow:true,signaturePilotPreserved:true,tacticalGeometryNoStatInflation:true,counterEvolutionSessionOnly:true,counterEvolutionNoStatInflation:true,microSequenceIdentityMatrix:Object.keys(HIVE_MICRO_SEQUENCE_IDENTITY).length===10,microSequenceNoStatInflation:true,microSequenceAntiStack:true,feintBranchingLegible:HIVE_DIRECTOR_CONFIG.branchFeint&&HIVE_DIRECTOR_CONFIG.branchTelegraphSec>=.75,feintSingleCommit:true,feintNoTrackingAfterCommit:true,orchestratorEnabled:HIVE_DIRECTOR_CONFIG.orchestrator===true,contextualOrchestration:HIVE_DIRECTOR_CONFIG.orchestratorContextual===true,predictiveReservations:!!HIVE_DIRECTOR_CONFIG.orchestratorReservationDurations,pressureRecovery:HIVE_DIRECTOR_CONFIG.orchestratorPressureMajorGate<1,phaseSaliencePolicy:!!HIVE_DIRECTOR_CONFIG.orchestratorPhasePolicies?.[2],phaseAttentionCap:HIVE_DIRECTOR_CONFIG.orchestratorPhaseTotalCap===5,gracefulRecoveryDeferral:HIVE_DIRECTOR_CONFIG.orchestratorRecoveryDefers.includes('ARENA_MORPHOLOGY'),orchestratorBudgetBounded:HIVE_DIRECTOR_CONFIG.orchestratorBudgetStart<=HIVE_DIRECTOR_CONFIG.orchestratorBudgetMax,adaptiveSampling:[HIVE_DIRECTOR_CONFIG.sampleInterval,HIVE_DIRECTOR_CONFIG.sampleBalanced,HIVE_DIRECTOR_CONFIG.sampleRecovery].every((v,i,a)=>i===0||v>=a[i-1]),pass:passStates&&burstCapped!=='A3'&&lowConfidence!=='A3'&&!h1&&!h2&&h3&&ctx.state==='A2'&&Object.keys(HIVE_MICRO_SEQUENCE_IDENTITY).length===10&&HIVE_DIRECTOR_CONFIG.branchFeint&&HIVE_DIRECTOR_CONFIG.branchTelegraphSec>=.75&&HIVE_DIRECTOR_CONFIG.orchestrator&&HIVE_DIRECTOR_CONFIG.orchestratorContextual&&HIVE_DIRECTOR_CONFIG.orchestratorPhaseTotalCap===5&&HIVE_DIRECTOR_CONFIG.orchestratorRecoveryDefers.includes('ARENA_MORPHOLOGY')&&HIVE_DIRECTOR_CONFIG.orchestratorBudgetStart<=HIVE_DIRECTOR_CONFIG.orchestratorBudgetMax};
}
window.__SWARM_HIVE_DIRECTOR_STATUS=()=>hiveDirectorStatus();
window.__SWARM_HIVE_DIRECTOR_REPORT=()=>hiveDirectorReport();
window.__SWARM_HIVE_DIRECTOR_SELFTEST=()=>hiveDirectorSelfTest();
function hiveOrchestratorSelfTest(){const savedG=G,savedTier=PERF.tier;try{
  const mk=(phase=2)=>({elapsed:10,boss:{phase,phaseTransitionT:0,phaseElapsed:4},sector:15,hiveDirector:{startedAt:0,state:'A3',confidence:.95,experience:{risk:2,coverage:.55,signatureDebt:1,phaseElapsed:4,phaseMin:10,timeToNextPhase:8,nativeGateProtecting:false},spatial:{confidence:.9,read:'EVASIVE'},counterEvolution:{currentTactic:'EVASIVE',streak:HIVE_DIRECTOR_CONFIG.counterEvolutionRepeatSamples,learned:{EVASIVE:'X'},active:null},orchestrator:{budget:HIVE_DIRECTOR_CONFIG.orchestratorBudgetStart,quietUntil:0,recent:[],events:[],spent:{},denied:{},commits:0,phaseCounts:{},phaseTotals:{},phaseMajors:{},phaseSpend:{},lastByType:{},lastUtility:{},utilityEma:{},contextCache:null,familyFatigue:{},pressure:0,reservations:[],lastPhase:phase,phaseEntries:0,lastCommitAt:-999,maintenanceAcc:0,maintenancePasses:0}}});
  PERF.tier='FULL';G=mk(2);const start=hiveOrchestratorSnapshot(),p1=hiveOrchestratorPermit('SIGNATURE_PRIORITY',{major:true}),c1=hiveOrchestratorCommit('SIGNATURE_PRIORITY',{major:true,reason:'selftest'}),after1=hiveOrchestratorSnapshot(),conflict=hiveOrchestratorPermit('ARENA_MORPHOLOGY',{major:true,ignoreQuiet:true,skipContext:true});
  G=mk(2);G.hiveDirector.experience.phaseElapsed=7;G.hiveDirector.experience.timeToNextPhase=5;const headroom=hiveOrchestratorPermit('MICRO_SEQUENCE',{major:true,ignoreQuiet:true,skipContext:true});G.hiveDirector.experience.timeToNextPhase=1;const signatureExit=hiveOrchestratorPermit('SIGNATURE_PRIORITY',{major:true,ignoreQuiet:true,skipContext:true});
  G=mk(3);G.hiveDirector.orchestrator.familyFatigue.SEQUENCE=.70;const forecastFatigue=hiveOrchestratorPermit('MICRO_SEQUENCE',{major:true,ignoreQuiet:true,skipContext:true});
  G=mk(3);G.hiveDirector.orchestrator.lastCommitAt=9.6;const spacing=hiveOrchestratorPermit('COUNTER_TRIAL',{ignoreQuiet:true,skipContext:true});
  G=mk(2);G.hiveDirector.experience.phaseElapsed=7;G.hiveDirector.experience.timeToNextPhase=.5;G.hiveDirector.counterEvolution.active={mode:'MICRO_SEQUENCE'};G.hiveDirector.orchestrator.familyFatigue.SEQUENCE=.99;G.hiveDirector.orchestrator.lastCommitAt=9.9;const nested=hiveOrchestratorPermit('FEINT_BRANCH',{ignoreQuiet:true,skipContext:true});
  G=mk(3);G.hiveDirector.orchestrator.budget=50;const b0=G.hiveDirector.orchestrator.budget;hiveOrchestratorTick(.04);hiveOrchestratorTick(.04);const preBatch=G.hiveDirector.orchestrator.maintenancePasses||0;hiveOrchestratorTick(.03);const postBatch=G.hiveDirector.orchestrator.maintenancePasses||0,b1=G.hiveDirector.orchestrator.budget;
  PERF.tier='RECOVERY';const recoveryDefer=hiveOrchestratorPermit('ARENA_MORPHOLOGY',{major:true,skipContext:true,ignoreQuiet:true});const full=HIVE_DIRECTOR_CONFIG.sampleInterval;PERF.tier='BALANCED';const balanced=hiveDirectorSampleInterval();PERF.tier='RECOVERY';const recovery=hiveDirectorSampleInterval();
  const checks={firstPermit:!!p1.ok,firstCommit:!!c1,reservationConflict:!conflict.ok&&conflict.reason==='reservation_conflict_SIGNATURE_PRIORITY',durationHeadroom:!headroom.ok&&headroom.reason==='phase_duration_headroom',signatureExitException:!!signatureExit.ok,forecastFatigue:!forecastFatigue.ok&&forecastFatigue.reason==='forecast_family_fatigue',rhythmSpacing:!spacing.ok&&spacing.reason==='rhythm_spacing',nestedBranchException:!!nested.ok,maintenanceBatched:preBatch===0&&postBatch===1&&b1>b0,recoveryDefersVisual:!recoveryDefer.ok&&recoveryDefer.reason==='performance_recovery',samplingOrdered:full<=balanced&&balanced<=recovery,budgetSpent:(start?.budget??0)>(after1?.budget??0)};
  return {...checks,sampling:{full,balanced,recovery},headroom:headroom.headroom||null,forecast:forecastFatigue,spacing:spacing.spacing??null,pass:Object.values(checks).every(Boolean)};
}finally{G=savedG;PERF.tier=savedTier;}}
window.__SWARM_HIVE_ORCHESTRATOR_SELFTEST=()=>hiveOrchestratorSelfTest();
window.__SWARM_HIVE_CONTEXTUAL_ORCHESTRATOR_SELFTEST=()=>hiveOrchestratorSelfTest();
window.__SWARM_HIVE_STABILITY_COORDINATOR_SELFTEST=()=>hiveOrchestratorSelfTest();
window.__SWARM_HIVE_PACING_ECOLOGY_SELFTEST=()=>hiveOrchestratorSelfTest();
function hiveRhythmForecastSelfTest(){const savedG=G,savedTier=PERF.tier;try{const mk=(phase=2,elapsed=4,timeToNext=12)=>({elapsed:elapsed,boss:{phase,phaseTransitionT:0,phaseElapsed:elapsed},hiveDirector:{startedAt:0,state:'A3',confidence:.95,experience:{risk:1,coverage:.55,signatureDebt:0,phaseElapsed:elapsed,phaseMin:10,timeToNextPhase:timeToNext,nativeGateProtecting:false},spatial:{confidence:.9,read:'EVASIVE'},counterEvolution:{currentTactic:'EVASIVE',streak:HIVE_DIRECTOR_CONFIG.counterEvolutionRepeatSamples,learned:{EVASIVE:'X'},active:null},orchestrator:{budget:100,quietUntil:0,recent:[],events:[],spent:{},denied:{},commits:0,phaseCounts:{},phaseTotals:{},phaseMajors:{},phaseSpend:{},lastByType:{},lastUtility:{},utilityEma:{},contextCache:null,familyFatigue:{},pressure:0,reservations:[],lastPhase:phase,phaseEntries:0}}});PERF.tier='FULL';G=mk(2,.5,12);const entry=hiveOrchestratorPermit('ARENA_MORPHOLOGY',{major:true,skipContext:true});G=mk(2,8,2.1);const exit=hiveOrchestratorPermit('MICRO_SEQUENCE',{major:true,skipContext:true});G=mk(2,5,12);const first=hiveOrchestratorCommit('ARENA_MORPHOLOGY',{major:true,skipContext:true,ignoreQuiet:true,reserveFor:2});G.elapsed=5.1;const secondReserve=hiveOrchestratorReserve('ARENA_MORPHOLOGY',hiveOrchestratorAge(),3,true),snap=hiveOrchestratorSnapshot();G=mk(2,5,12);G.hiveDirector.orchestrator.familyFatigue={SPATIAL:.82};const fatigue=hiveOrchestratorPermit('ARENA_MORPHOLOGY',{major:true,skipContext:true,ignoreQuiet:true});G=mk(2,5,12);const c1=hiveOrchestratorContext('COUNTER_TRIAL'),c2=hiveOrchestratorContext('COUNTER_TRIAL');return {entryRhythmBlocked:!entry.ok&&entry.reason==='phase_entry_rhythm',exitRiskBlocked:!exit.ok&&exit.reason==='phase_exit_risk',reservationCoalesced:!!first&&!!secondReserve&&(snap?.coalescedReservations||0)>=1&&(snap?.reservations?.filter(r=>r.type==='ARENA_MORPHOLOGY').length||0)===1,familyFatigueBlocked:!fatigue.ok&&fatigue.reason==='family_fatigue',contextCacheStable:c1.score===c2.score&&c1.rawScore===c2.rawScore,utilitySmoothingConfigured:HIVE_DIRECTOR_CONFIG.orchestratorUtilityAlpha>0&&HIVE_DIRECTOR_CONFIG.orchestratorUtilityAlpha<1,pass:!entry.ok&&entry.reason==='phase_entry_rhythm'&&!exit.ok&&exit.reason==='phase_exit_risk'&&!!first&&!!secondReserve&&(snap?.coalescedReservations||0)>=1&&!fatigue.ok&&fatigue.reason==='family_fatigue'&&c1.score===c2.score};}finally{G=savedG;PERF.tier=savedTier;}}
window.__SWARM_HIVE_RHYTHM_FORECAST_SELFTEST=()=>hiveRhythmForecastSelfTest();
...
function makeCH2IdentitySignature(b,p,cfg,id){
  const tactical=hiveTacticalVector(b,p,id),sigY=tactical.active?lerp(p.y,tactical.predictedY,.60):p.y,sigX=p.x;
  const s={pattern:b.pattern,identity:id.key,stage:'wind',t:cfg.wind,max:cfg.wind,activeMax:cfg.active,hit:0,healLock:0,started:false,base:b.t||0,tacticalVariant:tactical.variant||'NEUTRAL'};
  if(id.key==='KHEPRI'){s.cx=b.x;s.cy=b.y;s.r0=48;s.maxR=Math.max(W,H)*.78;s.nodes=2+b.phase;s.spin=Math.random()*TAU;}
  else if(id.key==='VESPERA'){const n=b.phase;s.lanes=[];const gap=58;for(let i=0;i<n;i++)s.lanes.push(clamp(sigY+(i-(n-1)/2)*gap,70,H-70));s.startX=W*.88;s.endX=W*.12;s.side=Math.random()<.5?-1:1;}
  else if(id.key==='REGENT'){s.centerY=clamp(sigY,105,H-105);s.startGap=Math.min(H*.72,390);s.endGap=92+b.phase*8;s.jawWidth=16+b.phase*3;}
  else if(id.key==='PYROLUX'){s.cx=clamp(sigX+W*.12,W*.28,W*.68);s.cy=clamp(sigY,110,H-110);s.nodes=4+b.phase;s.radius=78+b.phase*18;s.spin=Math.random()*TAU;}
  else if(id.key==='CAUSTIC'){s.pools=[];const n=b.phase===1?2:3;for(let i=0;i<n;i++){const off=(i-(n-1)/2)*95;s.pools.push({x:clamp(sigX+rnd(-45,80),W*.16,W*.70),y:clamp(sigY+off+rnd(-22,22),78,H-78),r:44+b.phase*8,phase:rnd(0,TAU)});}s.blast=b.phase===3;}
  else if(id.key==='ATLAS'){s.cx=clamp(sigX+W*.16,W*.30,W*.66);s.cy=clamp(sigY,105,H-105);s.rays=5+b.phase*2;s.safe=rndI(0,s.rays-1);s.rot=Math.atan2(sigY-s.cy,sigX-s.cx)-s.safe*TAU/s.rays;s.inner=34;s.outer=Math.max(W,H)*1.18;}
  else if(id.key==='MIRAGE'){s.lines=[];const n=2+b.phase;for(let i=0;i<n;i++){const off=(i-(n-1)/2)*(68-b.phase*5),y1=clamp(sigY+off,66,H-66),y2=clamp(sigY-off+rnd(-34,34),66,H-66);s.lines.push({x1:W*.88,y1,x2:W*.08,y2,phase:rnd(0,TAU)});}s.echoes=Array.from({length:2+b.phase},(_,i)=>({a:i*TAU/(2+b.phase)+rnd(-.2,.2),r:64+i*18}));}
  else if(id.key==='PIT'){s.pits=[];const n=b.phase;for(let i=0;i<n;i++){const off=(i-(n-1)/2)*(112-b.phase*8);s.pits.push({x:clamp(sigX+rnd(40,150),W*.22,W*.72),y:clamp(sigY+off+rnd(-20,20),82,H-82),r:48+b.phase*8,spin:rnd(0,TAU)});}s.emergeX=clamp(sigX+W*.18,W*.46,W*.78);s.emergeY=clamp(H-sigY+rnd(-45,45),90,H-90);s.emerged=false;}
  else if(id.key==='AURICULUS'){s.base=Math.atan2(sigY-b.y,sigX-b.x);s.sep0=.68;s.sep1=.075;s.len=Math.max(W,H)*1.30;s.ox=b.x-b.r*.18;s.oy=b.y;s.chainPulse=false;}
  else if(id.key==='APIS'){s.cx=clamp(sigX+W*.14,W*.30,W*.64);s.cy=clamp(sigY,105,H-105);s.cells=[];const n=6+b.phase*2,rad=92+b.phase*15;s.safe=rndI(0,n-1);for(let i=0;i<n;i++){const a=i*TAU/n+(i%2?0:.08),rr=rad*(i%3===0?.70:1);s.cells.push({x:s.cx+Math.cos(a)*rr,y:s.cy+Math.sin(a)*rr,r:30+b.phase*4,safe:i===s.safe});}s.volleyCd=.28;s.guardSpawned=false;}
  return s;
}
...
function updateCH2BossIdentityMovement(b,p,sec,dt,phaseSpeed){
  const id=bossIdentity();if(!id)return false;const tactical=hiveTacticalVector(b,p,id),a=tactical.active?tactical.aimAngle:Math.atan2(p.y-b.y,p.x-b.x),emit=()=>bossAttackEmitter(b,'primary'),tx=tactical.xOffset||0,ty=tactical.yOffset||0,ts=v=>hiveTacticalSpread(v,tactical),tr=v=>hiveTacticalRingPhase(v,tactical);
  if(id.key==='KHEPRI'){b.x=W*.68+Math.cos(b.t*(.62+b.phase*.07))*W*(.12+.015*b.phase)+tx*.55;b.y=H*.5+Math.sin(b.t*(.86+b.phase*.08))*H*(.20+.025*b.phase)+ty*.72;if(b.fire<=0){b.fire=Math.max(.32,.62-.055*b.phase);commanderFan(b,emit().x,emit().y,a,3+b.phase,ts(.08),265+25*b.phase,8,4,'khepri');if(b.phase>=2)ringBullets(b.x,b.y,6+b.phase*2,145+b.phase*16,7,id.color,tr(b.t),b,'khepri-orbit');}}
  else if(id.key==='VESPERA'){b.x=W*.63+Math.sin(b.t*(2.15+b.phase*.22))*W*.22+tx*.78;b.y=H*.5+Math.sin(b.t*(3.55+b.phase*.30))*H*.34+ty*.86;if(b.fire<=0){b.fire=Math.max(.20,.46-.05*b.phase);commanderFan(b,emit().x,emit().y,a,2+b.phase,ts(.055),355+b.phase*35,7.5,3.6,'vespera');if(b.phase===3&&Math.random()<.38)spawnEnemy(sec.forms[0],b.x-35,b.y+rnd(-100,100));}}
  else if(id.key==='REGENT'){b.x=W*.69+Math.sin(b.t*.58)*W*.105+tx*.32;b.y=lerp(b.y,clamp((tactical.active?tactical.predictedY:p.y)+ty*.48,110,H-110),clamp(dt*(.38+b.phase*.08),0,1));if(b.fire<=0){b.fire=Math.max(.38,.72-.055*b.phase);const sep=.13+b.phase*.018;commanderFan(b,bossAttackEmitter(b,'upper').x,bossAttackEmitter(b,'upper').y,a+sep,3,ts(.05),255+b.phase*22,10,4.8,'regent-upper');commanderFan(b,bossAttackEmitter(b,'lower').x,bossAttackEmitter(b,'lower').y,a-sep,3,ts(.05),255+b.phase*22,10,4.8,'regent-lower');}}
  else if(id.key==='PYROLUX'){b.x=W*.65+Math.sin(b.t*1.22)*W*.17+tx*.48;b.y=H*.5+Math.sin(b.t*2.44)*Math.cos(b.t*.77)*H*.31+ty*.62;if(b.fire<=0){b.fire=Math.max(.30,.60-.055*b.phase);commanderFan(b,emit().x,emit().y,a,3,ts(.10),245+b.phase*18,7,4,'pyrolux');if(b.phase>=2)ringBullets(b.x,b.y,5+b.phase,120+b.phase*14,6,id.color,tr(b.t*.72),b,'lumen');}}
  else if(id.key==='CAUSTIC'){b.x=W*.70+Math.sin(b.t*.52)*W*.10+tx*.30;b.y=H*.5+Math.sin(b.t*.92)*H*.23+ty*.48;if(b.fire<=0){b.fire=Math.max(.34,.69-.05*b.phase);commanderFan(b,emit().x,emit().y,a,5+b.phase,ts(.10),205+b.phase*16,9,5,'caustic');if(b.phase===3&&Math.random()<.26)spawnEnemy(sec.forms[1],b.x-40,b.y+rnd(-110,110));}}
  else if(id.key==='ATLAS'){b.identityOpacity=1;b.x=W*.66+Math.sin(b.t*.74)*W*.14+tx*.44;b.y=H*.5+Math.sin(b.t*1.62)*Math.cos(b.t*.48)*H*.30+ty*.72;if(b.fire<=0){b.fire=Math.max(.31,.64-.055*b.phase);commanderFan(b,emit().x,emit().y,a,3+b.phase,ts(.09),230+b.phase*18,7,4,'atlas-silk');if(b.phase>=2&&Math.random()<.42)ringBullets(b.x,b.y,5+b.phase,112+b.phase*14,6,id.color,tr(b.t*.5),b,'atlas-dust');}}
  else if(id.key==='MIRAGE'){b.identityOpacity=.82+.18*(.5+.5*Math.sin(b.t*7));b.x=W*.64+Math.sin(b.t*(1.85+b.phase*.13))*W*.20+tx*.70;b.y=H*.5+Math.sin(b.t*(3.05+b.phase*.20))*H*.34+(tactical.active?-ty*.88:0);if(Math.random()<dt*(4+b.phase))G.particles.push({kind:'slash',x:b.x-rnd(18,65),y:b.y+rnd(-30,30),a:rnd(-.18,.18),len:rnd(40,95),life:.16,max:.16,col:hexA(id.color,.72)});if(b.fire<=0){b.fire=Math.max(.22,.50-.045*b.phase);commanderFan(b,emit().x,emit().y,a,2+b.phase,ts(.065),325+b.phase*24,7.5,3.8,'mirage');}}
  else if(id.key==='PIT'){b.identityOpacity=1;b.x=W*.70+Math.sin(b.t*.58)*W*.11+tx*.26;b.y=lerp(b.y,clamp((tactical.active?tactical.predictedY:p.y)+ty*.52,100,H-100),clamp(dt*(.34+b.phase*.07),0,1));if(b.fire<=0){b.fire=Math.max(.34,.70-.055*b.phase);ringBullets(b.x,b.y,6+b.phase*2,160+b.phase*18,8,id.color,tr(b.t),b,'pit-maw');if(b.phase===3&&Math.random()<.28)commanderFan(b,emit().x,emit().y,a,5,ts(.10),245,9,4.5,'pit-shards');}}
  else if(id.key==='AURICULUS'){b.identityOpacity=1;b.x=W*.66+Math.sin(b.t*(1.28+b.phase*.11))*W*.18+tx*.62;b.y=H*.5+Math.sin(b.t*(2.28+b.phase*.16))*H*.29+ty*.68;if(b.fire<=0){b.fire=Math.max(.23,.51-.045*b.phase);commanderFan(b,emit().x,emit().y,a,3+b.phase,ts(.075),315+b.phase*26,8,4,'auriculus');if(b.phase>=2){const t=G.enemies.filter(e=>!e.dead&&!e.microSwarm)[0];if(t)G.particles.push({kind:'arc',x:b.x,y:b.y,x2:t.x,y2:t.y,life:.16,max:.16,col:id.color});}}}
  else if(id.key==='APIS'){b.identityOpacity=1;b.x=W*.66+Math.sin(b.t*.92)*W*.16+tx*.38;b.y=H*.5+Math.sin(b.t*1.72)*H*.28+ty*.60;if(b.fire<=0){b.fire=Math.max(.25,.55-.05*b.phase);commanderFan(b,emit().x,emit().y,a,4+b.phase,ts(.085),285+b.phase*24,8,4,'apis-royal');if(b.phase>=2&&Math.random()<.30&&G.enemies.filter(e=>!e.dead&&!e.microSwarm).length<11)spawnEnemy(sec.forms[0],b.x-35,b.y+rnd(-105,105),'summon');}}
...
function updateBossSignatureAttack(b,p,sec,dt){
  ...
  if(!s){
    if(b.entry<=0&&(b.phaseTransitionT||0)<=0&&!b.telegraphT&&!b.chargeT&&!b.specialT&&b.signatureAttackCd<=0&&(b.phaseElapsed||0)>2.0){
      b.signatureAttack=makeBossSignature(b,p,cfg);hiveRecordSignature(cfg.name);bossAnimStartSignatureCue(b,cfg,cfg.name);b.animAttackT=Math.max(b.animAttackT,cfg.wind+.2);b.specialCd=Math.max(b.specialCd,cfg.wind+cfg.active+.35);G.bossWarningText=cfg.name;G.bossWarningT=Math.max(G.bossWarningT,cfg.wind);notify(`⚡ ${cfg.name}`,cfg.color,1.15);AudioX.bossWarn(G.sector-1);return true;
    }return false;
  }
...
function updateBoss(dt){
  ...
  if(!hiveArenaMorphologyUpdate(b,p,sec,dt,pressure)&&b.arenaCd<=0){bossArenaHazard(b,p,sec);b.arenaCd=Math.max(2.15,(6.2-b.phase*.72-G.sector*.12-(G.mode==='bossRush'?.45:0))*pressure.cool);}
...
window.__SWARM_V21520_STATUS=()=>({version:VERSION,adaptiveHivePredictive:hiveDirectorStatus(),novelty:{multiWindowDps:true,confidenceAware:true,burstFingerprint:true,localBossMemory:true,experienceDebt:true,counterfactualPlanner:true,signaturePriorityPilot:true},liveIntervention:{signaturePriorityOnly:true,worlds:'11-20',maxOncePerPhase:true,telegraphPreserved:true,armor:false,suppression:false,rebirth:false,extraSoftGates:false},preserved:{worlds:activeSectorCount(),bosses:Object.keys(BOSS_ANIMATIONS).length,hudMobileReadability:true,bossDeathAftermath:true,bossArenaPresentation:true,preBossEscalation:true,waveChoreographer:true,enemySonicEcology:true,bossAudioEventSync:true,animationAttackSync:true,bossIdentityPass:true,microSwarmFrom:1,riftAllyFrom:11,lastChanceSeconds:5,directTouch:true,noJoystick:true,noDash:true,assetStreamer:true,performanceGovernor:true,pwa:true}});
window.__SWARM_V21521_STATUS=()=>({version:VERSION,adaptiveHiveTacticalIdentity:hiveDirectorStatus(),novelty:{bossFightSpatialReader:true,sessionOnly:true,tacticalIdentityMatrix:Object.keys(HIVE_TACTICAL_IDENTITY).length,adaptiveAimLeadBounded:true,movementBiasBounded:true,fanGeometryRedistribution:true,ringPhaseVariation:true,neutralRecoverySegments:true},liveIntervention:{signaturePriority:true,tacticalIdentityGeometry:true,worlds:'11-20',phaseFrom:2,noExtraProjectiles:true,noHpInflation:true,noDamageBoost:true,noCadenceBoost:true,telegraphPreserved:true,armor:false,suppression:false,rebirth:false},preserved:{worlds:activeSectorCount(),bosses:Object.keys(BOSS_ANIMATIONS).length,hudMobileReadability:true,bossDeathAftermath:true,bossArenaPresentation:true,preBossEscalation:true,waveChoreographer:true,enemySonicEcology:true,bossAudioEventSync:true,animationAttackSync:true,bossIdentityPass:true,microSwarmFrom:1,riftAllyFrom:11,lastChanceSeconds:5,directTouch:true,noJoystick:true,noDash:true,assetStreamer:true,performanceGovernor:true,pwa:true}});
window.__SWARM_V21522_STATUS=()=>({version:VERSION,adaptiveHiveArenaMorphology:hiveDirectorStatus(),novelty:{arenaIdentityMatrix:Object.keys(HIVE_ARENA_IDENTITY).length,readableSafeCorridor:true,preAttackTelegraph:true,spatiallyInformedCorridor:true,reusesExistingArenaBudget:true,noExtraProjectileDensity:true,noExtraObstacleDensity:true},liveIntervention:{signaturePriority:true,tacticalIdentityGeometry:true,arenaMorphology:true,worlds:'11-20',phaseFrom:2,safeCorridorGuaranteed:true,telegraphSeconds:HIVE_DIRECTOR_CONFIG.arenaTelegraph,noHpInflation:true,noDamageBoost:true,noCadenceBoost:true,armor:false,suppression:false,rebirth:false},preserved:{worlds:activeSectorCount(),bosses:Object.keys(BOSS_ANIMATIONS).length,hudMobileReadability:true,bossDeathAftermath:true,bossArenaPresentation:true,preBossEscalation:true,waveChoreographer:true,enemySonicEcology:true,bossAudioEventSync:true,animationAttackSync:true,bossIdentityPass:true,microSwarmFrom:1,riftAllyFrom:11,lastChanceSeconds:5,directTouch:true,noJoystick:true,noDash:true,assetStreamer:true,performanceGovernor:true,pwa:true}});
window.__SWARM_V21523_STATUS=()=>({version:VERSION,adaptiveHiveCounterEvolution:hiveDirectorStatus(),novelty:{sessionTacticalMemory:true,repeatPatternDetection:true,trialMeasureRetireLoop:true,learnedResponseReuse:true,counterIdentityMatrix:Object.keys(HIVE_COUNTER_EVOLUTION_IDENTITY).length,fairnessAbort:true,noPersistentPlayerProfile:true},liveIntervention:{signaturePriority:true,tacticalIdentityGeometry:true,arenaMorphology:true,counterEvolutionGeometry:true,worlds:'11-20',phaseFrom:2,maxTrialsPerPhase:HIVE_DIRECTOR_CONFIG.counterEvolutionMaxTrialsPerPhase,maxReuses:HIVE_DIRECTOR_CONFIG.counterEvolutionMaxReuses,noHpInflation:true,noDamageBoost:true,noCadenceBoost:true,noExtraProjectiles:true,noBuildMutation:true,armor:false,suppression:false,rebirth:false},preserved:{worlds:activeSectorCount(),bosses:Object.keys(BOSS_ANIMATIONS).length,hudMobileReadability:true,bossDeathAftermath:true,bossArenaPresentation:true,preBossEscalation:true,waveChoreographer:true,enemySonicEcology:true,bossAudioEventSync:true,animationAttackSync:true,bossIdentityPass:true,microSwarmFrom:1,riftAllyFrom:11,lastChanceSeconds:5,directTouch:true,noJoystick:true,noDash:true,assetStreamer:true,performanceGovernor:true,pwa:true}});
window.__SWARM_V21524_STATUS=()=>({version:VERSION,adaptiveHiveCoevolutionSequences:hiveDirectorStatus(),novelty:{microSequenceIdentityMatrix:Object.keys(HIVE_MICRO_SEQUENCE_IDENTITY).length,threeActSequences:true,learnedResponseComposition:true,decodeDetection:true,oneMutationBeforeAbandon:true,sequenceRetirementPerFight:true,arenaAntiStack:true,signatureAntiStack:true,noPersistentPlayerProfile:true},liveIntervention:{signaturePriority:true,tacticalIdentityGeometry:true,arenaMorphology:true,counterEvolutionGeometry:true,coevolutionMicroSequences:true,worlds:'11-20',phaseFrom:2,maxSequencePerPhase:HIVE_DIRECTOR_CONFIG.microSequenceMaxPerPhase,decodeSamples:HIVE_DIRECTOR_CONFIG.microSequenceDecodeSamples,noHpInflation:true,noDamageBoost:true,noCadenceBoost:true,noExtraProjectiles:true,noBuildMutation:true,armor:false,suppression:false,rebirth:false},preserved:{worlds:activeSectorCount(),bosses:Object.keys(BOSS_ANIMATIONS).length,hudMobileReadability:true,bossDeathAftermath:true,bossArenaPresentation:true,preBossEscalation:true,waveChoreographer:true,enemySonicEcology:true,bossAudioEventSync:true,animationAttackSync:true,bossIdentityPass:true,microSwarmFrom:1,riftAllyFrom:11,lastChanceSeconds:5,directTouch:true,noJoystick:true,noDash:true,assetStreamer:true,performanceGovernor:true,pwa:true}});
window.__SWARM_V21525_STATUS=()=>({version:VERSION,adaptiveHiveFeintBranching:hiveDirectorStatus(),novelty:{legibleDualIntentTelegraph:true,reactionSnapshotSingleRead:true,frozenBranchAfterCommit:true,branchIdentityUsesExistingResponses:true,microSequenceMutationStillActive:true,arenaAntiStack:true,signatureAntiStack:true,noPersistentPlayerProfile:true},liveIntervention:{signaturePriority:true,tacticalIdentityGeometry:true,arenaMorphology:true,counterEvolutionGeometry:true,coevolutionMicroSequences:true,legibleFeintBranching:true,worlds:'11-20',phaseFrom:2,branchTelegraphSec:HIVE_DIRECTOR_CONFIG.branchTelegraphSec,branchCommitDelay:HIVE_DIRECTOR_CONFIG.branchCommitDelay,maxBranchPerPhase:HIVE_DIRECTOR_CONFIG.branchFeintMaxPerPhase,noHpInflation:true,noDamageBoost:true,noCadenceBoost:true,noExtraProjectiles:true,noBuildMutation:true,armor:false,suppression:false,rebirth:false},preserved:{worlds:activeSectorCount(),bosses:Object.keys(BOSS_ANIMATIONS).length,hudMobileReadability:true,bossDeathAftermath:true,bossArenaPresentation:true,preBossEscalation:true,waveChoreographer:true,enemySonicEcology:true,bossAudioEventSync:true,animationAttackSync:true,bossIdentityPass:true,microSwarmFrom:1,riftAllyFrom:11,lastChanceSeconds:5,directTouch:true,noJoystick:true,noDash:true,assetStreamer:true,performanceGovernor:true,pwa:true}});
window.__SWARM_V21527_STATUS=()=>({version:VERSION,adaptiveHiveContextualOrchestration:hiveDirectorStatus(),novelty:{salienceBudget:true,contextUtility:true,phaseAttentionBudget:true,typeCooldowns:true,quietWindows:true,repeatFatigue:true,adaptiveSamplingByPerformanceTier:true,gracefulRecoveryDeferral:true,existingLayersArbitrated:true,noNewDamageLayer:true},liveIntervention:{orchestrator:true,signaturePriority:true,tacticalIdentityGeometry:true,arenaMorphology:true,counterEvolutionGeometry:true,coevolutionMicroSequences:true,legibleFeintBranching:true,worlds:'11-20',phaseFrom:2,budgetMax:HIVE_DIRECTOR_CONFIG.orchestratorBudgetMax,phaseTotalCap:HIVE_DIRECTOR_CONFIG.orchestratorPhaseTotalCap,phaseMajorCap:HIVE_DIRECTOR_CONFIG.orchestratorPhaseMajorCap,minUtility:HIVE_DIRECTOR_CONFIG.orchestratorMinUtility,recoveryPerSec:HIVE_DIRECTOR_CONFIG.orchestratorRecoveryPerSec,quietMajor:HIVE_DIRECTOR_CONFIG.orchestratorQuietMajor,noHpInflation:true,noDamageBoost:true,noCadenceBoost:true,noExtraProjectiles:true,noBuildMutation:true,armor:false,suppression:false,rebirth:false},performance:{adaptiveSampling:true,full:HIVE_DIRECTOR_CONFIG.sampleInterval,balanced:HIVE_DIRECTOR_CONFIG.sampleBalanced,recovery:HIVE_DIRECTOR_CONFIG.sampleRecovery,recoveryDefers:[...HIVE_DIRECTOR_CONFIG.orchestratorRecoveryDefers]},preserved:{worlds:activeSectorCount(),bosses:Object.keys(BOSS_ANIMATIONS).length,hudMobileReadability:true,bossDeathAftermath:true,bossArenaPresentation:true,preBossEscalation:true,waveChoreographer:true,enemySonicEcology:true,bossAudioEventSync:true,animationAttackSync:true,bossIdentityPass:true,microSwarmFrom:1,riftAllyFrom:11,lastChanceSeconds:5,directTouch:true,noJoystick:true,noDash:true,assetStreamer:true,performanceGovernor:true,pwa:true}});
window.__SWARM_V21528_STATUS=()=>({version:VERSION,adaptiveHiveStabilityCoordination:hiveDirectorStatus(),novelty:{predictiveInterventionReservations:true,conflictMatrix:true,pressureRecoveryEnvelope:true,phaseSalienceBudget:true,phaseTransitionBreathing:true,contextUtility:true,adaptiveSamplingByPerformanceTier:true,noNewDamageLayer:true},liveIntervention:{orchestrator:true,signaturePriority:true,tacticalIdentityGeometry:true,arenaMorphology:true,counterEvolutionGeometry:true,coevolutionMicroSequences:true,legibleFeintBranching:true,worlds:'11-20',phaseFrom:2,phasePolicies:HIVE_DIRECTOR_CONFIG.orchestratorPhasePolicies,pressureMajorGate:HIVE_DIRECTOR_CONFIG.orchestratorPressureMajorGate,reservationDurations:HIVE_DIRECTOR_CONFIG.orchestratorReservationDurations,noHpInflation:true,noDamageBoost:true,noCadenceBoost:true,noExtraProjectiles:true,noBuildMutation:true,armor:false,suppression:false,rebirth:false},performance:{adaptiveSampling:true,full:HIVE_DIRECTOR_CONFIG.sampleInterval,balanced:HIVE_DIRECTOR_CONFIG.sampleBalanced,recovery:HIVE_DIRECTOR_CONFIG.sampleRecovery,recoveryDefers:[...HIVE_DIRECTOR_CONFIG.orchestratorRecoveryDefers]},preserved:{worlds:activeSectorCount(),bosses:Object.keys(BOSS_ANIMATIONS).length,hudMobileReadability:true,bossDeathAftermath:true,bossArenaPresentation:true,preBossEscalation:true,waveChoreographer:true,enemySonicEcology:true,bossAudioEventSync:true,animationAttackSync:true,bossIdentityPass:true,microSwarmFrom:1,riftAllyFrom:11,lastChanceSeconds:5,directTouch:true,noJoystick:true,noDash:true,assetStreamer:true,performanceGovernor:true,pwa:true}});
window.__SWARM_V21529_STATUS=()=>({version:VERSION,adaptiveHiveRhythmForecasting:hiveDirectorStatus(),novelty:{phaseRhythmForecast:true,phaseExitRiskGuard:true,crossFamilyFatigue:true,smoothedContextUtility:true,contextCache:true,reservationCoalescing:true,noNewDamageLayer:true},liveIntervention:{orchestrator:true,signaturePriority:true,tacticalIdentityGeometry:true,arenaMorphology:true,counterEvolutionGeometry:true,coevolutionMicroSequences:true,legibleFeintBranching:true,worlds:'11-20',phaseFrom:2,phaseEntryRhythm:HIVE_DIRECTOR_CONFIG.orchestratorPhaseEntryRhythm,phaseExitLookahead:HIVE_DIRECTOR_CONFIG.orchestratorPhaseExitLookahead,familyGate:HIVE_DIRECTOR_CONFIG.orchestratorFamilyGate,noHpInflation:true,noDamageBoost:true,noCadenceBoost:true,noExtraProjectiles:true,noBuildMutation:true,armor:false,suppression:false,rebirth:false},performance:{adaptiveSampling:true,contextCacheSec:HIVE_DIRECTOR_CONFIG.orchestratorContextCacheSec,full:HIVE_DIRECTOR_CONFIG.sampleInterval,balanced:HIVE_DIRECTOR_CONFIG.sampleBalanced,recovery:HIVE_DIRECTOR_CONFIG.sampleRecovery},preserved:{worlds:activeSectorCount(),bosses:Object.keys(BOSS_ANIMATIONS).length,hudMobileReadability:true,bossDeathAftermath:true,bossArenaPresentation:true,preBossEscalation:true,waveChoreographer:true,enemySonicEcology:true,bossAudioEventSync:true,animationAttackSync:true,bossIdentityPass:true,microSwarmFrom:1,riftAllyFrom:11,lastChanceSeconds:5,directTouch:true,noJoystick:true,noDash:true,assetStreamer:true,performanceGovernor:true,pwa:true}});
window.__SWARM_V21530_STATUS=()=>({version:VERSION,adaptiveHivePacingEcology:hiveDirectorStatus(),novelty:{durationAwareHeadroom:true,predictiveFamilyFatigue:true,dynamicRhythmSpacing:true,batchedOrchestratorMaintenance:true,reservationCoalescing:true,crossFamilyFatigue:true,smoothedContextUtility:true,noNewDamageLayer:true},liveIntervention:{orchestrator:true,signaturePriority:true,tacticalIdentityGeometry:true,arenaMorphology:true,counterEvolutionGeometry:true,coevolutionMicroSequences:true,legibleFeintBranching:true,worlds:'11-20',phaseFrom:2,durationSafety:HIVE_DIRECTOR_CONFIG.orchestratorDurationSafety,forecastFamilyGate:HIVE_DIRECTOR_CONFIG.orchestratorForecastFamilyGate,rhythmSpacing:HIVE_DIRECTOR_CONFIG.orchestratorRhythmSpacing,maintenanceInterval:HIVE_DIRECTOR_CONFIG.orchestratorMaintenanceInterval,noHpInflation:true,noDamageBoost:true,noCadenceBoost:true,noExtraProjectiles:true,noBuildMutation:true,armor:false,suppression:false,rebirth:false},performance:{adaptiveSampling:true,batchedMaintenance:true,maintenanceInterval:HIVE_DIRECTOR_CONFIG.orchestratorMaintenanceInterval,contextCacheSec:HIVE_DIRECTOR_CONFIG.orchestratorContextCacheSec,full:HIVE_DIRECTOR_CONFIG.sampleInterval,balanced:HIVE_DIRECTOR_CONFIG.sampleBalanced,recovery:HIVE_DIRECTOR_CONFIG.sampleRecovery},preserved:{worlds:activeSectorCount(),bosses:Object.keys(BOSS_ANIMATIONS).length,hudMobileReadability:true,bossDeathAftermath:true,bossArenaPresentation:true,preBossEscalation:true,waveChoreographer:true,enemySonicEcology:true,bossAudioEventSync:true,animationAttackSync:true,bossIdentityPass:true,microSwarmFrom:1,riftAllyFrom:11,lastChanceSeconds:5,directTouch:true,noJoystick:true,noDash:true,assetStreamer:true,performanceGovernor:true,pwa:true}});
})();
