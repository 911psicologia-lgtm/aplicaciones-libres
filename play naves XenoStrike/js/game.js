...
// RESPUESTA EVOLUTIVA DE LA COLMENA
// Primera etapa segura: observa, estima y registra localmente. NO modifica HP, daño,
// poderes, patrones, drops ni reglas de boss. Todas las respuestas son "wouldApply".
// ─────────────────────────────────────────────────────────────
const HIVE_DIRECTOR_MODE='shadow';
const HIVE_TELEMETRY_KEY='swarm_rift_hive_boss_telemetry_v1';
const HIVE_DIRECTOR={
  sampleWindow:20,reviewEvery:1.0,hysteresis:3.0,stateCooldown:5.0,telemetryCap:120,
  states:['A0','A1','A2','A3'],labels:{A0:'ASISTENCIA',A1:'EQUILIBRIO',A2:'SOBREPOTENCIA',A3:'DOMINACIÓN'}
};
const HIVE_OFFENSIVE_POWERS=new Set(['twin','tesla','missile','rail','cryo','acid','drone','overdrive','gravity','burst','prismburst','resonance','sparklaser','bio','phase','nanites','riftmine','echo','nova','ionweb','orbitblade']);
const HIVE_SURVIVAL_POWERS=new Set(['shield','hemadrain','resinwall']);
function hiveLoadTelemetry(){try{const v=JSON.parse(localStorage.getItem(HIVE_TELEMETRY_KEY)||'null');if(v&&Array.isArray(v.records))return v;}catch(_){}return {schema:1,mode:HIVE_DIRECTOR_MODE,records:[]};}
let HIVE_TELEMETRY=hiveLoadTelemetry();
function hiveSaveTelemetry(){HIVE_TELEMETRY.schema=1;HIVE_TELEMETRY.mode=HIVE_DIRECTOR_MODE;HIVE_TELEMETRY.records=(HIVE_TELEMETRY.records||[]).slice(-HIVE_DIRECTOR.telemetryCap);try{localStorage.setItem(HIVE_TELEMETRY_KEY,JSON.stringify(HIVE_TELEMETRY));}catch(_){}return HIVE_TELEMETRY.records.length;}
function hiveTargetWindow(world=G?.sector||1){world=clamp(Math.round(world||1),1,20);if(world<=5)return {min:45,max:75,tier:'EARLY'};if(world<=10)return {min:55,max:90,tier:'MID'};if(world<20)return {min:70,max:110,tier:'ADVANCED'};return {min:90,max:130,tier:'FINAL'};}
function hiveThresholds(world=G?.sector||1){const w=hiveTargetWindow(world);return {domination:Math.min(25,w.min*.36),overpower:w.min*.62,assistance:w.max*1.45};}
function hiveStateRank(st){return Math.max(0,HIVE_DIRECTOR.states.indexOf(st));}
function hiveEnsureRuntime(){if(!G)return null;G.hiveDamageSamples=G.hiveDamageSamples||[];return G;}
function hivePruneSamples(now=G?.elapsed||0){if(!G)return;hiveEnsureRuntime();const cut=now-30;G.hiveDamageSamples=G.hiveDamageSamples.filter(x=>x.t>=cut);}
function hiveRecordOutgoingDamage(amount,target,kind='normal'){
  if(!G||!Number.isFinite(amount)||amount<=0||kind==='debug')return;hiveEnsureRuntime();const now=G.elapsed||0;G.hiveDamageSamples.push({t:now,amount,scope:target===G.boss?'boss':'world',kind:String(kind||'normal')});hivePruneSamples(now);
  const hs=G.hiveBossSession;if(hs&&target===G.boss&&!hs.finalized){hs.bossDamage=(hs.bossDamage||0)+amount;hs.damageEvents=(hs.damageEvents||0)+1;}
}
function hiveRecentDps(scope='boss',windowSec=HIVE_DIRECTOR.sampleWindow,now=G?.elapsed||0){if(!G)return 0;hiveEnsureRuntime();const cut=now-windowSec;let sum=0,first=now;for(const x of G.hiveDamageSamples){if(x.t>=cut&&x.scope===scope){sum+=x.amount;first=Math.min(first,x.t);}}const span=Math.max(1,Math.min(windowSec,now-first+.35));return sum/span;}
function hiveExpectedVolley(){if(!G)return 1;const pr=powerOn('prismburst')?powerRank('prismburst'):0,tr=powerOn('twin')?powerRank('twin'):0,br=powerOn('burst')?powerRank('burst'):0;if(pr)return pr>=7?9:pr>=5?7:pr>=3?5:pr>=2?5:3;if(G.activeCombos?.[comboId('rail','twin')]||G.activeCombos?.[comboId('burst','twin')]||tr>=3||br>=3)return (tr>=7||br>=7)?9:(tr>=5||br>=5)?7:5;if(tr)return branchIs('twin','B')?5:3;if(br)return br>=2?3:3;return 1;}
function hiveShotInterval(){if(!G)return .19;const lv=G.level||1;return Math.max(.052,.19*(1-up('rate')*.055)*(powerOn('overdrive')?(0.82-powerRank('overdrive')*.05):1)*(powerOn('burst')?(0.86-powerRank('burst')*.05):1)*(1-Math.min(.16,(lv-1)*.008))*(G.fusionSurgeT>0?.90:1)*riftApexRateMult());}
function hiveBuildSnapshot(boss=G?.boss,observedDps=null){
  const world=G?.sector||1,p=G?.player||{hp:1,maxHp:1,shield:0,maxShield:1};let offensiveQuality=0,survivalQuality=0,activePowers=0,rankSum=0;
  for(const [k,t] of Object.entries(G?.powers||{})){if(!(t>0)||!POWERS[k])continue;activePowers++;const r=powerRank(k)||1,quality=1+(r-1)*.28+(r>=6?.18:0);rankSum+=r;if(HIVE_OFFENSIVE_POWERS.has(k))offensiveQuality+=quality;if(HIVE_SURVIVAL_POWERS.has(k))survivalQuality+=quality;}
  const combos=Object.keys(G?.activeCombos||{}).length,interval=hiveShotInterval(),volley=hiveExpectedVolley(),cadence=.19/Math.max(.052,interval),simultaneous=(G?.bullets?.length||0)+volley;
  const permanentOffense=(up('damage')/8+up('rate')/7+up('drone')/4+up('synergy')/3+up('apexcore')/3+up('evolution')/2)/6;
  const permanentSurvival=(up('hull')/8+up('shield')/8)/2,allyScore=(powerOn('drone')?.85:0)+up('drone')*.18+(G?.riftAlly?1.0:0);
  const hpRatio=clamp(p.hp/Math.max(1,p.maxHp),0,1),shieldRatio=clamp(p.shield/Math.max(1,p.maxShield),0,1),survival=clamp(hpRatio*.52+shieldRatio*.28+Math.min(1.5,(p.maxHp+p.maxShield)/190)*.12+Math.min(.35,survivalQuality*.06)+permanentSurvival*.08,.15,1.6);
  const performance=clamp(1+(G?.combo||0)*.012-Math.min(.35,(G?.waveHits||0)*.035)+(G?.bossMasteryAchieved?.08:0),.45,1.55);
  const expectedDps=boss?.maxHp?boss.maxHp/((hiveTargetWindow(world).min+hiveTargetWindow(world).max)/2):0,dpsRatio=(observedDps!=null&&expectedDps>0)?clamp(observedDps/expectedDps,0,2.8):null;
  const parts=[['powers',clamp(offensiveQuality/4.2,0,2.6),.18],['combos',clamp((combos+(G?.fusionSurgeT>0?.75:0)+(G?.apexT>0?.85:0))/1.65,0,2.6),.12],['cadence',clamp(cadence/1.28,.45,2.6),.13],['projectiles',clamp(simultaneous/3.2,.35,2.6),.10],['allies',clamp(allyScore/1.15,0,2.5),.06],['upgrades',clamp(permanentOffense/(.32+world*.018),0,2.4),.10],['survival',survival,.09],['performance',performance,.07]];if(dpsRatio!=null)parts.push(['dps',dpsRatio,.32]);
  let num=0,den=0;for(const [,v,w] of parts){num+=v*w;den+=w;}const index=clamp((num/Math.max(.001,den))*100,20,240);
  return {index:+index.toFixed(1),observedDps:observedDps==null?null:+observedDps.toFixed(2),expectedDps:+expectedDps.toFixed(2),activePowers,offensiveQuality:+offensiveQuality.toFixed(2),survivalQuality:+survivalQuality.toFixed(2),rankSum,combos,cadence:+cadence.toFixed(2),shotInterval:+interval.toFixed(3),projectiles:simultaneous,volley,allies:+allyScore.toFixed(2),permanentOffense:+permanentOffense.toFixed(2),permanentSurvival:+permanentSurvival.toFixed(2),hpRatio:+hpRatio.toFixed(3),shieldRatio:+shieldRatio.toFixed(3),survival:+survival.toFixed(3),performance:+performance.toFixed(3),critical:{available:false,value:0,reason:'el motor actual no expone un stat crítico dedicado; reservado sin inventar datos'}};
}
function hiveClassify(metrics,world=G?.sector||1,combatElapsed=0){const th=hiveThresholds(world),ttk=metrics.ttk,power=metrics.powerIndex,surv=metrics.build?.survival??1;if(Number.isFinite(ttk)){if(combatElapsed>=3&&ttk<=th.domination&&(power>=120||metrics.dpsRatio>=1.8))return 'A3';if(combatElapsed>=4&&ttk<=th.overpower&&(power>=105||metrics.dpsRatio>=1.35))return 'A2';if(combatElapsed>=10&&ttk>=th.assistance&&(power<92||surv<.62))return 'A0';}if(metrics.initial&&metrics.initialTtk<=th.domination&&power>=145)return 'A3';if(metrics.initial&&metrics.initialTtk<=th.overpower&&power>=120)return 'A2';if(metrics.initial&&metrics.initialTtk>=th.assistance&&power<82)return 'A0';return 'A1';}
function hivePlanResponse(state,metrics,world=G?.sector||1){
  if(state==='A1')return {state,action:'none',label:'SILENCIO ADAPTATIVO',reason:'build dentro del rango esperado'};
  if(state==='A0'){const b=metrics.build||{},survivalWeak=(b.survival??1)<.62,offenseWeak=(metrics.dpsRatio??1)<.72||b.offensiveQuality<2.2,mobilityWeak=up('engine')<2&&!powerOn('overdrive');let pickup='power',label='RESPUESTA SIMBIÓTICA';if(survivalWeak){pickup=(b.shieldRatio??1)<.35?'shield':'heal';label='ADAPTACIÓN DE SUPERVIVENCIA';}else if(offenseWeak){pickup='weaponBoost';label='NUTRICIÓN DE LA COLMENA';}else if(mobilityWeak){pickup='mobilityBoost';label='IMPULSO SIMBIÓTICO';}return {state,action:'contextual-drop',label,pickup,maxPerBoss:2,minGap:18,requiresCollection:true,noFreeVictory:true};}
  const extreme=state==='A3',armor=extreme?(world>=16?.68:.72):((metrics.ttk||999)<hiveThresholds(world).overpower*.72?.78:.88),suppression=extreme&&world>=16?{label:world===20?'RIFT DISRUPTION':'INTERFERENCIA DE COLMENA',seconds:world===20?4.2:3.6,telegraph:true,avoidable:true,secondaryOnly:true}:null,rebirth=extreme&&world>=16?{enabled:true,once:true,hpRatio:world===20?.48:.40,chance:world===20?.72:world>=18?.45:.30,alternatePattern:true}:null;
  return {state,action:'boss-adaptation',label:extreme?'RESPUESTA EVOLUTIVA MÁXIMA':'ARMADURA REACTIVA',damageReceived:armor,minDamageReceived:.68,signaturePriority:extreme?1.35:1.18,defensiveMobility:true,tacticalSummons:true,softPhaseGate:{brief:true,mandatorySignature:extreme},suppression,rebirth};
}
function hiveMetrics(session=G?.hiveBossSession,b=G?.boss){if(!session||!b)return null;const now=G.elapsed||0,combatElapsed=Math.max(0,now-session.startAt-session.entryDuration),bossDps=hiveRecentDps('boss',HIVE_DIRECTOR.sampleWindow,now),build=hiveBuildSnapshot(b,bossDps>0?bossDps:null),fallback=session.predictedDps||build.expectedDps*.7,effectiveDps=bossDps>0?bossDps:fallback,ttk=effectiveDps>0?Math.max(0,b.hp)/effectiveDps:Infinity,dpsRatio=build.expectedDps>0?effectiveDps/build.expectedDps:0;return {initial:false,combatElapsed:+combatElapsed.toFixed(2),bossDps:+bossDps.toFixed(2),effectiveDps:+effectiveDps.toFixed(2),dpsRatio:+dpsRatio.toFixed(3),ttk:Number.isFinite(ttk)?+ttk.toFixed(2):null,powerIndex:build.index,build};}
function hiveStartBossSession(b=G?.boss){if(!G||!b)return null;hiveEnsureRuntime();const world=G.sector||1,priorDps=hiveRecentDps('world',HIVE_DIRECTOR.sampleWindow,G.elapsed||0),baseBuild=hiveBuildSnapshot(b,priorDps>0?priorDps:null),expected=baseBuild.expectedDps||1,modelDps=expected*clamp(baseBuild.index/100,.42,2.5),predictedDps=priorDps>0?modelDps*.65+priorDps*.35:modelDps,initialTtk=b.maxHp/Math.max(.01,predictedDps),metrics={initial:true,initialTtk,powerIndex:baseBuild.index,dpsRatio:predictedDps/expected,build:baseBuild,ttk:initialTtk};const state=hiveClassify(metrics,world,0),plan=hivePlanResponse(state,metrics,world);const hs={schema:1,mode:HIVE_DIRECTOR_MODE,version:VERSION,world,boss:b.name,gameMode:G.mode||'campaign',difficulty:G.runDifficulty||'normal',startedUtc:new Date().toISOString(),startAt:G.elapsed||0,entryDuration:b.entry||0,priorDps:+priorDps.toFixed(2),predictedDps:+predictedDps.toFixed(2),initialTtk:+initialTtk.toFixed(2),initialPowerIndex:baseBuild.index,maxPowerIndex:baseBuild.index,state,candidate:state,candidateT:0,lastStateAt:G.elapsed||0,maxState:state,stateTimeline:[{t:0,state,ttk:+initialTtk.toFixed(2),powerIndex:baseBuild.index}],responseHistory:[{t:0,state,wouldApply:plan}],lastPlan:plan,bossDamage:0,damageEvents:0,playerDamageReceived:0,supportsDelivered:0,reactiveArmorActivated:false,suppressions:0,resurrection:false,planned:{supports:state==='A0'?1:0,armor:state==='A2'||state==='A3'?1:0,suppressions:plan.suppression?1:0,resurrection:plan.rebirth?1:0},reviewT:HIVE_DIRECTOR.reviewEvery,finalized:false};G.hiveBossSession=hs;return hs;}
function hiveUpdateBossDirector(dt){const hs=G?.hiveBossSession,b=G?.boss;if(!hs||hs.finalized||!b||b.dead||b.dying||b.entry>0||(b.phaseTransitionT||0)>0)return;hs.reviewT-=dt;if(hs.reviewT>0)return;hs.reviewT=HIVE_DIRECTOR.reviewEvery;const m=hiveMetrics(hs,b);if(!m)return;hs.maxPowerIndex=Math.max(hs.maxPowerIndex||0,m.powerIndex);const next=hiveClassify(m,hs.world,m.combatElapsed),nextPlan=hivePlanResponse(next,m,hs.world);if(next===hs.state){hs.candidate=next;hs.candidateT=0;const a=JSON.stringify({action:hs.lastPlan?.action,pickup:hs.lastPlan?.pickup,damageReceived:hs.lastPlan?.damageReceived,suppression:!!hs.lastPlan?.suppression,rebirth:!!hs.lastPlan?.rebirth}),z=JSON.stringify({action:nextPlan.action,pickup:nextPlan.pickup,damageReceived:nextPlan.damageReceived,suppression:!!nextPlan.suppression,rebirth:!!nextPlan.rebirth});if(a!==z){hs.lastPlan=nextPlan;hs.responseHistory.push({t:+m.combatElapsed.toFixed(1),state:next,refined:true,wouldApply:nextPlan});}}else{if(hs.candidate!==next){hs.candidate=next;hs.candidateT=0;}hs.candidateT+=HIVE_DIRECTOR.reviewEvery;if(hs.candidateT>=HIVE_DIRECTOR.hysteresis&&(G.elapsed-hs.lastStateAt)>=HIVE_DIRECTOR.stateCooldown){hs.state=next;hs.lastStateAt=G.elapsed;hs.candidateT=0;if(hiveStateRank(next)>hiveStateRank(hs.maxState))hs.maxState=next;const plan=nextPlan;hs.lastPlan=plan;hs.stateTimeline.push({t:+m.combatElapsed.toFixed(1),state:next,ttk:m.ttk,powerIndex:m.powerIndex});hs.responseHistory.push({t:+m.combatElapsed.toFixed(1),state:next,wouldApply:plan});if(next==='A0')hs.planned.supports++;if(next==='A2'||next==='A3')hs.planned.armor++;if(plan.suppression)hs.planned.suppressions++;if(plan.rebirth)hs.planned.resurrection++;}}hs.lastMetrics=m;}
function hiveRecordIncomingDamage(amount){const hs=G?.hiveBossSession;if(!hs||hs.finalized||!Number.isFinite(amount)||amount<=0)return;hs.playerDamageReceived=(hs.playerDamageReceived||0)+amount;}
function hiveFinalizeBossSession(outcome='unknown'){
  const hs=G?.hiveBossSession;if(!hs||hs.finalized)return null;const b=G?.boss,now=G?.elapsed||0,metrics=b?hiveMetrics(hs,b):hs.lastMetrics,duration=Math.max(0,now-hs.startAt-hs.entryDuration);hs.finalized=true;const record={schema:1,version:VERSION,mode:HIVE_DIRECTOR_MODE,world:hs.world,boss:hs.boss,gameMode:hs.gameMode,difficulty:hs.difficulty,startedUtc:hs.startedUtc,dpsPrior:hs.priorDps,ttkInitialEstimated:hs.initialTtk,ttkReal:outcome==='victory'?+duration.toFixed(2):null,stateInitial:hs.stateTimeline[0]?.state||'A1',stateAdaptiveMax:hs.maxState||hs.state,powerIndexInitial:hs.initialPowerIndex,powerIndexMax:+(hs.maxPowerIndex||hs.initialPowerIndex).toFixed(1),bossDamage:+(hs.bossDamage||0).toFixed(2),damageEvents:hs.damageEvents||0,helpsDelivered:hs.supportsDelivered||0,reactiveArmorActivated:!!hs.reactiveArmorActivated,suppressions:hs.suppressions||0,resurrection:!!hs.resurrection,durationReal:+duration.toFixed(2),playerDamageReceived:+(hs.playerDamageReceived||0).toFixed(2),result:outcome,lastDps:metrics?.effectiveDps??0,lastTtk:metrics?.ttk??null,plannedResponses:hs.responseHistory,stateTimeline:hs.stateTimeline,shadowGuarantee:{gameplayInterventionsApplied:0,damageMultiplierApplied:1,powersSuppressed:false,bossResurrected:false,externalTransmission:false}};HIVE_TELEMETRY.records.push(record);hiveSaveTelemetry();hs.record=record;return record;
}
function hiveTelemetrySummary(){const records=HIVE_TELEMETRY.records||[],byWorld={};for(const r of records){const k=r.world;const q=byWorld[k]||(byWorld[k]={runs:0,wins:0,totalDuration:0,maxState:'A0'});q.runs++;if(r.result==='victory')q.wins++;q.totalDuration+=r.durationReal||0;if(hiveStateRank(r.stateAdaptiveMax)>hiveStateRank(q.maxState))q.maxState=r.stateAdaptiveMax;}for(const q of Object.values(byWorld))q.avgDuration=+(q.totalDuration/Math.max(1,q.runs)).toFixed(1);return {schema:1,mode:HIVE_DIRECTOR_MODE,count:records.length,byWorld};}
function hiveDirectorStatus(){const hs=G?.hiveBossSession,m=hs&&!hs.finalized&&G?.boss?hiveMetrics(hs,G.boss):null;return {version:VERSION,name:'ADAPTIVE HIVE BOSS DIRECTOR',alias:'RESPUESTA EVOLUTIVA DE LA COLMENA',mode:HIVE_DIRECTOR_MODE,shadow:true,gameplayNeutral:true,localOnly:true,sampleWindow:HIVE_DIRECTOR.sampleWindow,target:hiveTargetWindow(G?.sector||1),thresholds:hiveThresholds(G?.sector||1),current:hs?{world:hs.world,state:hs.state,maxState:hs.maxState,priorDps:hs.priorDps,initialTtk:hs.initialTtk,powerIndex:m?.powerIndex??hs.initialPowerIndex,dps:m?.effectiveDps??0,ttk:m?.ttk??null,wouldApply:hs.lastPlan}:null,telemetry:hiveTelemetrySummary(),criticalStatFabricated:false};}
...
function enterSector(n,keepPlayer=false){
  ...
  G.sector=clamp(n,1,SECTORS.length);G.hiveDamageSamples=[];G.hiveBossSession=null;streamPrepareWorld(G.sector,'enter-sector');G.wave=1;G.kills=0;G.goal=waveGoal(G.sector,1);G.powerMeter=0;G.powerDropsThisSector=0;G.spawn=.72;G.obstacleTimer=G.sector===1?2.8:2.2;G.frontTimer=G.mode==='training'?5.5:rnd(6.5,10.5);G.boss=null;G.bossAftermath=null;G.bossPending=false;G.preBossT=0;G.preBossMax=0;PreBossX.stop(false);G.sectorClear=false;G.postBossT=0;G.bossCheckpoint=false;
...
function damageEntity(e,dmg,kind='normal'){
  if(!e||e.dead)return;if(G?.weaponBoostT>0&&kind!=='bomb'&&kind!=='debug')dmg*=G.weaponBoostMult||1.35;if(kind!=='bomb'&&kind!=='debug')dmg*=combatDamageMult();if(e.kind==='objectiveNode'){e.hp-=dmg;e.flash=.07;if(e.hp<=0)destroyObjectiveTarget(e);return;}if(e===G.boss&&e.dying)return;if(e===G.boss&&relicUnlocked(10))dmg*=1.05;if(e===G.boss&&kind!=='debug')dmg*=bossCounterDamageMult();
  ...
  const hiveHpBefore=e.hp;e.hp-=dmg;hiveRecordOutgoingDamage(Math.max(0,Math.min(hiveHpBefore,Math.max(0,dmg))),e,kind);e.flash=.08;AudioX.hit();AudioX.impact(G?.level||1,e===G.boss);
...
function killBoss(){
  ...
  hiveFinalizeBossSession('victory');
...
function update(dt){
  ...
  updatePowers(dt);updatePlayer(dt);updatePlayerBehavior(dt);updateWorldDynamics(dt);updateWaveObjective(dt);updateSpawns(dt);updateEnemies(dt);updateFamilyManeuver(dt);updateBoss(dt);hiveUpdateBossDirector(dt);updateBossFamilyResponse(dt);updateObstacles(dt);updateFrontThreats(dt);updateBullets(dt);updateEnemyBullets(dt);updatePickups(dt);updateParticles(dt);
...
function updateBossSignatureAttack(b,p,sec,dt){
  ...
  if(!s){
    if(b.entry<=0&&(b.phaseTransitionT||0)<=0&&!b.telegraphT&&!b.chargeT&&!b.specialT&&b.signatureAttackCd<=0&&(b.phaseElapsed||0)>2.0){
      b.signatureAttack=makeBossSignature(b,p,cfg);bossAnimStartSignatureCue(b,cfg,cfg.name);b.animAttackT=Math.max(b.animAttackT,cfg.wind+.2);b.specialCd=Math.max(b.specialCd,cfg.wind+cfg.active+.35);G.bossWarningText=cfg.name;G.bossWarningT=Math.max(G.bossWarningT,cfg.wind);notify(`⚡ ${cfg.name}`,cfg.color,1.15);AudioX.bossWarn(G.sector-1);return true;
    }return false;
  }
...
window.__SWARM_HIVE_SHADOW_STATUS=()=>hiveDirectorStatus();
window.__SWARM_HIVE_TELEMETRY=()=>({summary:hiveTelemetrySummary(),records:[...(HIVE_TELEMETRY.records||[])]});
window.__SWARM_V21519_STATUS=()=>({version:VERSION,adaptiveHiveBossDirector:hiveDirectorStatus(),states:['A0 · ASISTENCIA','A1 · EQUILIBRIO','A2 · SOBREPOTENCIA','A3 · DOMINACIÓN'],inputs:['DPS efectivo reciente','poderes y rango','combos/fusiones','cadencia','crítico (hook reservado: sin stat nativo)','proyectiles simultáneos','aliados','escudo/supervivencia','mejoras permanentes','desempeño reciente','daño últimos 20s'],shadowMode:{enabled:true,interventionsApplied:false,recordsLocalOnly:true,externalTransmission:false},plannedResponses:['drops contextuales limitados','armadura reactiva 88→78→68% aprox.','prioridad signature','movilidad defensiva','invocaciones tácticas','puertas blandas de fase','supresión telegráfica 3–5s en avanzados','RIFT REBIRTH una vez en extremos'],preserved:{worlds:activeSectorCount(),bosses:20,hudMobileReadability:true,mobileCombatErgonomics:true,bossDeathAftermath:true,bossArenaPresentation:true,waveChoreographer:true,enemySonicEcology:true,bossAudioEventSync:true,bossIdentityPass:true,microSwarmFrom:1,riftAllyFrom:11,lastChanceSeconds:5,noHpInflationApplied:true,noPowerSuppressionApplied:true,noResurrectionApplied:true,assetStreamer:true,performanceGovernor:true,pwa:true}});
})();
