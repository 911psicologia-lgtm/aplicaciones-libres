...
const VERSION='2.12.1.3';
const KEY_META='swarm_rift_meta_v21213';
const KEY_RUN='swarm_rift_run_v21213';
const TAU=Math.PI*2;
...
// v2.12.1.3 · antesala final reducida al 50% sin alterar la transición musical.
const PREBOSS_DURATION=8.25;
...
let META=loadJSON(KEY_META,null)||loadJSON('swarm_rift_meta_v21212',null);
...
function spawnLieutenant(){
  ...
  const lieStamina=(72+G.sector*7+(hard?18:0))*COMMANDER_DIFFICULTY.stamina,e={x:W+90,y:H*.32,r,hp,maxHp:hp,spd:(145+G.sector*5)*COMMANDER_DIFFICULTY.move,score:1500+G.sector*220,form:prev.forms[2],family:prev.family,kind:'lieutenant',lieutenantSector:G.sector-1,pattern:prev.pattern,name:`TENIENTE ${prev.boss}`,phase:1,t:0,fireCd:.58,fireRate:.78,specialCd:2.05,dead:false,slow:0,flash:0,contact:(24+G.sector*2.4)*COMMANDER_DIFFICULTY.contact,entryT:1.1,entryTX:W*.74,entryTY:H*.34,guardT:0,telegraphT:0,chargeT:0,animPulse:0,phaseFlash:0,stamina:lieStamina,staminaMax:lieStamina,coreOpenT:0};G.enemies.push(e);notify(`TENIENTE RESISTENTE · ${prev.boss}`,prev.accent,2.1);AudioX.bossWarn(G.sector-2);return e;
}
...
function updateLieutenantEnemy(e,dt,p){
  ...
  const tempo=1/COMMANDER_DIFFICULTY.tempo,ratio=e.hp/e.maxHp;e.phase=ratio>.52?1:2;e.fireCd-=dt*tempo;e.specialCd-=dt*tempo;e.flash=Math.max(0,e.flash-dt);e.x+=(e.x>W*.78?-e.spd*.58:e.x<W*.56?e.spd*.24:0)*dt;e.y+=Math.sin(e.t*(2.2+e.phase*.25)+e.phase)*(72+e.phase*12)*dt;e.y=clamp(e.y,55,H-55);
...
function spawnPickup(x,y,type='credit',key=null){
  ...
  const q={x,y,vx:rnd(-20,35),vy:rnd(-55,55),r:type==='power'?15:type==='fragment'?12:10,type,key,t:0,life:16,value:1};G.pickups.push(q);return q;
}
...
function enemyShoot(e,style='aim'){const p=G.player,a=Math.atan2(p.y-e.y,p.x-e.x),commander=e===G?.boss||e?.kind==='lieutenant',fx=commander?commanderProjectileFx(e,style==='spread'?'volley':'primary'):null;const col=commander?(COMMANDER_PROJECTILES[e.pattern]?.col||'#ff6b78'):(e.family==='MOSQUITOS'?'#ff6175':e.family==='LIBÉLULAS'?'#78ecff':e.family==='CIGARRAS'?'#d9a7ff':'#ff6b78');if(style==='spread'){for(let k=-1;k<=1;k++)spawnEnemyBullet(e.x,e.y,a+k*.19,235+G.sector*4,8,5,col,e,fx?{...fx,phase:(fx.phase||0)+k*.45}:null);}else spawnEnemyBullet(e.x,e.y,a,260+G.sector*5,9,5,col,e,fx);}
...
function ringBullets(x,y,count,speed,dmg,col='#ff768e',offset=0,source=null,role='ring'){const fx=source?commanderProjectileFx(source,role,true):null;for(let i=0;i<count;i++){const a=offset+i/count*TAU;spawnEnemyBullet(x,y,a,speed,dmg,5,col,source,fx?{...fx,phase:(fx.phase||0)+i/count*TAU}:null);}}
function commanderFan(source,x,y,base,count,spread,speed,dmg,r=5,role='special',legacyUnscaled=true){const col=COMMANDER_PROJECTILES[source?.pattern]?.col||'#ff768e',fx=commanderProjectileFx(source,role,legacyUnscaled);for(let i=0;i<count;i++){const k=i-(count-1)/2;spawnEnemyBullet(x,y,base+k*spread,speed,dmg,r,col,source,fx?{...fx,phase:(fx.phase||0)+i*.28}:null);}}
...
function update(dt){
  ...
  if(G.preBossT>0){G.preBossT=Math.max(0,G.preBossT-dt);const preMax=Math.max(.1,G.preBossMax||PREBOSS_DURATION),elapsed=preMax-G.preBossT;if(elapsed>preMax*.21)preBossCue(1);if(G.preBossT<preMax*.49)preBossCue(2);if(G.preBossT<preMax*.20)preBossCue(3);updatePowers(dt);updatePlayer(dt);updatePickups(dt);updatePreBossSetpiece(dt);updateAmbientProps(dt);updateParticles(dt);if(G.preBossT<=0&&G.bossPending&&!G.boss)spawnBoss();return;}
