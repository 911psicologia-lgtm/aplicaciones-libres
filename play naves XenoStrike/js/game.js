...
const KEY_META='swarm_rift_meta_v2141';
const KEY_RUN='swarm_rift_run_v2141';
const TAU=Math.PI*2;
...
for(const [key,file] of Object.entries({phase:'phase_lance.png',nanites:'nanite_swarm.png',riftmine:'rift_mine.png',echo:'temporal_echo.png',nova:'entomic_nova.png'})){const im=new Image();im.src=`assets/powers_hangar/${file}?av=2141`;IMG.generatedPowers[key]=im;}
...
const POWER_WORLD_FOCUS={
  ...
  6:['gravity','rail','overdrive','tesla','sparklaser','riftmine'],
  7:['hemadrain','missile','overdrive','shield','bio','nanites'],
  8:['resinwall','drone','shield','bomb','bio','echo'],
  9:['prismburst','rail','tesla','burst','sparklaser','phase','echo'],
  10:['resonance','gravity','prismburst','hemadrain','sparklaser','bio','nova','phase']
};
...
META.upgrades=META.upgrades||{};META.bossUnlocks=META.bossUnlocks||{};META.defeated=META.defeated||{};META.bossMastery=META.bossMastery||{};META.bestWaveGrade=META.bestWaveGrade||{};META.relics=META.relics||{};META.supplies=META.supplies||{};META.powerStock=META.powerStock||{};META.powerBranches=META.powerBranches||{};
...
function phaseLance(){
  ...
  G.bullets.push({x:p.x+Math.cos(a)*26,y:p.y+Math.sin(a)*26,vx:Math.cos(a)*spd,vy:Math.sin(a)*spd,r:4.6,dmg,life:1.15,col:'#72f6ff',type:'phase',pierce:2+Math.floor(r/2),trail:52+r*5});
...
function temporalEchoVolley(){
  ...
  for(let i=0;i<n;i++){const off=(i-(n-1)/2)*spread,a=base+off;G.bullets.push({x:p.x-12,y:p.y,vx:Math.cos(a)*(720+r*24),vy:Math.sin(a)*(720+r*24),r:3.5,dmg,life:1.5,col:'#b9a6ff',type:'spark',pierce:r>=6?1:0,trail:26});}
...
function drawPowerMines(){
  for(const m of G.powerMines||[]){cx.save();cx.translate(m.x,m.y);cx.rotate(m.pulse*.45);cx.globalAlpha=m.arm>0?.45:.82+.15*Math.sin(m.pulse*3);cx.strokeStyle=m.col;cx.fillStyle='rgba(255,176,100,.12)';cx.lineWidth=2;cx.beginPath();for(let i=0;i<6;i++){const a=i*TAU/6,x=Math.cos(a)*m.r,y=Math.sin(a)*m.r;i?cx.lineTo(x,y):cx.moveTo(x,y);}cx.closePath();cx.fill();cx.stroke();cx.beginPath();cx.arc(0,0,m.r*.34,0,TAU);cx.fillStyle=m.col;cx.fill();cx.restore();}
}
...
function powerDropPool(){
  ...
  if(sector>=6)pool.push('sparklaser','riftmine');
...
function activatePower(key,source='pickup',heritageSeconds=null){
  ...
  const durationBase=heritageSeconds!=null?heritageSeconds:p.duration,duration=durationBase*POWER_DURATION_MULT*(1+up('reactor')*.05)*(1+(rank-1)*.09);
  if(G.powers[key]){const extension=duration*.45,capSeconds=p.duration*POWER_DURATION_MULT*(1+up('reactor')*.05)*(1.70+(rank>=4?.10:0)+(rank>=5?.10:0)+(rank>=6?.12:0)+(rank>=7?.12:0));G.powers[key]=Math.min(capSeconds,(G.powers[key]||0)+extension);if(key==='resinwall')G.resinCharges=Math.min(6,(G.resinCharges||0)+1);AudioX.power(key);notify(`${p.icon} ${p.name} ${rankRoman(rank)} · ${Math.ceil(G.powers[key])}s`,p.color,1.8);burst(G.player.x,G.player.y,p.color,18+rank*2,140);powerActivationVfx(key);updateComboState();return;}
...
// UPDATE LOOP
// ─────────────────────────────────────────────────────────────
let timers={missile:0,rail:0,tesla:0,gravity:0,drone:0,resonance:0,sparklaser:0,bio:0,phase:0,nanites:0,riftmine:0,echo:0,nova:0};
// ─────────────────────────────────────────────────────────────
...
function updatePowers(dt){
  ...
  timers.missile-=dt;timers.rail-=dt;timers.tesla-=dt;timers.gravity-=dt;timers.drone-=dt;timers.resonance-=dt;timers.sparklaser-=dt;timers.bio-=dt;timers.phase-=dt;timers.nanites-=dt;timers.riftmine-=dt;timers.echo-=dt;timers.nova-=dt;
...
function drawShip(){
  ...
  if(powerOn('nanites')){cx.save();cx.fillStyle='#93ff9b';cx.globalAlpha=.55;const n=5+Math.min(4,powerRank('nanites'));for(let i=0;i<n;i++){const a=t*2.8+i*TAU/n;cx.beginPath();cx.arc(p.x+Math.cos(a)*(p.r+25),p.y+Math.sin(a)*(p.r+18),1.8+(i%2),0,TAU);cx.fill();}cx.restore();}
  if(powerOn('echo')){cx.save();cx.globalAlpha=.15;for(let i=1;i<=2;i++){cx.strokeStyle='#b9a6ff';cx.strokeRect(p.x-36-i*10,p.y-17-i*2,30,34+i*4);}cx.restore();}
  if(powerOn('phase')){cx.save();cx.globalAlpha=.32+.12*Math.sin(t*9);cx.strokeStyle='#72f6ff';cx.lineWidth=2;cx.beginPath();cx.moveTo(p.x+18,p.y-8);cx.lineTo(p.x+46,p.y);cx.lineTo(p.x+18,p.y+8);cx.stroke();cx.restore();}
  if(powerOn('nova')){cx.save();cx.globalAlpha=.16+.06*Math.sin(t*6);cx.strokeStyle='#ff8be2';cx.beginPath();cx.arc(p.x,p.y,p.r+34+Math.sin(t*4)*5,0,TAU);cx.stroke();cx.restore();}
...
function drawPowerAura(key,col,t,r){cx.save();cx.strokeStyle=col;cx.fillStyle=col;cx.globalAlpha=.32+.18*Math.sin(t*7);cx.lineWidth=2;
  ...
  cx.restore();}
function powerActivationVfx(key){const pd=POWERS[key];if(!pd||!G)return;const p=G.player;G.particles.push({kind:key==='gravity'?'shock':'halo',x:p.x,y:p.y,r:12,vr:key==='gravity'?520:330,life:.55,max:.55,col:pd.color});if(['tesla','rail','burst','twin','bomb','hemadrain','resinwall','prismburst','resonance','sparklaser','bio','phase','nanites','riftmine','echo','nova'].includes(key))for(let i=0;i<10;i++){const a=i/10*TAU;G.particles.push({kind:'spark',x:p.x,y:p.y,vx:Math.cos(a)*rnd(70,180),vy:Math.sin(a)*rnd(70,180),r:rnd(1.2,2.8),life:rnd(.18,.38),max:.38,col:pd.color});}}
...
function drawHUD(){
  ...
  if(compact){
    ...
    for(const [k,v] of active.slice().reverse()){const pd=POWERS[k];const txt=`${pd.icon}${rankRoman(powerRank(k))} ${Math.ceil(v)}`;cx.fillStyle=pd.color;if(rx< W*.68){rx=W-12;y=topY+topH+10;cx.font='800 8px system-ui';}cx.fillText(txt,rx,y);rx-=Math.min(62,cx.measureText(txt).width+9);}
  ...
  if((G.weaponBoostT||0)>0){cx.fillStyle='rgba(2,7,17,.74)';cx.strokeStyle='rgba(255,207,115,.60)';rr(W/2-92,H-112,184,24,9);cx.fill();cx.stroke();cx.fillStyle='#ffcf73';cx.font='900 10px system-ui';cx.textAlign='center';cx.fillText(`⚡ IMPACTO +${Math.round(((G.weaponBoostMult||1)-1)*100)}% · ${Math.ceil(G.weaponBoostT)}s`,W/2,H-99);}const active=Object.entries(G.powers).filter(([,v])=>v>0).sort((a,b)=>b[1]-a[1]).slice(0,G.maxActivePowers||activePowerSlotLimit());const gap=7,ww=82,tot=active.length*ww+Math.max(0,active.length-1)*gap;let x=W/2-tot/2;for(const [k,v] of active){const pd=POWERS[k],br=powerBranch(k);cx.fillStyle='rgba(2,7,17,.74)';cx.strokeStyle=hexA(pd.color,.55);rr(x,H-44,ww,30,9);cx.fill();cx.stroke();cx.fillStyle=pd.color;cx.font='900 10px system-ui';cx.textAlign='center';cx.fillText(`${pd.icon} ${rankRoman(powerRank(k))}${br?br:''} · ${Math.ceil(v)}s`,x+ww/2,H-28);x+=ww+gap;}
  const q=(G.powerQueue||[]).slice(0,G.maxQueuePowers||queuedPowerSlotLimit());let qx=12,qy=H-82;for(const k of q){const pd=POWERS[k],br=powerBranch(k);if(qx+74>W-10){qx=12;qy-=28;}cx.fillStyle='rgba(2,7,17,.66)';cx.strokeStyle=hexA(pd.color,.4);rr(qx,qy,74,24,8);cx.fill();cx.stroke();cx.fillStyle=pd.color;cx.font='800 9px system-ui';cx.textAlign='center';cx.fillText(`⏳ ${pd.icon}${rankRoman(powerRank(k))}${br||''} ${pd.name.slice(0,5)}`,qx+37,qy+13);qx+=80;}
...
const POWER_SHOP_WORLD={twin:1,shield:1,magnet:1,cryo:2,acid:2,drone:2,missile:3,tesla:3,rail:3,burst:3,bomb:3,overdrive:4,phase:4,gravity:5,nanites:5,sparklaser:6,riftmine:6,bio:7,hemadrain:7,resinwall:8,echo:8,prismburst:9,resonance:10,nova:10};
const SUPPLY_DEFS={
...
function shopCatalog(tab=storeTab){
  ...
  let out=tab==='powers'?powers:tab==='support'?supplies:tab==='evolution'?evolution:tab==='upgrades'?standardUpgrades:[...powers,...supplies,...upgrades,...branches];out.forEach(i=>i.smart=smartItemScore(i));if(tab==='smart'){const open=out.filter(i=>!i.maxed);out=(open.length?open:out).sort((a,b)=>b.smart-a.smart).slice(0,mobileUI()?8:12);}return out;
}
...
function drawStore(){
  UI.buttons.length=0;drawBackground();cx.fillStyle='rgba(2,6,14,.91)';cx.fillRect(0,0,W,H);const mobile=mobileUI(),portrait=portraitUI(),m=mobile?10:22;cx.textAlign='left';cx.fillStyle='#ffb7e8';cx.font=`950 ${mobile?18:27}px system-ui`;cx.fillText(portrait?'HANGAR // TÁCTICO':'HANGAR // COMPRA TÁCTICA',m,mobile?22:34);cx.fillStyle='#ffd76a';cx.font=`800 ${mobile?8:11}px system-ui`;const wallet=storeInRun()?`¤ ${G.credits.toLocaleString()} · XP ${Math.round(G.xp||0).toLocaleString()}`:`¤ ${G.credits.toLocaleString()} · RESERVAS PERSISTENTES`;cx.fillText(`${wallet} · ACTIVOS ${activePowerSlotLimit()} · COLA ${queuedPowerSlotLimit()} · CAP ${rankRoman(powerRankCap(storeContextSector()))}`,m,mobile?39:55);
  const tabs=[['smart','★ SMART'],['powers','✦ PODERES'],['evolution','◇ EVOL.'],['support','♥ SOPORTE'],['upgrades','⬢ MEJORAS']],tabY=mobile?47:66,gap=mobile?3:7,tw=(W-m*2-gap*4)/5,th=mobile?27:32;tabs.forEach(([id,label],i)=>uiButton(`store_tab_${id}`,label,m+i*(tw+gap),tabY,tw,th,storeTab===id?'#fff09a':id==='evolution'?'#d9a7ff':'#8edbff'));
...
function drawGuide(){
  ...
  const pages=[
    ...
    {title:'COMBATE',cards:[['PODERES Y COMBOS','Tres poderes activos de base; el Hangar permite cuatro y hasta seis en reserva. Evoluciones VI/VII con Matriz.','powers'],['AMENAZAS FRONTALES','Meteoros, arietes, cápsulas y restos llegan hacia cámara. Interceptarlos pronto paga mejor.','front'],['JEFES Y FASES',`Los ${activeSectorCount()} jefes activos mutan en tres fases y abren brevemente el núcleo tras ataques importantes.`,'boss']]},
...
window.__SWARM_V2141_HANGAR_STATUS=()=>({version:VERSION,powers:{total:POWER_KEYS.length,activeBase:POWER_SLOT_LIMIT,activeMax:activePowerSlotLimit(),queueBase:POWER_QUEUE_LIMIT,queueMax:queuedPowerSlotLimit(),rankCap:powerRankCap(),rankLabel:rankRoman(powerRankCap()),dropBudget:powerDropBudget()},branches:{unlocked:powerBranchUnlocked(),available:Object.keys(POWER_BRANCHES).length,selected:{...(META.powerBranches||{})}},hangar:{evolutionUpgrades:[...EVOLUTION_UPGRADE_IDS],reactor:up('reactor'),powerbay:up('powerbay'),reservebay:up('reservebay'),evolution:up('evolution')},hud:{showsAllActive:true,showsQueue:true,branchBadges:true},chapterIIStarted:false});
})();
