...
const KEY_META='swarm_rift_meta_v21517';
const KEY_RUN='swarm_rift_run_v21517';
const TAU=Math.PI*2;
...
function drawCompactHud(){
  ...
  const actionReserve=mobile?Math.min(212,W*.29):Math.min(238,W*.25),left=6,top=6,panelW=Math.max(250,Math.min(mobile?530:640,W-actionReserve-18)),panelH=42;
  ...
  if(showFocus){const fw=Math.min(W-24,mobile?430:520),fy=top+panelH+6;cx.fillStyle=critical?'rgba(45,3,10,.84)':'rgba(2,7,17,.78)';cx.strokeStyle=hexA(priority.color,.55);rr(W/2-fw/2,fy,fw,24,9);cx.fill();cx.stroke();cx.fillStyle=priority.color;cx.font='900 9.5px system-ui';cx.textAlign='center';cx.fillText(fitNoticeText(priority.text,fw-18),W/2,fy+13);}
