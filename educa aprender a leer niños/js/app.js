(function(){
  ...
  function go(screen){
    ...
    const map={onboarding:EmiliaScreens.onboarding,home:EmiliaScreens.home,book:EmiliaScreens.book,practice:EmiliaScreens.practice,treasures:EmiliaScreens.treasures,notebook:EmiliaScreens.notebook,gate:EmiliaScreens.gate,adult:EmiliaScreens.adult};
  ...
  function startMission(id){tryFullscreen();const m=EmiliaEngine.missionById(id);if(!EmiliaEngine.isUnlocked(m)){toast('Ese lugar todavía está dormido.');return;}const saved=EmiliaStore.get().activeSession;if(saved&&saved.kind==='mission'&&saved.missionId===id&&saved.index<saved.activities.length){session=saved;EmiliaStore.event('mission_resume_from_map',{missionId:id,index:session.index,total:session.activities.length});EmiliaScreens.activity(session);return;}session=EmiliaEngine.buildSession(m);persistSession();EmiliaScreens.activity(session);}
  ...
  function startUnlockBridgePractice(){tryFullscreen();session=EmiliaEngine.buildUnlockBridgeSession();persistSession();EmiliaStore.event('unlock_bridge_start',{skill:session.bridgeSkill||'',targetMission:session.bridgeTargetMission||'',count:session.activities.length});EmiliaScreens.activity(session);}
  ...
  function startPuzzlePractice(){tryFullscreen();session=EmiliaEngine.buildPuzzlePracticeSession();persistSession();EmiliaStore.event('ling_puzzle_start',{count:session.activities.length});EmiliaScreens.activity(session);}
  function startDotPractice(){tryFullscreen();session=EmiliaEngine.buildDotPracticeSession();persistSession();EmiliaStore.event('ling_dots_start',{count:session.activities.length});EmiliaScreens.activity(session);}
  ...
  function exitMission(){EmiliaVoice.stop&&EmiliaVoice.stop();persistSession();if(session&&session.kind==='mission')sessionStorage.setItem('emiliaResumeGuide','1');go('home');}
  ...
  function spokenInstruction(q){
    ...
    if(q.type==='lingPuzzle')return q.voicePrompt||'Arma la imagen y descubre la palabra.';
    if(q.type==='lingDots')return q.voicePrompt||'Toca los puntos en orden y descubre la palabra.';
  ...
  function bindActivity(q,s){
    ...
    if(q.type==='lingPuzzle'){
      const board=document.getElementById('lingPuzzleBoard'),pieces=[...document.querySelectorAll('.ling-puzzle-piece')],reveal=document.getElementById('lingPuzzleReveal'),read=document.getElementById('lingPuzzleRead'),hint=document.getElementById('lingPuzzleHint'),replay=isReplay();let selected=null,swaps=0,done=false,readDone=false;
      const say=()=>playInstruction(q,{button:document.getElementById('speakQ'),lockSelector:'.ling-puzzle-piece'});const sb=document.getElementById('speakQ');if(sb)sb.onclick=say;say();
      const n=pieces.length,cols=n===4?2:3,rows=Math.ceil(n/cols),seed=(Number(EmiliaStore.get().sessions||0)+String(q.word||'').length)%Math.max(1,n),order=Array.from({length:n},(_,i)=>(i+seed+1)%n);if(order.every((v,i)=>v===i)&&n>1){[order[0],order[1]]=[order[1],order[0]];}
      const paint=()=>pieces.forEach((b,pos)=>{const tile=Number(b.dataset.tile),c=tile%cols,r=Math.floor(tile/cols);b.style.backgroundImage=`url("${q.src}")`;b.style.backgroundSize=`${cols*100}% ${rows*100}%`;b.style.backgroundPosition=`${cols===1?0:(c/(cols-1))*100}% ${rows===1?0:(r/(rows-1))*100}%`;b.classList.toggle('selected',selected===b);});
      pieces.forEach((b,i)=>b.dataset.tile=String(order[i]));paint();
      const check=async()=>{if(!pieces.every((b,i)=>Number(b.dataset.tile)===i))return;done=true;pieces.forEach(b=>b.disabled=true);board.classList.add('complete');if(reveal)reveal.hidden=false;if(hint)hint.textContent='¡Imagen completa! Ahora toca Leer.';EmiliaVoice.tone('ok');feedback(true);EmiliaStore.event('ling_puzzle_complete',{word:q.word,pieces:n,swaps});};
      pieces.forEach(b=>b.onclick=()=>{if(done)return;if(!selected){selected=b;paint();return;}if(selected===b){selected=null;paint();return;}const a=selected.dataset.tile;selected.dataset.tile=b.dataset.tile;b.dataset.tile=a;swaps++;selected=null;paint();check();});
      if(read)read.onclick=async()=>{if(!done||readDone)return;readDone=true;await learningAudio(q.word,{kind:'word',repeat:false,button:read,listeningText:'👂',readyText:'●'});const assisted=swaps>Math.max(3,n);if(!replay){if(q.skill)EmiliaMastery.record(q.skill,true,assisted,{lingPuzzle:true});s.hits++;if(!assisted){s.independentHits++;registerSuccess(true);}else registerMiss();}rewardBurst('star');nextAfter(300);};
    }
    if(q.type==='lingDots'){
      const dots=[...document.querySelectorAll('.ling-dot')],slots=[...document.querySelectorAll('.ling-dots-word span')],svg=document.getElementById('lingDotsLines'),reveal=document.getElementById('lingDotsReveal'),replay=isReplay(),units=q.units||[];let pos=0,misses=0,done=false;
      const say=()=>playInstruction(q,{button:document.getElementById('speakQ'),lockSelector:'.ling-dot'});const sb=document.getElementById('speakQ');if(sb)sb.onclick=say;say();
      const point=b=>({x:parseFloat(b.style.left)||0,y:parseFloat(b.style.top)||0});
      const line=(a,b)=>{if(!svg)return;const A=point(a),B=point(b),el=document.createElementNS('http://www.w3.org/2000/svg','line');el.setAttribute('x1',A.x);el.setAttribute('y1',A.y);el.setAttribute('x2',B.x);el.setAttribute('y2',B.y);el.setAttribute('class','ling-dot-line');svg.appendChild(el);};
      dots.forEach(btn=>btn.onclick=async()=>{if(done)return;const i=Number(btn.dataset.i);if(i!==pos){misses++;btn.classList.add('bad');dots[pos]&&dots[pos].classList.add('hint');EmiliaVoice.tone('bad');setTimeout(()=>{btn.classList.remove('bad');dots[pos]&&dots[pos].classList.remove('hint');},420);return;}btn.classList.add('done');if(pos>0)line(dots[pos-1],btn);if(slots[pos])slots[pos].textContent=units[pos];const u=String(units[pos]||'');await learningAudio(u,{kind:u.length>1?'syllable':(/^[aeiouáéíóú]$/i.test(u)?'vowel':'letterName'),repeat:false,button:null,listeningText:'👂',readyText:'●'});pos++;if(pos>=dots.length){done=true;if(reveal)reveal.hidden=false;EmiliaVoice.tone('ok');feedback(true);const assisted=misses>0;if(!replay){if(q.skill)EmiliaMastery.record(q.skill,true,assisted,{lingDots:true});s.hits++;if(!assisted){s.independentHits++;registerSuccess(true);}else registerMiss();}EmiliaStore.event('ling_dots_complete',{word:q.word,misses});await learningAudio(q.word,{kind:'word',repeat:false,button:null,listeningText:'👂',readyText:'●'});rewardBurst('star');nextAfter(420);}});
    }
    if(q.type==='wordWrite'){
      const canvas=document.getElementById('wordWriteCanvas'),again=document.getElementById('wordWriteAgain'),againMini=document.getElementById('wordWriteAgainMini'),action=document.getElementById('wordWriteAction'),ready=document.getElementById('wordWriteReady'),help=document.getElementById('wordWriteHelp'),hint=document.getElementById('wordWriteHint');let completed=false,assisted=false,logged=false,tracer=null;const replay=isReplay(),stage=q.writeStage||'copy';
      const say=(helpUsed=false)=>{if(helpUsed)assisted=true;return playInstruction(q,{button:document.getElementById('speakQ'),lockSelector:'#wordWriteAction,#wordWriteHelp'});};const sb=document.getElementById('speakQ');if(sb)sb.onclick=()=>say(true);if(q.autoSpeak!==false)say(false);else{const st=document.getElementById('listenStatus');if(st)st.textContent='👀 Primero tú · 👂 si lo necesitas';}
      ...
      const complete=async(cov,meta={})=>{if(completed)return;completed=true;const ink=tracer&&tracer.exportInk?tracer.exportInk():[];if(!logged){logged=true;EmiliaStore.event('word_write_complete',{word:q.word,skill:q.skill,coverage:Math.round((cov||0)*100),manual:!!meta.manual,assisted,stage});if(!replay&&ink.length)EmiliaStore.addNotebookEntry({kind:'word',text:String(q.word||'').toLocaleLowerCase('es'),skill:q.skill,assisted,stage,ink:[{label:q.word,strokes:ink}]});}EmiliaVoice.tone('ok');feedback(true);if(!replay){EmiliaMastery.record(q.skill,true,assisted,{writing:true,writingStage:stage});s.hits++;if(!assisted){s.independentHits++;registerSuccess(true);}else registerMiss();}await learningAudio(q.word,{kind:'word',repeat:false,button:null,listeningText:'👂',readyText:'●'});if(action){action.disabled=false;action.textContent='➜';action.classList.add('is-next');}};
      const progress=pr=>{if(completed)return;if(action)action.disabled=!pr.canComplete;if(ready){ready.hidden=!pr.autoComplete;ready.classList.toggle('on',!!pr.autoComplete);}};
      if(canvas)tracer=EmiliaTracing.startWord(canvas,q.word,complete,progress,{guide:q.guide||'minimal'});
      if(help)help.onclick=()=>{assisted=true;if(hint){hint.hidden=false;hint.classList.add('show');}tracer&&tracer.setGuide(stage==='copy'?'full':'faded');EmiliaStore.event('word_write_help',{word:q.word,skill:q.skill,stage});EmiliaVoice.speak(q.word,{kind:'word',repeat:false});};
    ...
    if(q.type==='sentenceWrite'){
      const desktopCanvas=document.getElementById('sentenceWriteCanvas'),again=document.getElementById('sentenceWriteAgain'),againMini=document.getElementById('sentenceWriteAgainMini'),action=document.getElementById('sentenceWriteAction'),ready=document.getElementById('sentenceWriteReady'),wordReady=document.getElementById('sentenceWriteWordReady'),help=document.getElementById('sentenceWriteHelp'),hint=document.getElementById('sentenceWriteHint'),wordLabel=document.getElementById('sentenceWriteWordLabel');
      let mobileCanvas=document.getElementById('sentenceWriteWordCanvas'),completed=false,assisted=false,logged=false,tracer=null,wordIndex=0,wordComplete=false,mobileInk=[];const replay=isReplay(),phrase=String(q.sentence||q.say||'').trim(),words=phrase.replace(/[.!?]+$/,'').split(/\s+/).filter(Boolean),mobileMode=!!(q.responsiveWords&&mobileCanvas&&window.matchMedia&&window.matchMedia('(max-width:620px)').matches),tokens=[...document.querySelectorAll('.sentence-mobile-token')];
      const noteHelp=kind=>{assisted=true;if(q.trackAudioHelp)EmiliaStore.event('activity_audio_help',{activity:q.id,skill:q.skill,kind:kind||q.type});};
      const say=(helpUsed=true)=>{if(helpUsed)noteHelp('sentenceWriteAudio');return playInstruction(q,{button:document.getElementById('speakQ'),lockSelector:'#sentenceWriteAction,#sentenceWriteHelp'});};const sb=document.getElementById('speakQ');if(sb)sb.onclick=()=>say(true);if(q.autoSpeak!==false)say(false);else{const st=document.getElementById('listenStatus');if(st){st.textContent='👀 Primero tú · 👂 si lo necesitas';st.classList.remove('listening');st.classList.add('ready');}}
      const markFinal=async(cov,meta={})=>{if(completed)return;completed=true;tokens.forEach(t=>t.classList.remove('current'));if(wordLabel&&mobileMode)wordLabel.textContent='Frase completa';if(!logged){logged=true;const desktopInk=!mobileMode&&tracer&&tracer.exportInk?tracer.exportInk():[];EmiliaStore.event('sentence_write_complete',{sentence:phrase,skill:q.skill,coverage:Math.round((cov||0)*100),manual:!!meta.manual,assisted,mobileWordByWord:mobileMode,words:words.length});if(!replay){const ink=mobileMode?mobileInk:(desktopInk.length?[{label:phrase,strokes:desktopInk}]:[]);if(ink.length)EmiliaStore.addNotebookEntry({kind:'sentence',text:phrase,skill:q.skill,assisted,stage:q.readingMode||'supported',mobileWordByWord:mobileMode,ink});}}EmiliaVoice.tone('ok');feedback(true);if(!replay){EmiliaMastery.record(q.skill,true,assisted,{writing:true,sentenceWriting:true,responsiveWords:mobileMode});s.hits++;if(!assisted){s.independentHits++;registerSuccess(true);}else registerMiss();}await learningAudio(phrase,{kind:'sentence',repeat:false,button:null,listeningText:'👂',readyText:'●'});if(action){action.disabled=false;action.textContent='➜';action.setAttribute('aria-label','Siguiente');action.classList.add('is-next');}};
      const desktopProgress=pr=>{if(completed)return;if(action)action.disabled=!pr.canComplete;if(ready){ready.hidden=!pr.autoComplete;ready.classList.toggle('on',!!pr.autoComplete);}};
      const setMobileTokenState=()=>{tokens.forEach((t,i)=>{t.classList.toggle('current',i===wordIndex&&!completed);t.classList.toggle('done',i<wordIndex||(completed&&i<=wordIndex));});if(wordLabel)wordLabel.textContent=`Palabra ${Math.min(wordIndex+1,words.length)} de ${words.length}`;};
      const freshMobileCanvas=()=>{if(!mobileCanvas)return null;const clone=mobileCanvas.cloneNode(true);mobileCanvas.replaceWith(clone);mobileCanvas=clone;return mobileCanvas;};
      const startMobileWord=()=>{
        if(!mobileMode||completed||!words[wordIndex])return;wordComplete=false;if(wordReady){wordReady.hidden=true;wordReady.classList.remove('on');}if(action){action.disabled=true;action.textContent='✓ Palabra';action.setAttribute('aria-label','Completar palabra');action.classList.remove('is-next');}setMobileTokenState();const c=freshMobileCanvas();if(!c)return;
        const word=words[wordIndex],progress=pr=>{if(completed||wordComplete)return;if(action)action.disabled=!pr.canComplete;if(wordReady){wordReady.hidden=!pr.autoComplete;wordReady.classList.toggle('on',!!pr.autoComplete);}};
        tracer=EmiliaTracing.startWord(c,word,async(cov,meta={})=>{if(wordComplete||completed)return;wordComplete=true;const wordInk=tracer&&tracer.exportInk?tracer.exportInk():[];if(wordInk.length)mobileInk.push({label:word,strokes:wordInk});tokens[wordIndex]&&tokens[wordIndex].classList.add('done');if(wordReady){wordReady.hidden=false;wordReady.classList.add('on');}EmiliaStore.event('sentence_word_write_complete',{sentence:phrase,word,wordIndex,skill:q.skill,coverage:Math.round((cov||0)*100),assisted});await learningAudio(word,{kind:'word',repeat:false,button:null,listeningText:'👂',readyText:'●'});if(wordIndex>=words.length-1){await markFinal(cov,Object.assign({},meta,{wordByWord:true}));}else if(action){action.disabled=false;action.textContent='➜ palabra';action.setAttribute('aria-label','Siguiente palabra');action.classList.add('is-next');}},progress,{guide:assisted?'faded':'minimal',preserveCase:true});
      };
      const reset=()=>{if(completed)return;if(hint){hint.hidden=true;hint.classList.remove('show');}if(mobileMode){wordComplete=false;startMobileWord();return;}if(ready){ready.hidden=true;ready.classList.remove('on');}if(action){action.disabled=true;action.textContent='✓ Completar';action.classList.remove('is-next');}tracer&&tracer.setGuide('minimal');tracer&&tracer.reset();};
      if(mobileMode){startMobileWord();}else if(desktopCanvas){tracer=EmiliaTracing.startSentence(desktopCanvas,phrase,markFinal,desktopProgress,{guide:'minimal'});}
      if(help)help.onclick=()=>{noteHelp('sentenceWriteHint');if(hint){hint.hidden=false;hint.classList.add('show');}tracer&&tracer.setGuide('faded');EmiliaStore.event('sentence_write_help',{sentence:phrase,skill:q.skill,mobileWordByWord:mobileMode,word:mobileMode?words[wordIndex]:''});if(mobileMode&&words[wordIndex])EmiliaVoice.speak(words[wordIndex],{kind:'word',repeat:false});else EmiliaVoice.speak(phrase,{kind:'sentence',repeat:false});};
      [again,againMini].forEach(b=>{if(b)b.onclick=reset;});
      if(action)action.onclick=()=>{if(completed){nextAfter(80);return;}if(mobileMode){if(wordComplete){wordIndex++;startMobileWord();}else tracer&&tracer.forceComplete();}else tracer&&tracer.forceComplete();};
    }
    if(q.type==='build'){
  ...
  function exportProgress(){const blob=new Blob([EmiliaStore.exportJSON()],{type:'application/json'}),u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download='emilia_bosque_v1.10.0_'+new Date().toISOString().slice(0,10)+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(u),1000);toast('Copia exportada.');}
  ...
  window.EmiliaApp={go,startMission,startPractice,startUnlockBridgePractice,startFlashPractice,startRescuePractice,startGapPractice,startWritingPractice,startSentencePractice,startMagicTracePractice,startSecretWordPractice,startPuzzlePractice,startDotPractice,resumeSession,continueSession,exitMission,saveAndExitUser,previousActivity,bindActivity,bindSpeechSpeed,toast,playStory,completeStory,openStory,exportProgress,importProgress,learningAudio,rewardBurst,tryFullscreen,refreshPWAControls};
