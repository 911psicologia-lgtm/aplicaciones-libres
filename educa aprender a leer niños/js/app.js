(function(){
  ...
  function bindActivity(q,s){
    ...
    if(q.type==='sentenceWrite'){
      ...
      let mobileCanvas=document.getElementById('sentenceWriteWordCanvas'),completed=false,assisted=false,logged=false,tracer=null,wordIndex=0,wordComplete=false;const replay=isReplay(),phrase=String(q.sentence||q.say||'').trim(),words=phrase.replace(/[.!?]+$/,'').split(/\s+/).filter(Boolean),mobileMode=!!(q.responsiveWords&&mobileCanvas&&window.matchMedia&&window.matchMedia('(max-width:620px)').matches),tokens=[...document.querySelectorAll('.sentence-mobile-token')];
      ...
      const markFinal=async(cov,meta={})=>{if(completed)return;completed=true;tokens.forEach(t=>t.classList.remove('current'));if(wordLabel&&mobileMode)wordLabel.textContent='Frase completa';if(!logged){logged=true;EmiliaStore.event('sentence_write_complete',{sentence:phrase,skill:q.skill,coverage:Math.round((cov||0)*100),manual:!!meta.manual,assisted,mobileWordByWord:mobileMode,words:words.length});}EmiliaVoice.tone('ok');feedback(true);if(!replay){EmiliaMastery.record(q.skill,true,assisted,{writing:true,sentenceWriting:true,responsiveWords:mobileMode});s.hits++;if(!assisted){s.independentHits++;registerSuccess(true);}else registerMiss();}await learningAudio(phrase,{kind:'sentence',repeat:false,button:null,listeningText:'👂',readyText:'●'});if(action){action.disabled=false;action.textContent='➜';action.setAttribute('aria-label','Siguiente');action.classList.add('is-next');}};
  ...
  function exportProgress(){const blob=new Blob([EmiliaStore.exportJSON()],{type:'application/json'}),u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download='emilia_bosque_v1.7.0_'+new Date().toISOString().slice(0,10)+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(u),1000);toast('Copia exportada.');}
